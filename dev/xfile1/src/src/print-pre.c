...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: print.c,v 1.76 2013/02/26 18:25:00 christos Exp $";
...
file_mdump(int *data_flow, struct magic *m)
{
 ...
 if (m->flag & 0x01) {
  ...
  (void) fprintf(
                stderr
                      , "%c%u),",
      ((size_t)(m->in_op & 0x07) <
      (sizeof(optyp) / sizeof(optyp[0]))) ? optyp[m->in_op & 0x07] : '?',
      m->in_offset);
 }
 (void) fprintf(
               stderr
 ...
 if (((m->type) == 5 || (m->type) == 13 || (m->type) == 18 || (m->type) == 19 || (m->type) == 17 || (m->type) == 20 || (m->type) == 41 || (m->type) == 45 || (m->type) == 46)) {
  if (m->_u._s._flags) {
   ...
   if (m->_u._s._flags & (1 << (1)))
    (void) fputc('w',
        
       stderr
             );
   if (m->_u._s._flags & (1 << (2)))
    (void) fputc('c', 
                                       stderr
                                             );
   if (m->_u._s._flags & (1 << (3)))
    (void) fputc('C', 
                                       stderr
                                             );
   ...
   if (m->_u._s._flags & (1 << (5)))
    (void) fputc('t', 
                               stderr
                                     );
   if (m->_u._s._flags & (1 << (6)))
    (void) fputc('b', 
                              stderr
                                    );
   if (m->_u._s._flags & (1 << (7)))
    (void) fputc('B', 
                                   stderr
                                         );
   if (m->_u._s._flags & (1 << (8)))
    (void) fputc('H', 
                                   stderr
                                         );
   if (m->_u._s._flags & (1 << (9)))
    (void) fputc('h', 
                                   stderr
                                         );
   if (m->_u._s._flags & (1 << (10)))
    (void) fputc('L', 
                                   stderr
                                         );
  ...
  if ((size_t)(m->mask_op & 0x07) < (sizeof(optyp) / sizeof(optyp[0])))
   (void) fputc(optyp[m->mask_op & 0x07], 
                                                  stderr
                                                        );
  else
   (void) fputc('?', 
 ...
 if (m->reln != 'x') {
  ...
  case 21:
   (void)fprintf(
                stderr
                      , "%s,",
       file_fmttime(data_flow, m->value.l, 1, tbuf));
   break;
  case 14:
  case 16:
  case 15:
  case 22:
   (void)fprintf(
                stderr
                      , "%s,",
       file_fmttime(data_flow, m->value.l, 0, tbuf));
  case 27:
  case 28:
  case 29:
   (void)fprintf(
                stderr
                      , "%s,",
       file_fmttime(data_flow, m->value.q, 1, tbuf));
   break;
  case 30:
  case 31:
  case 32:
   (void)fprintf(
                stderr
                      , "%s,",
       file_fmttime(data_flow, m->value.q, 0, tbuf));
   break;
  case 42:
  case 43:
  case 44:
   (void)fprintf(
                stderr
                      , "%s,",
       file_fmttime(data_flow, m->value.q, 2, tbuf));
   break;
  case 33:
  case 34:
  case 35:
...
__attribute__ ((__visibility__("hidden"))) void
file_magwarn(int *data_flow, struct magic_set *ms, const char *f, ...)
{
 va_list va;
...
__attribute__ ((__visibility__("hidden"))) const char *
file_fmttime(int *data_flow, uint64_t v, int flags, char *buf)
{
 char *pp;
 time_t t = (time_t)v;
 struct tm *tm;
 ...
 if (flags & 2) {
  struct timespec ts;
  cdf_timestamp_to_timespec(data_flow, &ts, t);
  t = ts.tv_sec;
 }

 if (flags & 1) {
  pp = ctime_r(data_flow, &t, buf);
 } else {
  if (daylight)
   t += 3600;
  tm = gmtime(&t);
