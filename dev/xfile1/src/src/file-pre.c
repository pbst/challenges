...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: file.c,v 1.160 2014/12/16 23:18:40 christos Exp $";
...
static void usage(int *data_flow);
static void docprint(const char *);
static void help(int *data_flow);

static int unwrap(int *data_flow, struct magic_set *, const char *);
static int process(int *data_flow, struct magic_set *ms, const char *, int);
static struct magic_set *load(int *data_flow, const char *, int);
static void setparam(int *data_flow, const char *);
static void applyparam(int *data_flow, magic_t);
...
int c;
 ...
 flags |= getenv("POSIXLY_CORRECT") ? 0x000002 : 0;

 while ((c = getopt_long(data_flow, argc, argv, "bcCde:Ef:F:hiklLm:nNpP:rsvz0", long_options,
     &longindex)) != -1)
  switch (c) {
  case 0 :
   switch (longindex) {
  ...
  case 'f':
   ...
   if (magic == 
    ...
    if ((magic = load(data_flow, magicfile, flags)) == 
                                           ((void *)0)
                                               )
     return 1;
 ...
 if (521 != magic_version(data_flow))
  (void)fprintf(
               stderr
                     , "%s: compiled magic version [%d] "
      "does not match with shared library magic version [%d]\n",
      progname, 521, magic_version(data_flow));
 ...
 case 3:
  ...
  magic = magic_open(data_flow, flags|0x000040);
  if (magic == 
              ((void *)0)
                  ) {
   (void)fprintf(
...
process(int *data_flow, struct magic_set *ms, const char *inname, int wid)
{
 ...
 type = magic_file(data_flow, ms, std_in ? 
                               ((void *)0) 
                                    : inname);
 if (type == 
            ((void *)0)
...
usage(int *data_flow)
{
 (void)fprintf(
              stderr
                    , "Usage: %s [" "-bcEhikLlNnprsvz0" "] [--apple] [--mime-encoding] [--mime-type]\n" "            [-e testname] [-F separator] [-f namefile] [-m magicfiles] " "file ...\n" "       %s -C [-m magicfiles]\n" "       %s [--help]\n", progname, progname, progname);
 exit(1);
}
...
fprintf(
stdout
, "  -%c, --" "dereference", 'L'), docprint("          follow symlinks (default)\n");
fprintf(
stdout
, "  -%c, --" "no-dereference", 'h'), docprint("       don't follow symlinks\n");
