...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: is_tar.c,v 1.37 2010/11/30 14:58:53 rrt Exp $";
...
static int is_tar(int *data_flow, const unsigned char *, size_t);
static int from_oct(int *data_flow, int, const char *);
...
is_tar(int *data_flow, const unsigned char *buf, size_t nbytes)
{
 ...
 if (p) {
data_flow[9] = *(const unsigned int *)p;
}
...
from_oct(int *data_flow, int digs, const char *where)
{
 ...
 if (where) {
data_flow[12] = *(const unsigned int *)where;
}
if (digs > 0 && *where && !
                           ((*__ctype_b_loc ())[(int) ((
                           (unsigned char)*where
