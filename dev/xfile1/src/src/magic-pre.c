...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: magic.c,v 1.91 2014/12/16 23:18:40 christos Exp $";
...
static void close_and_restore(int *data_flow, const struct magic_set *, const char *, int,
    const struct stat *);
static int unreadable_info(int *data_flow, struct magic_set *, mode_t, const char *);
static const char* get_default_magic(int *data_flow);

static const char *file_or_fd(int *data_flow, struct magic_set *, const char *, int);
...
static const char *
get_default_magic(int *data_flow)
{
 ...
 if (asprintf(data_flow, &hmagicpath, "%s/.magic.mgc", home) < 0)
  return "/etc/magic";
 if (stat(hmagicpath, &st) == -1) {
  free(hmagicpath);
  if (asprintf(data_flow, &hmagicpath, "%s/.magic", home) < 0)
   return "/etc/magic";
  if (stat(hmagicpath, &st) == -1)
   goto out;
  if (
   ...
   free(hmagicpath);
   if (asprintf(data_flow, &hmagicpath, "%s/%s", home, hmagic) < 0)
    return "/etc/magic";
   if (access(hmagicpath, 
                         4
                             ) == -1)
    goto out;
  }
 ...
 if (asprintf(data_flow, &default_magic, "%s:%s", hmagicpath, "/etc/magic") < 0)
  goto out;
 free(hmagicpath);
...
magic_getpath(int *data_flow, const char *magicfile, int action)
{
 if (magicfile != 
                 ((void *)0)
                     )
 ...
 return action == 0 ? get_default_magic(data_flow) : "/etc/magic";
}

__attribute__ ((__visibility__("default"))) struct magic_set *
magic_open(int *data_flow, int flags)
{
 return file_ms_alloc(data_flow, flags);
}

static int
unreadable_info(int *data_flow, struct magic_set *ms, mode_t md, const char *file)
{
 if (file) {
  ...
  if (access(file, 
   ...
   if (file_printf(data_flow, ms, "writable, ") == -1)
    return -1;
  if (access(file, 
                  1
                      ) == 0)
...
magic_close(int *data_flow, struct magic_set *ms)
{
 if (ms == 
          ((void *)0)
              )
  return;
 file_ms_free(data_flow, ms);
}
...
magic_load(int *data_flow, struct magic_set *ms, const char *magicfile)
{
 if (ms == 
          ((void *)0)
              )
  return -1;
 return file_apprentice(data_flow, ms, magicfile, 0);
}
...
__attribute__ ((__visibility__("default"))) int
magic_load_buffers(int *data_flow, struct magic_set *ms, void **bufs, size_t *sizes,
    size_t nbufs)
{
 if (ms == 
          ((void *)0)
  ...
  return -1;
 return buffer_apprentice(data_flow, ms, (struct magic **)bufs, sizes, nbufs);
}


__attribute__ ((__visibility__("default"))) int
magic_compile(int *data_flow, struct magic_set *ms, const char *magicfile)
{
 if (ms == 
          ((void *)0)
              )
  return -1;
 return file_apprentice(data_flow, ms, magicfile, 2);
}

__attribute__ ((__visibility__("default"))) int
magic_check(int *data_flow, struct magic_set *ms, const char *magicfile)
{
 if (ms == 
          ((void *)0)
              )
  return -1;
 return file_apprentice(data_flow, ms, magicfile, 1);
}

__attribute__ ((__visibility__("default"))) int
magic_list(int *data_flow, struct magic_set *ms, const char *magicfile)
{
 if (ms == 
          ((void *)0)
              )
  return -1;
 return file_apprentice(data_flow, ms, magicfile, 3);
}

static void
close_and_restore(int *data_flow, const struct magic_set *ms, const char *name, int fd,
...
__attribute__ ((__visibility__("default"))) const char *
magic_descriptor(int *data_flow, struct magic_set *ms, int fd)
{
 if (ms == 
          ((void *)0)
              )
  return 
        ((void *)0)
            ;
 return file_or_fd(data_flow, ms, 
                      ((void *)0)
                          , fd);
}
...
__attribute__ ((__visibility__("default"))) const char *
magic_file(int *data_flow, struct magic_set *ms, const char *inname)
{
 if (ms == 
          ((void *)0)
              )
  return 
        ((void *)0)
            ;
 return file_or_fd(data_flow, ms, inname, 
                              0
                                          );
}

static const char *
file_or_fd(int *data_flow, struct magic_set *ms, const char *inname, int fd)
{
 int rv = -1;
 unsigned char *buf;
 struct stat sb;
 ...
 if ((buf = (unsigned char *)(malloc((256 * 1024) + (1 + sizeof(union VALUETYPE))))) == 
                                                             ((void *)0)
                                                                 )
  return 
        ((void *)0)
 ...
 switch (file_fsmagic(data_flow, ms, inname, &sb)) {
 case -1:
  goto done;
 case 0:
  break;
 ...
 (*__errno_location ()) 
  ...
  if ((fd = open(inname, flags)) < 0) {
   if (okstat &&
       unreadable_info(data_flow, ms, sb.st_mode, inname) == -1)
    goto done;
   rv = 0;
   goto done;
  }
 ...
 if (ispipe) {
  ...
  while ((r = sread(data_flow, fd, (void *)&buf[nbytes],
      (size_t)((256 * 1024) - nbytes), 1)) > 0) {
   nbytes += r;
   if (r < 
          4096
                  ) break;
  ...
  if (nbytes == 0) {
   ...
   if (unreadable_info(data_flow, ms, sb.st_mode, inname) == -1)
    goto done;
   rv = 0;
   goto done;
  }
 ...
 if (buf) {
data_flow[2] = *((const unsigned int *)buf + 2);
}
(void)memset(buf + nbytes, 0, (1 + sizeof(union VALUETYPE)));
 if (buf) {
data_flow[1] = *((const unsigned int *)buf + 1);
}
if (buf) {
data_flow[3] = *((const unsigned int *)buf + 2);
}
if (file_buffer(data_flow, ms, fd, inname, buf, (size_t)nbytes) == -1)
 ...
 close_and_restore(data_flow, ms, inname, fd, &sb);
out:
...
__attribute__ ((__visibility__("default"))) const char *
magic_buffer(int *data_flow, struct magic_set *ms, const void *buf, size_t nb)
{
 if (ms == 
          ((void *)0)
              )
 ...
 return file_getbuffer(data_flow, ms);
}


__attribute__ ((__visibility__("default"))) const char *
magic_error(int *data_flow, struct magic_set *ms)
{
 if (ms == 
          ((void *)0)
              )
  return "Magic database is not open";
 return (ms->event_flags & 0x01) ? ms->o.buf : 
                                                       ((void *)0)
                                                           ;
}

__attribute__ ((__visibility__("default"))) int
magic_errno(int *data_flow, struct magic_set *ms)
{
 if (ms == 
          ((void *)0)
              )
  return 
        22
              ;
 return (ms->event_flags & 0x01) ? ms->error : 0;
}

__attribute__ ((__visibility__("default"))) int
magic_setflags(int *data_flow, struct magic_set *ms, int flags)
{
 if (ms == 
          ((void *)0)
              )
...
__attribute__ ((__visibility__("default"))) int
magic_version(int *data_flow)
{
 return 521;
}

__attribute__ ((__visibility__("default"))) int
magic_setparam(int *data_flow, struct magic_set *ms, int param, const void *val)
{
 switch (param) {
 case 0:
...
__attribute__ ((__visibility__("default"))) int
magic_getparam(int *data_flow, struct magic_set *ms, int param, void *val)
{
 switch (param) {
 case 0:
