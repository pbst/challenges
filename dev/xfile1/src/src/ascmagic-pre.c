...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: ascmagic.c,v 1.91 2014/11/28 02:46:39 christos Exp $";
...
file_ascmagic(int *data_flow, struct magic_set *ms, const unsigned char *buf, size_t nbytes,
 int text)
{
 unichar *ubuf = 
                ((void *)0)
 ...
 if (file_encoding(data_flow, ms, buf, nbytes, &ubuf, &ulen, &code, &code_mime,
     &type) == 0)
  rv = 0;
        else
  rv = file_ascmagic_with_encoding(data_flow, ms, buf, nbytes, ubuf, ulen, code,
       type, text);
...
__attribute__ ((__visibility__("hidden"))) int
file_ascmagic_with_encoding(int *data_flow, struct magic_set *ms, const unsigned char *buf,
    ...
    const char *type, int text)
{
 ...
 if (ulen > 0 && (ms->flags & 0x004000) == 0) {
  ...
  if ((utf8_end = encode_utf8(data_flow, utf8_buf, mlen, ubuf, ulen))
   ...
   goto done;
  if ((rv = file_softmagic(data_flow, ms, utf8_buf,
      (size_t)(utf8_end - utf8_buf), 0, 
                                       ((void *)0)
                                           ,
      0x40, text)) == 0)
   rv = -1;
 ...
 if (mime) {
  if (!file_printedlen(data_flow, ms) && (mime & 0x000010) != 0) {
   if (subtype_mime) {
