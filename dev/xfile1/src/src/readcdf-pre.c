...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: readcdf.c,v 1.49 2014/12/04 15:56:46 christos Exp $";
...
"c_lc_ctype != NULL"
, "readcdf.c", 111, __PRETTY_FUNCTION__))
                          ;
...
((void *)0)) ? (void) (0) : __assert_fail (
"old_lc_ctype != NULL"
, "readcdf.c", 113, __PRETTY_FUNCTION__))
                            ;

 for (i = 0; nv[i].pattern != 
                             ((void *)0)
                                 ; i++)
  if (strcasestr(data_flow, vbuf, nv[i].pattern) != 
                                        ((void *)0)
                                            ) {
   rv = nv[i].mime;
   break;
    ...
    size_t count, const cdf_directory_t *root_storage)
{
  ...
  str = cdf_clsid_to_mime(data_flow, root_storage->d_storage_uuid,
      clsid2mime);
        ...
        for (i = 0; i < count; i++) {
                ...
                case 0x00000001:
                        break;
                case 0x00000002:
                        if ((((ms)->flags & (0x000010|0x000400)) == 0) && file_printf(data_flow, ms, ", %s: %hd", buf,
                            info[i].pi_val._pi_s16) == -1)
                                return -1;
                        break;
                case 0x00000003:
                        if ((((ms)->flags & (0x000010|0x000400)) == 0) && file_printf(data_flow, ms, ", %s: %d", buf,
                            info[i].pi_val._pi_s32) == -1)
                                return -1;
                        break;
                case 0x00000013:
                        if ((((ms)->flags & (0x000010|0x000400)) == 0) && file_printf(data_flow, ms, ", %s: %u", buf,
                            info[i].pi_val._pi_u32) == -1)
                                return -1;
                        break;
                case 0x00000004:
                        if ((((ms)->flags & (0x000010|0x000400)) == 0) && file_printf(data_flow, ms, ", %s: %g", buf,
                            info[i].pi_val._pi_f) == -1)
                                return -1;
                        break;
                case 0x00000005:
                        if ((((ms)->flags & (0x000010|0x000400)) == 0) && file_printf(data_flow, ms, ", %s: %g", buf,
                            info[i].pi_val._pi_d) == -1)
                                return -1;
                        break;
                case 0x0000001e:
                case 0x0000001f:
                        len = info[i].pi_val._pi_str.s_len;
                        if (len > 1) {
                                char vbuf[1024];
                                size_t j, k = 1;

                                if (info[i].pi_type == 0x0000001f)
                                    k++;
                                s = info[i].pi_val._pi_str.s_buf;
                                ...
                                if (j == sizeof(vbuf))
                                        --j;
                                vbuf[j] = '\0';
                                if ((((ms)->flags & (0x000010|0x000400)) == 0)) {
                                        if (vbuf[0]) {
     ...
     str = cdf_app_to_mime(data_flow, vbuf, app2mime);
                        ...
                        break;
                case 0x00000040:
                        tp = info[i].pi_val._pi_tp;
                        if (tp != 0) {
    char tbuf[64];
                                if (tp < 1000000000000000LL) {
                                        cdf_print_elapsed_time(data_flow, tbuf,
                                            sizeof(tbuf), tp);
                                        if ((((ms)->flags & (0x000010|0x000400)) == 0) && file_printf(data_flow, ms,
                                            ", %s: %s", buf, tbuf) == -1)
                                                return -1;
                                } else {
                                        char *c, *ec;
    ...
    size_t nbytes)
{
        ...
        if (ms->flags & 0x000800)
                return 0;
        if ((info.i_buf)) {
data_flow[11] = *(const unsigned int *)(info.i_buf);
}
if (cdf_read_header(data_flow, &info, &h) == -1)
 ...
 if ((i = cdf_read_user_stream(data_flow, &info, &h, &sat, &ssat, &sst, &dir,
  ...
  if (scn.sst_dirlen >= sizeof("HWP Document File") - 1
      && memcmp(scn.sst_tab, "HWP Document File",
      sizeof("HWP Document File") - 1) == 0) {
      if ((((ms)->flags & (0x000010|0x000400)) == 0)) {
   ...
   if ((i = cdf_read_user_stream(data_flow, &info, &h, &sat, &ssat, &sst, &dir, "Catalog", &scn)
                  ) == -1) {
    corrupt = expn;
    if ((i = cdf_read_user_stream(data_flow, &info, &h, &sat, &ssat, &sst, &dir, "EncryptedPackage", &scn)
                                      ) == -1)
 ...
 if (i == 0) {
  const char *str = 
                   ((void *)0)
                       ;
  cdf_directory_t *d;
  char name[(sizeof(d->d_name) / sizeof(d->d_name[0]))];
  size_t j, k;
  ...
  for (j = 0; str == 
   ...
   str = cdf_app_to_mime(data_flow, name,
       (((ms)->flags & (0x000010|0x000400)) == 0) ? name2desc : name2mime);
  }
  if ((((ms)->flags & (0x000010|0x000400)) == 0)) {
   if (str != 
             ((void *)0)
