...
static int
_strncasecmp(int *data_flow, const char *s1, const char *s2, size_t n)
{
 if (n != 0) {
...
char *
strcasestr(int *data_flow, const char *s, const char *find)
{
 char c, sc;
 size_t len;
 ...
 if ((c = *find++) != 0) {
  ...
  } while (_strncasecmp(data_flow, s, find, len) != 0);
  s--;
 }
