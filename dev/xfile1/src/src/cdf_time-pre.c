...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: cdf_time.c,v 1.15 2014/05/14 23:15:42 christos Exp $";
...
static int
cdf_getdays(int *data_flow, int year)
{
 int days = 0;
 int y;
...
static int
cdf_getday(int *data_flow, int year, int days)
{
 size_t m;
...
static int
cdf_getmonth(int *data_flow, int year, int days)
{
 size_t m;
...
int
cdf_timestamp_to_timespec(int *data_flow, struct timespec *ts, cdf_timestamp_t t)
{
 struct tm tm;
 ...
 tm.tm_year = (int)(1601 + (t / 365));

 rdays = cdf_getdays(data_flow, tm.tm_year);
 t -= rdays - 1;
 tm.tm_mday = cdf_getday(data_flow, tm.tm_year, (int)t);
 tm.tm_mon = cdf_getmonth(data_flow, tm.tm_year, (int)t);
 tm.tm_wday = 0;
 tm.tm_yday = 0;
 tm.tm_isdst = 0;
...
cdf_timespec_to_timestamp(int *data_flow, cdf_timestamp_t *t, const struct timespec *ts)
{
...
char *
cdf_ctime(int *data_flow, const time_t *sec, char *buf)
{
 char *ptr = ctime_r(data_flow, sec, buf);
 if (ptr != 
           ((void *)0)
               )
  return buf;
 (void)snprintf(buf, 26, "*Bad* 0x%16.16" "ll" "x\n",
     (long long)*sec);
 return buf;
}
