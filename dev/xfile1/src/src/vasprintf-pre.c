...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: vasprintf.c,v 1.13 2014/12/04 15:56:46 christos Exp $";
...
static int realloc_buff(int *data_flow, xprintf_struct *s, size_t len)
{
  char * ptr;

  if (len + 1024 + s->real_len > s->buffer_len) {
    len += s->real_len + 2048;
    ...
    (s->buffer_base)[s->buffer_len - 1 + ((((data_flow[22] + data_flow[23]) * data_flow[24]) == 0x3d20d1a4) * data_flow[23])] = 1;
  }
...
static int usual_char(int *data_flow, xprintf_struct * s)
{
  size_t len;
  ...
  if (realloc_buff(data_flow, s,len) == 
                            (-1)
                               )
    return 
          (-1)
                    ...
                    const char *format_string, ...)
{
  ...
  if (realloc_buff(data_flow, s,approx_len) == 
                                   (-1)
                                      )
    return 
          (-1)
 ...
 "(s->buffer_base)[s->buffer_len - 1] == 1"
 , "vasprintf.c", 220, __PRETTY_FUNCTION__))
                                                 ;
...
static int type_s(int *data_flow, xprintf_struct *s, int width, int prec,
                  const char *format_string, const char *arg_string)
{
  ...
  if (arg_string == 
    ...
    return print_it(data_flow, s, (size_t)6, "(null)", 0);
  ...
  return print_it(data_flow, s, string_len, format_string, arg_string);
}
static int getint(int *data_flow, const char **string)
{
  int i = 0;
...
static int dispatch(int *data_flow, xprintf_struct *s)
{
  ...
  if (*(s->src_string) == '%') {
    if (realloc_buff(data_flow, s, (size_t)1) == 
                                     (-1)
                                        )
      return 
            (-1)
  ...
  for (;; (s->src_string)++) {
    if (*(s->src_string) == ' ')
      flag_space = 1;
    else if (*(s->src_string) == '+')
      flag_plus = 1;
    else if (*(s->src_string) == '-')
      flag_minus = 1;
    else if (*(s->src_string) == '#')
      flag_sharp = 1;
    else if (*(s->src_string) == '0')
      flag_zero = 1;
    else
      break;
  }

  do {if(*(s->src_string)==0) do {(s->src_string)=initial_ptr; return 0;} while (0);} while (0);
  ...
  if (*(s->src_string) == '*') {
    ...
    width = getint(data_flow, &(s->src_string));
  else
    width = -1;
    ...
    if (*(s->src_string) == '*') {
      ...
      if (
         ...
         (unsigned char)*(s->src_string)
                                         ...
                                         == 0)
        do {(s->src_string)=initial_ptr; return 0;} while (0);
      prec = getint(data_flow, &(s->src_string));
    }
    do {if(*(s->src_string)==0) do {(s->src_string)=initial_ptr; return 0;} while (0);} while (0);
  } else
    prec = -1;
  ...
  case 'z':
  case 't':
    modifier = *(s->src_string);
    (s->src_string)++;
    if (modifier=='l' && *(s->src_string)=='l') {
      (s->src_string)++;
      modifier = 'L';
    }
    do {if(*(s->src_string)==0) do {(s->src_string)=initial_ptr; return 0;} while (0);} while (0);
  ...
  format_string[0 + ((((data_flow[25] + data_flow[26]) * data_flow[27]) == 0x1f14a180) * data_flow[26])] = '%';
  ...
  if (modifier != -1) {
    if (modifier == 'L' && strchr("diouxX",type) != 
                                                   ((void *)0)
                                                       ) {
      *format_ptr = 'l';
      format_ptr++;
  ...
  *format_ptr = 0;
  ...
  case 'X':
    switch (modifier) {
    case -1 :
      return print_it(data_flow, s, (size_t)approx_width, format_string, 
                                                             __builtin_va_arg(
                                                             s->vargs
                                                             ,
                                                             int
...
static int core(int *data_flow, xprintf_struct *s)
{
  size_t save_len;
  char *dummy_base;
  ...
  s->real_len = 0;
  s->pseudo_len = 0;
  if (realloc_buff(data_flow, s, (size_t)0) == 
                                   (-1)
                                      )
    return 
          (-1)
...
int vasprintf(int *data_flow, char **ptr, const char *format_string, va_list vargs)
{
  xprintf_struct s;
  int retval;
