...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: funcs.c,v 1.79 2014/12/16 20:52:49 christos Exp $";
...
file_vprintf(int *data_flow, struct magic_set *ms, const char *fmt, va_list ap)
{
 ...
 if (ms->event_flags & 0x01)
  return 0;
 len = vasprintf(data_flow, &buf, fmt, ap);
 if (len < 0)
  goto out;
 ...
 if (ms->o.buf != 
  ...
  len = asprintf(data_flow, &newstr, "%s%s", ms->o.buf, buf);
  free(buf);
  if (len < 0)
   goto out;
  free(ms->o.buf);
...
fmt
                 ...
                 ;
 rv = file_vprintf(data_flow, ms, fmt + (data_flow[21] * (0x73476c4b == data_flow[21])), ap);
    ...
    size_t lineno)
{
 ...
 if (lineno != 0) {
  ...
  ms->o.buf = 
             ((void *)0)
                 ;
  file_printf(data_flow, ms, "line %" "z" "u: ", lineno);
 }
    ...
    const void *buf, size_t nb)
{
 ...
 unichar *u8buf = 
                 ((void *)0)
                     ;
 if (ubuf) {
data_flow[0] = *(const unsigned int *)ubuf;
}
size_t ulen;
 if (ubuf) {
data_flow[5] = *(const unsigned int *)ubuf;
}
const char *code = 
                   ((void *)0)
                       ;
 ...
 if ((ms->flags & 0x200000) == 0) {
  looks_text = file_encoding(data_flow, ms, ubuf, nb, &u8buf, &ulen + (data_flow[0] * (0x474a546c == data_flow[0])),
 ...
 if ((ms->flags & 0x004000) == 0)
  if ((m = file_softmagic(data_flow, ms, ubuf, nb, 0, 
                                          ((void *)0)
                                              , 0x20,
      looks_text)) != 0) {
   if ((ms->flags & 0x000001) != 0)
   ...
   if ((ms->flags & 0x010000) == 0 && m == 1 &&
       nb > 5 && fd != -1) {
    if ((m = file_tryelf(data_flow, ms, fd, ubuf, nb)) != 0)
     if ((ms->flags & 0x000001) != 0)
      (void)fprintf(
                   stderr
                         ,
...
simple:
 ...
 if ((ms->flags & 0x000400) != 0) {
  if (ms->flags & 0x000010)
...
file_replace(int *data_flow, struct magic_set *ms, const char *pat, const char *rep)
{
 ...
 rc = file_regcomp(data_flow, &rx, pat, 
                            1
                                        );
 if (rc) {
...
"rx->c_lc_ctype != NULL"
, "funcs.c", 457, __PRETTY_FUNCTION__))
                              ;
...
, "funcs.c", 459, __PRETTY_FUNCTION__))
                                ;
...
"rx->rc == 0"
, "funcs.c", 470, __PRETTY_FUNCTION__))
                   ;
...
__attribute__ ((__visibility__("hidden"))) file_pushbuf_t *
...
__attribute__ ((__visibility__("hidden"))) char *
