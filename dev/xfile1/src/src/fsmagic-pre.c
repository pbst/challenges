...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: fsmagic.c,v 1.75 2014/12/04 15:56:46 christos Exp $";
...
static int
bad_link(int *data_flow, struct magic_set *ms, int err, char *buf)
{
 int mime = ms->flags & (0x000010|0x000400);
 if ((mime & 0x000010) &&
     file_printf(data_flow, ms, "inode/symlink")
     == -1)
  return -1;
 else if (!mime) {
  if (ms->flags & 0x000200) {
   file_error(data_flow, ms, err,
       "broken symbolic link to %s", buf);
   return -1;
  }
  if (file_printf(data_flow, ms, "broken symbolic link to %s", buf) == -1)
   return -1;
 }
 return 1;
}
...
static int
handle_mime(int *data_flow, struct magic_set *ms, int mime, const char *str)
{
 if ((mime & 0x000010)) {
  if (file_printf(data_flow, ms, "inode/%s", str) == -1)
   return -1;
  if ((mime & 0x000400) && file_printf(data_flow, ms,
      "; charset=") == -1)
   return -1;
 }
 if ((mime & 0x000400) && file_printf(data_flow, ms, "binary") == -1)
  return -1;
 return 0;
}

__attribute__ ((__visibility__("hidden"))) int
file_fsmagic(int *data_flow, struct magic_set *ms, const char *fn, struct stat *sb)
{
 int ret, did = 0;
 int mime = ms->flags & (0x000010|0x000400);
 ...
 if (ret) {
  ...
  if (file_printf(data_flow, ms, "cannot open `%s' (%s)",
      fn, strerror(
                  (*__errno_location ())
                       )) == -1)
   return -1;
 ...
 if (!mime) {
  ...
  if (sb->st_mode & 
   ...
   if (file_printf(data_flow, ms, "%ssetuid", (did++ ? ", " : "")) == -1)
    return -1;
  ...
  if (sb->st_mode & 
   ...
   if (file_printf(data_flow, ms, "%ssetgid", (did++ ? ", " : "")) == -1)
    return -1;
  ...
  if (sb->st_mode & 
   ...
   if (file_printf(data_flow, ms, "%ssticky", (did++ ? ", " : "")) == -1)
    return -1;
 ...
 case 
     0040000
            :
  if (mime) {
   if (handle_mime(data_flow, ms, mime, "directory") == -1)
    return -1;
  } else if (file_printf(data_flow, ms, "%sdirectory", (did++ ? ", " : "")) == -1)
   return -1;
  break;
 ...
 case 
  ...
  if ((ms->flags & 0x000008) != 0) {
   ret = 0;
   break;
  }
  if (mime) {
   if (handle_mime(data_flow, ms, mime, "chardevice") == -1)
    return -1;
  } else {
   ...
   if (file_printf(data_flow, ms, "%scharacter special (%ld/%ld)",
       (did++ ? ", " : ""), (long)
                   gnu_dev_major (
                   sb->st_rdev
                   )
 ...
 case 
  ...
  if (mime) {
   if (handle_mime(data_flow, ms, mime, "blockdevice") == -1)
    return -1;
  } else {
   ...
   if (file_printf(data_flow, ms, "%sblock special (%ld/%ld)",
       (did++ ? ", " : ""), (long)
                   gnu_dev_major (
                   sb->st_rdev
                   )
 ...
 case 
  ...
  if((ms->flags & 0x000008) != 0)
   break;
  if (mime) {
   if (handle_mime(data_flow, ms, mime, "fifo") == -1)
    return -1;
  } else if (file_printf(data_flow, ms, "%sfifo (named pipe)", (did++ ? ", " : "")) == -1)
   return -1;
  break;
 case 
     0120000
  ...
  if ((nch = readlink(fn, buf, 
   ...
   if (mime) {
    if (handle_mime(data_flow, ms, mime, "symlink") == -1)
     return -1;
   } else if (file_printf(data_flow, ms,
       "%sunreadable symlink `%s' (%s)", (did++ ? ", " : ""), fn,
       strerror(
               (*__errno_location ())
                    )) == -1)
    return -1;
  ...
  if (*buf == '/') {
   ...
   if ((tmp = strrchr(fn, '/')) == 
    ...
    if (tmp - fn + 1 > 
     ...
     if (mime) {
      if (handle_mime(data_flow, ms, mime,
          "x-path-too-long") == -1)
       return -1;
     } else if (file_printf(data_flow, ms,
         "%spath too long: `%s'", (did++ ? ", " : ""),
         fn) == -1)
      return -1;
     break;
    }

    (void)strlcpy(data_flow, buf2, fn, sizeof buf2);
    buf2[tmp - fn + 1] = '\0';

    (void)strlcat(data_flow, buf2, buf, sizeof buf2);
    tmp = buf2;
   }
  ...
  if ((ms->flags & 0x000002) != 0) {
   ...
   if (mime) {
    if (handle_mime(data_flow, ms, mime, "symlink") == -1)
     return -1;
   } else if (file_printf(data_flow, ms, "%ssymbolic link to %s",
       (did++ ? ", " : ""), buf) == -1)
    return -1;
  }
  break;
 ...
 case 
     0140000
             :
  if (mime) {
   if (handle_mime(data_flow, ms, mime, "socket") == -1)
    return -1;
  } else if (file_printf(data_flow, ms, "%ssocket", (did++ ? ", " : "")) == -1)
   return -1;
  break;
 ...
 case 
     0100000
            :
  if ((ms->flags & 0x000008) == 0 && sb->st_size == 0) {
   if (mime) {
    if (handle_mime(data_flow, ms, mime, "x-empty") == -1)
     return -1;
   } else if (file_printf(data_flow, ms, "%sempty", (did++ ? ", " : "")) == -1)
    return -1;
   break;
  }
  ret = 0;
 ...
 default:
  file_error(data_flow, ms, 0, "invalid mode 0%o", sb->st_mode);
  return -1;
