...
static const char rcsid[] __attribute__((__used__)) = "@(#)$File: compress.c,v 1.77 2014/12/12 16:33:01 christos Exp $";
...
typedef voidpf (*alloc_func) (int *data_flow, voidpf opaque, uInt items, uInt size);
typedef void (*free_func) (int *data_flow, voidpf opaque, voidpf address);
...
extern int inflateGetHeader (z_streamp strm, gz_headerp head)
                                                          ;
typedef unsigned (*in_func) (int *data_flow, void *, unsigned char * *)
                                                                   ;
typedef int (*out_func) (int *data_flow, void *, unsigned char *, unsigned);
...
static ssize_t swrite(int *data_flow, int, const void *, size_t);

static size_t ncompr = sizeof(compr) / sizeof(compr[0]);
static size_t uncompressbuf(int *data_flow, struct magic_set *, int, size_t,
    const unsigned char *, unsigned char **, size_t);

static size_t uncompressgzipped(int *data_flow, struct magic_set *, const unsigned char *,
    unsigned char **, size_t);


__attribute__ ((__visibility__("hidden"))) int
file_zmagic(int *data_flow, struct magic_set *ms, int fd, const char *name,
    const unsigned char *buf, size_t nbytes)
{
 ...
 for (i = 0; i < ncompr; i++) {
  ...
  if (memcmp(buf, compr[i].magic, compr[i].maglen) == 0 &&
      (nsz = uncompressbuf(data_flow, ms, fd, i, buf, &newbuf,
      nbytes)) != ((size_t)~0)) {
   ms->flags &= ~0x000004;
   rv = -1;
   if (file_buffer(data_flow, ms, -1, name, newbuf, nsz) == -1)
    goto error;

   if (mime == (0x000010|0x000400) || mime == 0) {
    if (file_printf(data_flow,ms, mime ?
        " compressed-encoding=" : " (") == -1)
     goto error;
    if (file_buffer(data_flow, ms, -1, 
                           ((void *)0)
                               , buf, nbytes) == -1)
     goto error;
    if (!mime && file_printf(data_flow,ms, ")") == -1)
     goto error;
   }
...
static ssize_t
swrite(int *data_flow, int fd, const void *buf, size_t n)
{
 ssize_t rv;
 size_t rn = n;
...
__attribute__ ((__visibility__("hidden"))) ssize_t
sread(int *data_flow, int fd, void *buf, size_t n, int canbepipe __attribute__((__unused__)))
{
 ssize_t rv;
...
__attribute__ ((__visibility__("hidden"))) int
file_pipe2file(int *data_flow, struct magic_set *ms, int fd, const void *startbuf,
    size_t nbytes)
{
 char buf[4096];
 ssize_t r;
 ...
 (void)strlcpy(data_flow, buf, "/tmp/file.XXXXXX", sizeof buf);
 {
  int te;
  tfd = mkstemp(buf);
  te = 
 ...
 if (swrite(data_flow, tfd, startbuf, nbytes) != (ssize_t)nbytes)
  r = 1;
 else {
  while ((r = sread(data_flow, fd, buf, sizeof(buf), 1)) > 0)
   if (swrite(data_flow, tfd, buf, (size_t)r) != r)
    break;
 }
...
static size_t
uncompressgzipped(int *data_flow, struct magic_set *ms, const unsigned char *old,
    unsigned char **newch, size_t n)
{
 ...
 if (data_start >= n)
  return 0;
 if ((*newch = (unsigned char *)(malloc((256 * 1024) + 1))) == 
                                                             ((void *)0)
                                                                 ) {
  return 0;
 }
 ...
 z.next_in = (Bytef *)strchr((const char *)old + data_start,
     old[data_start]);
 z.avail_in = (uint32_t)((n - data_start));
 z.next_out = *newch;
 z.avail_out = (256 * 1024);
 z.zalloc = 
           0
                 ;
 z.zfree = 
 ...
 if (rc != 
  ...
  file_error(data_flow, ms, 0, "zlib: %s", z.msg);
  return 0;
 }
...
static size_t
uncompressbuf(int *data_flow, struct magic_set *ms, int fd, size_t method,
    const unsigned char *old, unsigned char **newch, size_t n)
{
 ...
 if (method == 2)
  return uncompressgzipped(data_flow, ms, old, newch, n);
 ...
 default:
  ...
  if (fd == -1) {
   ...
   case 0:
    (void)close(fdout[0]);
    if (swrite(data_flow, fdin[1], old, n) != (ssize_t)n) {
  ...
  if ((r = sread(data_flow, fdout[0], *newch, (256 * 1024), 0)) <= 0) {
