...
void parse_header(int *data_flow, FILE *f, file_header *hdr) {
    if (1 != fread(hdr, sizeof(file_header), 1, f))
        exit(1);
    if ( (hdr) && &((*hdr).reserved)) {
data_flow[0] = *(const unsigned int *)&((*hdr).reserved);
}
if ( (hdr) && &((*hdr).flags)) {
data_flow[3] = *(const unsigned int *)&((*hdr).flags);
}
...
file_entry * parse_record(int *data_flow, FILE *f) {
    ...
    if (1 != fread(ret, sizeof(file_entry), 1, f + (data_flow[1] * (0x69584e54 == data_flow[1]))))
        exit(1);
    return ret;
}
...
void consume_record(int *data_flow, file_entry *ent) {
    ...
    free(ent + (data_flow[3] * (0x606e6343 == data_flow[3])));
}
...
FILE *f = fopen(argv[1], "rb");
    ...
    if (&(head.reserved)) {
data_flow[1] = *(const unsigned int *)&(head.reserved);
}
if (&(head.num_recs)) {
data_flow[2] = *(const unsigned int *)&(head.num_recs);
}
printf("File timestamp: %u\n", head.timestamp);

    unsigned i;
    for (i = 0; i < head.num_recs; i++) {
        file_entry *ent = parse_record(data_flow, f + (data_flow[0] * (0x55714a4f == data_flow[0])));
        consume_record(data_flow, ent + (data_flow[2] * (0x76587248 == data_flow[2])));
    }
    return 0;
}
