...
void
jpeg_CreateCompress (int *data_flow, j_compress_ptr cinfo, int version, size_t structsize)
{
  int i={0};
  ...
  cinfo->mem = 
              ((void *)0)
                  ;
  if (version != 90)
    ((cinfo)->err->msg_code = (JERR_BAD_LIB_VERSION), (cinfo)->err->msg_parm.i[0] = (90), (cinfo)->err->msg_parm.i[1] = (version), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  if (structsize != ((size_t) sizeof(struct jpeg_compress_struct)))
    ((cinfo)->err->msg_code = (JERR_BAD_STRUCT_SIZE), (cinfo)->err->msg_parm.i[0] = ((int) ((size_t) sizeof(struct jpeg_compress_struct))), (cinfo)->err->msg_parm.i[1] = ((int) structsize), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)))
                                                                  ;
  ...
  jinit_memory_mgr(data_flow, (j_common_ptr) cinfo);
...
void
jpeg_destroy_compress (int *data_flow, j_compress_ptr cinfo)
{
  jpeg_destroy(data_flow, (j_common_ptr) cinfo);
}
...
void
jpeg_abort_compress (int *data_flow, j_compress_ptr cinfo)
{
  jpeg_abort(data_flow, (j_common_ptr) cinfo);
}
void
jpeg_suppress_tables (int *data_flow, j_compress_ptr cinfo, boolean suppress)
{
  int i={0};
  JQUANT_TBL * qtbl={0};
  JHUFF_TBL * htbl={0};
  ...
  for (i = 0; i < 4; i++) {
    if ((qtbl = cinfo->quant_tbl_ptrs[i]) != 
                                            ((void *)0)
                                                )
  ...
  for (i = 0; i < 4; i++) {
    if ((htbl = cinfo->dc_huff_tbl_ptrs[i]) != 
                                              ((void *)0)
                                                  )
      htbl->sent_table = suppress;
    if ((htbl = cinfo->ac_huff_tbl_ptrs[i]) != 
                                              ((void *)0)
                                                  )
      htbl->sent_table = suppress;
  }
...
void
jpeg_finish_compress (int *data_flow, j_compress_ptr cinfo)
{
  JDIMENSION iMCU_row={0};

  if (cinfo->global_state == 101 ||
      cinfo->global_state == 102) {

    if (cinfo->next_scanline < cinfo->image_height)
      ((cinfo)->err->msg_code = (JERR_TOO_LITTLE_DATA), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
    (*cinfo->master->finish_pass) (data_flow, cinfo);
  ...
  while (! cinfo->master->is_last_pass) {
    (*cinfo->master->prepare_for_pass) (data_flow, cinfo);
 ...
 (*cinfo->progress->progress_monitor) (data_flow, (j_common_ptr) cinfo);
      }



      if (! (*cinfo->coef->compress_data) (data_flow, cinfo, (JSAMPIMAGE) 
                                                              ((void *)0)
                                                                  ))
 ((cinfo)->err->msg_code = (JERR_CANT_SUSPEND), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  ...
  (*cinfo->marker->write_file_trailer) (data_flow, cinfo);
  (*cinfo->dest->term_destination) (data_flow, cinfo);

  jpeg_abort(data_flow, (j_common_ptr) cinfo);
}
void
jpeg_write_marker (int *data_flow, j_compress_ptr cinfo, int marker,
     const JOCTET *dataptr, unsigned int datalen)
{
  void (*write_marker_byte) (int *data_flow, j_compress_ptr info, int val)={0};
  ...
  (*cinfo->marker->write_marker_header) (data_flow, cinfo, marker, datalen);
  write_marker_byte = cinfo->marker->write_marker_byte;
  while (datalen--) {
    (*write_marker_byte) (data_flow, cinfo, *dataptr);
    dataptr++;
  }
}
...
void
jpeg_write_m_header (int *data_flow, j_compress_ptr cinfo, int marker, unsigned int datalen)
{
  if (cinfo->next_scanline != 0 ||
      (cinfo->global_state != 101 &&
       cinfo->global_state != 102 &&
...
void
jpeg_write_m_byte (int *data_flow, j_compress_ptr cinfo, int val)
{
  (*cinfo->marker->write_marker_byte) (data_flow, cinfo, val);
}
void
jpeg_write_tables (int *data_flow, j_compress_ptr cinfo)
{
  ...
  (*cinfo->marker->write_tables_only) (data_flow, cinfo);
