...
typedef void (*downsample1_ptr) (int *data_flow, j_compress_ptr cinfo, jpeg_component_info * compptr, JSAMPARRAY input_data, JSAMPARRAY output_data)
...
static void
start_pass_downsample (int *data_flow, j_compress_ptr cinfo)
{
...
static void
expand_right_edge (int *data_flow, JSAMPARRAY image_data, int num_rows,
     JDIMENSION input_cols, JDIMENSION output_cols)
{
  register JSAMPROW ptr={0};
  register JSAMPLE pixval={0};
...
static void
sep_downsample (int *data_flow, j_compress_ptr cinfo,
  ...
  JSAMPIMAGE output_buf, JDIMENSION out_row_group_index)
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    (*downsample->methods[ci]) (data_flow, cinfo, compptr, in_ptr, out_ptr);
  }
}
static void
int_downsample (int *data_flow, j_compress_ptr cinfo, jpeg_component_info * compptr,
  JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  my_downsample_ptr downsample = (my_downsample_ptr) cinfo->downsample;
  int inrow, outrow, h_expand, v_expand, numpix, numpix2, h, v={0};
  JDIMENSION outcol, outcol_h={0};
  JDIMENSION output_cols = compptr->width_in_blocks * compptr->DCT_h_scaled_size;
  JSAMPROW inptr, outptr={0};
  INT32 outvalue={0};
  ...
  expand_right_edge(data_flow, input_data, cinfo->max_v_samp_factor,
...
static void
fullsize_downsample (int *data_flow, j_compress_ptr cinfo, jpeg_component_info * compptr,
       JSAMPARRAY input_data, JSAMPARRAY output_data)
{

  jcopy_sample_rows(data_flow, input_data, 0, output_data, 0,
      cinfo->max_v_samp_factor, cinfo->image_width);

  expand_right_edge(data_flow, output_data, cinfo->max_v_samp_factor, cinfo->image_width,
      compptr->width_in_blocks * compptr->DCT_h_scaled_size);
}
static void
h2v1_downsample (int *data_flow, j_compress_ptr cinfo, jpeg_component_info * compptr,
   JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  int inrow={0};
  JDIMENSION outcol={0};
...
static void
h2v2_downsample (int *data_flow, j_compress_ptr cinfo, jpeg_component_info * compptr,
   JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  int inrow, outrow={0};
  JDIMENSION outcol={0};
  JDIMENSION output_cols = compptr->width_in_blocks * compptr->DCT_h_scaled_size;
  register JSAMPROW inptr0, inptr1, outptr={0};
  register int bias={0};
...
static void
h2v2_smooth_downsample (int *data_flow, j_compress_ptr cinfo, jpeg_component_info * compptr,
   JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  int inrow, outrow={0};
  JDIMENSION colctr={0};
  JDIMENSION output_cols = compptr->width_in_blocks * compptr->DCT_h_scaled_size;
  register JSAMPROW inptr0, inptr1, above_ptr, below_ptr, outptr={0};
  INT32 membersum, neighsum, memberscale, neighscale={0};





  expand_right_edge(data_flow, input_data - 1, cinfo->max_v_samp_factor + 2,
      cinfo->image_width, output_cols * 2);
...
static void
fullsize_smooth_downsample (int *data_flow, j_compress_ptr cinfo, jpeg_component_info *compptr,
       JSAMPARRAY input_data, JSAMPARRAY output_data)
{
  int inrow={0};
  JDIMENSION colctr={0};
  JDIMENSION output_cols = compptr->width_in_blocks * compptr->DCT_h_scaled_size;
  register JSAMPROW inptr, above_ptr, below_ptr, outptr={0};
  INT32 membersum, neighsum, memberscale, neighscale={0};
  int colsum, lastcolsum, nextcolsum={0};
...
void
jinit_downsampler (int *data_flow, j_compress_ptr cinfo)
{
  my_downsample_ptr downsample={0};
  int ci={0};
  jpeg_component_info * compptr={0};
  boolean smoothok = TRUE;
  int h_in_group, v_in_group, h_out_group, v_out_group={0};
  ...
  if (cinfo->CCIR601_sampling)
    ((cinfo)->err->msg_code = (JERR_CCIR601_NOTIMPL), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
 ...
 downsample->methods[ci] = h2v2_downsample;
      ...
      ((cinfo)->err->msg_code = (JERR_FRACT_SAMPLE_NOTIMPL), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  }


  if (cinfo->smoothing_factor && !smoothok)
    ((cinfo)->err->msg_code = (JTRC_SMOOTH_NOTIMPL), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), (0)));
