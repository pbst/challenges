...
static void
initial_setup (int *data_flow, j_compress_ptr cinfo)

{
  ...
  if (cinfo->block_size < 1 || cinfo->block_size > 16)
    ((cinfo)->err->msg_code = (JERR_BAD_DCTSIZE), (cinfo)->err->msg_parm.i[0] = (cinfo->block_size), (cinfo)->err->msg_parm.i[1] = (cinfo->block_size), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
 ...
 compptr->DCT_v_scaled_size = compptr->DCT_h_scaled_size * 2;
    ...
    compptr->width_in_blocks = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->jpeg_width * (long) compptr->h_samp_factor,
      (long) (cinfo->max_h_samp_factor * cinfo->block_size));
    compptr->height_in_blocks = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->jpeg_height * (long) compptr->v_samp_factor,
      (long) (cinfo->max_v_samp_factor * cinfo->block_size));

    compptr->downsampled_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->jpeg_width *
      (long) (compptr->h_samp_factor * compptr->DCT_h_scaled_size),
      (long) (cinfo->max_h_samp_factor * cinfo->block_size));
    compptr->downsampled_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->jpeg_height *
...
static void
validate_script (int *data_flow, j_compress_ptr cinfo)



{
  const jpeg_scan_info * scanptr={0};
  int scanno, ncomps, ci, coefi, thisi={0};
  int Ss, Se, Ah, Al={0};
  boolean component_sent[10]={0};

  int * last_bitpos_ptr={0};
  int last_bitpos[10][64]={0};



  if (cinfo->num_scans <= 0)
    ((cinfo)->err->msg_code = (JERR_BAD_SCAN_SCRIPT), (cinfo)->err->msg_parm.i[0] = (0), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
 ...
 *last_bitpos_ptr++ = -1;
  ...
  for (scanno = 1; scanno <= cinfo->num_scans; scanptr++, scanno++) {
    ...
    if (ncomps <= 0 || ncomps > 4)
      ((cinfo)->err->msg_code = (JERR_COMPONENT_COUNT), (cinfo)->err->msg_parm.i[0] = (ncomps), (cinfo)->err->msg_parm.i[1] = (4), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
    for (ci = 0; ci < ncomps; ci++) {
      thisi = scanptr->component_index[ci];
      if (thisi < 0 || thisi >= cinfo->num_components)
 ((cinfo)->err->msg_code = (JERR_BAD_SCAN_SCRIPT), (cinfo)->err->msg_parm.i[0] = (scanno), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
 ...
 ((cinfo)->err->msg_code = (JERR_BAD_PROG_SCRIPT), (cinfo)->err->msg_parm.i[0] = (scanno), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
      if (Ss == 0) {
 if (Se != 0)
   ((cinfo)->err->msg_code = (JERR_BAD_PROG_SCRIPT), (cinfo)->err->msg_parm.i[0] = (scanno), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
      } else {
 ...
 ((cinfo)->err->msg_code = (JERR_MISSING_DATA), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
    }
...
static void
reduce_script (int *data_flow, j_compress_ptr cinfo)
...
static void
select_scan_parameters (int *data_flow, j_compress_ptr cinfo)
 ...
 &cinfo->comp_info[scanptr->component_index[ci]];
...

  {
    ...
    if (cinfo->num_components > 4)
      ((cinfo)->err->msg_code = (JERR_COMPONENT_COUNT), (cinfo)->err->msg_parm.i[0] = (cinfo->num_components), (cinfo)->err->msg_parm.i[1] = (4), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)))
                          ;
...
static void
per_scan_setup (int *data_flow, j_compress_ptr cinfo)
...
static void
prepare_for_pass (int *data_flow, j_compress_ptr cinfo)
{
  my_master_ptr master = (my_master_ptr) cinfo->master;
  ...
  case main_pass:
    ...
    select_scan_parameters(data_flow, cinfo);
    per_scan_setup(data_flow, cinfo);
    if (! cinfo->raw_data_in) {
      (*cinfo->cconvert->start_pass) (data_flow, cinfo);
      (*cinfo->downsample->start_pass) (data_flow, cinfo);
      (*cinfo->prep->start_pass) (data_flow, cinfo, JBUF_PASS_THRU);
    }
    (*cinfo->fdct->start_pass) (data_flow, cinfo);
    (*cinfo->entropy->start_pass) (data_flow, cinfo, cinfo->optimize_coding);
    (*cinfo->coef->start_pass) (data_flow, cinfo,
    (master->total_passes > 1 ?
     JBUF_SAVE_AND_PASS : JBUF_PASS_THRU));
    (*cinfo->main->start_pass) (data_flow, cinfo, JBUF_PASS_THRU);
  ...
  case huff_opt_pass:
    ...
    if (cinfo->Ss != 0 || cinfo->Ah == 0) {
      (*cinfo->entropy->start_pass) (data_flow, cinfo, TRUE);
      (*cinfo->coef->start_pass) (data_flow, cinfo, JBUF_CRANK_DEST);
      master->pub.call_pass_startup = FALSE;
      break;
    }
  ...
  case output_pass:
    ...
    (*cinfo->entropy->start_pass) (data_flow, cinfo, FALSE);
    (*cinfo->coef->start_pass) (data_flow, cinfo, JBUF_CRANK_DEST);

    if (master->scan_number == 0)
      (*cinfo->marker->write_frame_header) (data_flow, cinfo);
    (*cinfo->marker->write_scan_header) (data_flow, cinfo);
    master->pub.call_pass_startup = FALSE;
    break;
  default:
    ((cinfo)->err->msg_code = (JERR_NOT_COMPILED), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  }
...
static void
pass_startup (int *data_flow, j_compress_ptr cinfo)
{
...
static void
finish_pass_master (int *data_flow, j_compress_ptr cinfo)
{
  my_master_ptr master = (my_master_ptr) cinfo->master;
...
void
jinit_c_master_control (int *data_flow, j_compress_ptr cinfo, boolean transcode_only)
{
  my_master_ptr master={0};
  ...
  if (cinfo->scan_info != 
    ...
    validate_script(data_flow, cinfo);
    if (cinfo->block_size < 8)
      reduce_script(data_flow, cinfo);
