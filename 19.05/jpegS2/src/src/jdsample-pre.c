...
typedef void (*upsample1_ptr) (int *data_flow, j_decompress_ptr cinfo, jpeg_component_info * compptr, JSAMPARRAY input_data, JSAMPARRAY * output_data_ptr)
...
static void
start_pass_upsample (int *data_flow, j_decompress_ptr cinfo)
{
...
static void
sep_upsample (int *data_flow, j_decompress_ptr cinfo,
       ...
       JDIMENSION out_rows_avail)
{
  ...
  ci++, compptr++) {
      ...
      (*upsample->methods[ci]) (data_flow, cinfo, compptr,
...
static void
fullsize_upsample (int *data_flow, j_decompress_ptr cinfo, jpeg_component_info * compptr,
...
static void
noop_upsample (int *data_flow, j_decompress_ptr cinfo, jpeg_component_info * compptr,
        JSAMPARRAY input_data, JSAMPARRAY * output_data_ptr)
{
  *output_data_ptr = 
                    ((void *)0)
...
static void
int_upsample (int *data_flow, j_decompress_ptr cinfo, jpeg_component_info * compptr,
       JSAMPARRAY input_data, JSAMPARRAY * output_data_ptr)
{
  my_upsample_ptr upsample = (my_upsample_ptr) cinfo->upsample;
  JSAMPARRAY output_data = *output_data_ptr;
  register JSAMPROW inptr, outptr={0};
  register JSAMPLE invalue={0};
  register int h={0};
  JSAMPROW outend={0};
  int h_expand, v_expand={0};
  int inrow, outrow={0};
 ...
 *outptr++ = invalue;
    ...
    if (v_expand > 1) {
      jcopy_sample_rows(data_flow, output_data, outrow, output_data, outrow+1,
...
static void
h2v1_upsample (int *data_flow, j_decompress_ptr cinfo, jpeg_component_info * compptr,
        JSAMPARRAY input_data, JSAMPARRAY * output_data_ptr)
{
  JSAMPARRAY output_data = *output_data_ptr;
  register JSAMPROW inptr, outptr={0};
...
static void
h2v2_upsample (int *data_flow, j_decompress_ptr cinfo, jpeg_component_info * compptr,
        JSAMPARRAY input_data, JSAMPARRAY * output_data_ptr)
{
  JSAMPARRAY output_data = *output_data_ptr;
  register JSAMPROW inptr, outptr={0};
...
void
jinit_upsampler (int *data_flow, j_decompress_ptr cinfo)
{
   ...
   cinfo->min_DCT_v_scaled_size;
    ...
    upsample->color_buf[ci] = (*cinfo->mem->alloc_sarray)
      (data_flow, (j_common_ptr) cinfo, 1,
       (JDIMENSION) jround_up(data_flow, (long) cinfo->output_width,
