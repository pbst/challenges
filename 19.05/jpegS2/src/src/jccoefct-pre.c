...
static boolean compress_data
    (int *data_flow, j_compress_ptr cinfo, JSAMPIMAGE input_buf);
...
static boolean
compress_data (int *data_flow, j_compress_ptr cinfo, JSAMPIMAGE input_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  JDIMENSION MCU_col_num={0};
  JDIMENSION last_MCU_col = cinfo->MCUs_per_row - 1;
  JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
  int blkn, bi, ci, yindex, yoffset, blockcnt={0};
  JDIMENSION ypos, xpos={0};
  jpeg_component_info *compptr={0};
  forward_DCT_ptr forward_DCT={0};
 ...
 for (yindex = 0; yindex < compptr->MCU_height; yindex++) {
   if (coef->iMCU_row_num < last_iMCU_row ||
     ...
     (*forward_DCT) (data_flow, cinfo, compptr,
  ...
  coef->MCU_buffer[blkn+bi][0][0] = coef->MCU_buffer[blkn+bi-1][0][0];
   ...
   ypos += compptr->DCT_v_scaled_size;
      ...
      if (! (*cinfo->entropy->encode_mcu) (data_flow, cinfo, coef->MCU_buffer)) {
...
static boolean
compress_first_pass (int *data_flow, j_compress_ptr cinfo, JSAMPIMAGE input_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
  JDIMENSION blocks_across, MCUs_across, MCUindex={0};
  int bi, ci, h_samp_factor, block_row, block_rows, ndummy={0};
  JCOEF lastDC={0};
  jpeg_component_info *compptr={0};
  JBLOCKARRAY buffer={0};
  JBLOCKROW thisblockrow, lastblockrow={0};
  forward_DCT_ptr forward_DCT={0};
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    for (block_row = 0; block_row < block_rows; block_row++) {
      ...
      (*forward_DCT) (data_flow, cinfo, compptr, input_buf[ci], thisblockrow,
        (JDIMENSION) (block_row * compptr->DCT_v_scaled_size),
        (JDIMENSION) 0, blocks_across);
      if (ndummy > 0) {

 thisblockrow += blocks_across;
 memset((void *)((void *) thisblockrow), 0, (size_t)(ndummy * ((size_t) sizeof(JBLOCK))));
 ...
 memset((void *)((void *) thisblockrow), 0, (size_t)((size_t) (blocks_across * ((size_t) sizeof(JBLOCK)))))
                                             ;
 for (MCUindex = 0; MCUindex < MCUs_across; MCUindex++) {
  ...
  return compress_output(data_flow, cinfo, input_buf);
}
static boolean
...
void
jinit_c_coef_controller (int *data_flow, j_compress_ptr cinfo, boolean need_full_buffer)
{
  my_coef_ptr coef={0};
 ...
 (data_flow, (j_common_ptr) cinfo, 1, FALSE,
