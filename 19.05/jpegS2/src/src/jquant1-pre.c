...
static int
select_ncolors (int *data_flow, j_decompress_ptr cinfo, int Ncolors[])
...

{
  ...
  if (iroot < 2)
    ((cinfo)->err->msg_code = (JERR_QUANT_FEW_COLORS), (cinfo)->err->msg_parm.i[0] = ((int) temp), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
...
static int
output_value (int *data_flow, j_decompress_ptr cinfo, int ci, int j, int maxj)
...
static int
largest_input_value (int *data_flow, j_decompress_ptr cinfo, int ci, int j, int maxj)
...
static void
create_colormap (int *data_flow, j_decompress_ptr cinfo)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  JSAMPARRAY colormap={0};
  int total_colors={0};
  int i,j,k, nci, blksize, blkdist, ptr, val={0};


  total_colors = select_ncolors(data_flow, cinfo, cquantize->Ncolors);


  if (cinfo->out_color_components == 3)
    do { int * _mp = (cinfo)->err->msg_parm.i; _mp[0] = (total_colors); _mp[1] = (cquantize->Ncolors[0]); _mp[2] = (cquantize->Ncolors[1]); _mp[3] = (cquantize->Ncolors[2]); (cinfo)->err->msg_code = (JTRC_QUANT_3_NCOLORS); (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), (1)); } while (0)

                                                   ;
  else
    ((cinfo)->err->msg_code = (JTRC_QUANT_NCOLORS), (cinfo)->err->msg_parm.i[0] = (total_colors), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), (1)));
  ...
  for (i = 0; i < cinfo->out_color_components; i++) {
    ...
    for (j = 0; j < nci; j++) {
      ...
      val = output_value(data_flow, cinfo, i, j, nci-1);
...
static void
create_colorindex (int *data_flow, j_decompress_ptr cinfo)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  JSAMPROW indexptr={0};
  int i,j,k, nci, blksize, val, pad={0};
  ...
  for (i = 0; i < cinfo->out_color_components; i++) {
    ...
    val = 0;
    k = largest_input_value(data_flow, cinfo, i, 0, nci-1);
    for (j = 0; j <= 255; j++) {
      while (j > k)
 k = largest_input_value(data_flow, cinfo, i, ++val, nci-1);
...
make_odither_array (int *data_flow, j_decompress_ptr cinfo, int ncolors)
{
  ODITHER_MATRIX_PTR odither={0};
  int j,k={0};
  INT32 num,den={0};
...
static void
create_odither_tables (int *data_flow, j_decompress_ptr cinfo)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  ODITHER_MATRIX_PTR odither={0};
  int i, j, nci={0};
 ...
 break;
    ...
    if (odither == 
      ...
      odither = make_odither_array(data_flow, cinfo, nci);
...
static void
color_quantize (int *data_flow, j_decompress_ptr cinfo, JSAMPARRAY input_buf,
  JSAMPARRAY output_buf, int num_rows)

{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  JSAMPARRAY colorindex = cquantize->colorindex;
  register int pixcode, ci={0};
  register JSAMPROW ptrin, ptrout={0};
  int row={0};
  JDIMENSION col={0};
...
static void
color_quantize3 (int *data_flow, j_decompress_ptr cinfo, JSAMPARRAY input_buf,
   JSAMPARRAY output_buf, int num_rows)
...
static void
quantize_ord_dither (int *data_flow, j_decompress_ptr cinfo, JSAMPARRAY input_buf,
       JSAMPARRAY output_buf, int num_rows)

{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  register JSAMPROW input_ptr={0};
  register JSAMPROW output_ptr={0};
  JSAMPROW colorindex_ci={0};
  int * dither={0};
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    memset((void *)((void *) output_buf[row]), 0, (size_t)((size_t) (width * ((size_t) sizeof(JSAMPLE)))))
                                         ;
...
static void
quantize3_ord_dither (int *data_flow, j_decompress_ptr cinfo, JSAMPARRAY input_buf,
        JSAMPARRAY output_buf, int num_rows)
...
static void
quantize_fs_dither (int *data_flow, j_decompress_ptr cinfo, JSAMPARRAY input_buf,
      JSAMPARRAY output_buf, int num_rows)

{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  register LOCFSERROR cur={0};
  LOCFSERROR belowerr={0};
  LOCFSERROR bpreverr={0};
  LOCFSERROR bnexterr={0};
  LOCFSERROR delta={0};
  register FSERRPTR errorptr={0};
  register JSAMPROW input_ptr={0};
  register JSAMPROW output_ptr={0};
  JSAMPROW colorindex_ci={0};
  JSAMPROW colormap_ci={0};
  int pixcode={0};
  int nc = cinfo->out_color_components;
  int dir={0};
  int dirnc={0};
...
static void
alloc_fs_workspace (int *data_flow, j_decompress_ptr cinfo)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  size_t arraysize={0};
  int i={0};
  ...
  for (i = 0; i < cinfo->out_color_components; i++) {
    cquantize->fserrors[i] = (FSERRPTR)
      (*cinfo->mem->alloc_large)(data_flow, (j_common_ptr) cinfo, 1, arraysize);
  }
}
...
static void
start_pass_1_quant (int *data_flow, j_decompress_ptr cinfo, boolean is_pre_scan)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  size_t arraysize={0};
  int i={0};
  ...
  case JDITHER_ORDERED:
    ...
    if (! cquantize->is_padded)
      create_colorindex(data_flow, cinfo);

    if (cquantize->odither[0] == 
                                ((void *)0)
                                    )
      create_odither_tables(data_flow, cinfo);
    break;
  case JDITHER_FS:
    cquantize->pub.color_quantize = quantize_fs_dither;
    cquantize->on_odd_row = FALSE;

    if (cquantize->fserrors[0] == 
                                 ((void *)0)
                                     )
      alloc_fs_workspace(data_flow, cinfo);

    arraysize = (size_t) ((cinfo->output_width + 2) * ((size_t) sizeof(FSERROR)));
    for (i = 0; i < cinfo->out_color_components; i++)
      memset((void *)((void *) cquantize->fserrors[i]), 0, (size_t)(arraysize));
    break;
  default:
...
static void
finish_pass_1_quant (int *data_flow, j_decompress_ptr cinfo)
{
...
static void
new_color_map_1_quant (int *data_flow, j_decompress_ptr cinfo)
{
...
void
jinit_1pass_quantizer (int *data_flow, j_decompress_ptr cinfo)
{
  ...
  if (cinfo->out_color_components > 4)
    ((cinfo)->err->msg_code = (JERR_QUANT_COMPONENTS), (cinfo)->err->msg_parm.i[0] = (4), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  ...
  create_colormap(data_flow, cinfo);
  create_colorindex(data_flow, cinfo);
