...
static void
forward_DCT (int *data_flow, j_compress_ptr cinfo, jpeg_component_info * compptr,
...

{
  ...
  for (bi = 0; bi < num_blocks; bi++, start_col += compptr->DCT_h_scaled_size) {
    ...
    (*do_dct) (data_flow, workspace, sample_data, start_col);
...
static void
forward_DCT_float (int *data_flow, j_compress_ptr cinfo, jpeg_component_info * compptr,
     JSAMPARRAY sample_data, JBLOCKROW coef_blocks,
     JDIMENSION start_row, JDIMENSION start_col,
     JDIMENSION num_blocks)
...
static void
start_pass_fdctmgr (int *data_flow, j_compress_ptr cinfo)
{
  my_fdct_ptr fdct = (my_fdct_ptr) cinfo->fdct;
  int ci, qtblno, i={0};
 ...
 break;
      ...
      break;
    default:
      ((cinfo)->err->msg_code = (JERR_BAD_DCTSIZE), (cinfo)->err->msg_parm.i[0] = (compptr->DCT_h_scaled_size), (cinfo)->err->msg_parm.i[1] = (compptr->DCT_v_scaled_size), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)))
                                                               ;
      break;
    }
...
void
jinit_forward_dct (int *data_flow, j_compress_ptr cinfo)
{
  my_fdct_ptr fdct={0};
  int ci={0};
  jpeg_component_info *compptr={0};
