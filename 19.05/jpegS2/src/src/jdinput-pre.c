...
static int consume_markers (int *data_flow, j_decompress_ptr cinfo);
void
jpeg_core_output_dimensions (int *data_flow, j_decompress_ptr cinfo)
...

{
  ...
  if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom) {
    ...
    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 1;
    cinfo->min_DCT_v_scaled_size = 1;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 2) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 2L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 2L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 2;
    cinfo->min_DCT_v_scaled_size = 2;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 3) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 3L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 3L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 3;
    cinfo->min_DCT_v_scaled_size = 3;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 4) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 4L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 4L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 4;
    cinfo->min_DCT_v_scaled_size = 4;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 5) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 5L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 5L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 5;
    cinfo->min_DCT_v_scaled_size = 5;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 6) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 6L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 6L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 6;
    cinfo->min_DCT_v_scaled_size = 6;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 7) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 7L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 7L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 7;
    cinfo->min_DCT_v_scaled_size = 7;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 8) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 8L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
    ...
    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 9L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 9L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 9;
    cinfo->min_DCT_v_scaled_size = 9;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 10) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 10L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 10L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 10;
    cinfo->min_DCT_v_scaled_size = 10;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 11) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 11L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 11L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 11;
    cinfo->min_DCT_v_scaled_size = 11;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 12) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 12L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 12L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 12;
    cinfo->min_DCT_v_scaled_size = 12;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 13) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 13L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 13L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 13;
    cinfo->min_DCT_v_scaled_size = 13;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 14) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 14L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 14L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 14;
    cinfo->min_DCT_v_scaled_size = 14;
  } else if (cinfo->scale_num * cinfo->block_size <= cinfo->scale_denom * 15) {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 15L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 15L, (long) cinfo->block_size);
    cinfo->min_DCT_h_scaled_size = 15;
    cinfo->min_DCT_v_scaled_size = 15;
  } else {

    cinfo->output_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * 16L, (long) cinfo->block_size);
    cinfo->output_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * 16L, (long) cinfo->block_size);
...
static void
initial_setup (int *data_flow, j_decompress_ptr cinfo)

{
  ...
  if ((long) cinfo->image_height > (long) 65500L ||
    ...
    ((cinfo)->err->msg_code = (JERR_IMAGE_TOO_BIG), (cinfo)->err->msg_parm.i[0] = ((unsigned int) 65500L), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
 ...
 compptr->v_samp_factor<=0 || compptr->v_samp_factor>4)
      ((cinfo)->err->msg_code = (JERR_BAD_SAMPLING), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  ...
  cinfo->total_iMCU_rows = (JDIMENSION)
    jdiv_round_up(data_flow, (long) cinfo->image_height,
           (long) (cinfo->max_v_samp_factor * cinfo->block_size));
...
static void
per_scan_setup (int *data_flow, j_decompress_ptr cinfo)
...

{
  ...
  if (cinfo->comps_in_scan == 1) {
    ...
    if (cinfo->comps_in_scan <= 0 || cinfo->comps_in_scan > 4)
      ((cinfo)->err->msg_code = (JERR_COMPONENT_COUNT), (cinfo)->err->msg_parm.i[0] = (cinfo->comps_in_scan), (cinfo)->err->msg_parm.i[1] = (4), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)))
                          ;


    cinfo->MCUs_per_row = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width,
 ...
 ((cinfo)->err->msg_code = (JERR_BAD_MCU_SIZE), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
      while (mcublks-- > 0) {
...
static void
latch_quant_tables (int *data_flow, j_decompress_ptr cinfo)
{
  int ci, qtblno={0};
  jpeg_component_info *compptr={0};
  JQUANT_TBL * qtbl={0};
...
{
  per_scan_setup(data_flow, cinfo);
  latch_quant_tables(data_flow, cinfo);
  (*cinfo->entropy->start_pass) (data_flow, cinfo);
  (*cinfo->coef->start_input_pass) (data_flow, cinfo);
  cinfo->inputctl->consume_input = cinfo->coef->consume_data;
}
static void
finish_input_pass (int *data_flow, j_decompress_ptr cinfo)
{
  (*cinfo->entropy->finish_pass) (data_flow, cinfo);
  cinfo->inputctl->consume_input = consume_markers;
}
static int
consume_markers (int *data_flow, j_decompress_ptr cinfo)
{
  ...
  for (;;) {
    val = (*cinfo->marker->read_markers) (data_flow, cinfo);

    switch (val) {
    case 1:
      if (inputctl->inheaders) {
 if (inputctl->inheaders == 1)
   initial_setup(data_flow, cinfo);
 ...
 if (! inputctl->pub.has_multiple_scans)
   ((cinfo)->err->msg_code = (JERR_EOI_EXPECTED), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
 if (cinfo->comps_in_scan == 0)
   break;
 start_input_pass(data_flow, cinfo);
      }
      return val;
    case 2:
 ...
 if (cinfo->marker->saw_SOF)
   ((cinfo)->err->msg_code = (JERR_SOF_NO_SOS), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
      } else {
...
static void
reset_input_controller (int *data_flow, j_decompress_ptr cinfo)
{
  my_inputctl_ptr inputctl = (my_inputctl_ptr) cinfo->inputctl;
  ...
  (*cinfo->err->reset_error_mgr) (data_flow, (j_common_ptr) cinfo);
  (*cinfo->marker->reset_marker_reader) (data_flow, cinfo);
...
void
jinit_input_controller (int *data_flow, j_decompress_ptr cinfo)
{
  my_inputctl_ptr inputctl={0};
