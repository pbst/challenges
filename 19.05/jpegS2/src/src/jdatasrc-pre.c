...
static void
init_source (int *data_flow, j_decompress_ptr cinfo)
{
...
static void
init_mem_source (int *data_flow, j_decompress_ptr cinfo)
{

}
static boolean
fill_input_buffer (int *data_flow, j_decompress_ptr cinfo)
{
  my_src_ptr src = (my_src_ptr) cinfo->src;
  size_t nbytes={0};
  ...
  if (nbytes <= 0) {
    if (src->start_of_file)
      ((cinfo)->err->msg_code = (JERR_INPUT_EMPTY), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
    ((cinfo)->err->msg_code = (JWRN_JPEG_EOF), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
...
static boolean
fill_mem_input_buffer (int *data_flow, j_decompress_ptr cinfo)
{
  static const JOCTET mybuffer[4] = {
    (JOCTET) 0xFF, (JOCTET) 0xD9, 0, 0
  };
...
static void
skip_input_data (int *data_flow, j_decompress_ptr cinfo, long num_bytes)
{
  ...
  if (num_bytes > 0) {
    while (num_bytes > (long) src->bytes_in_buffer) {
      ...
      (void) (*src->fill_input_buffer) (data_flow, cinfo);
...
static void
term_source (int *data_flow, j_decompress_ptr cinfo)
{

}
void
jpeg_stdio_src (int *data_flow, j_decompress_ptr cinfo, FILE * infile)
{
  my_src_ptr src={0};
  if (cinfo->src == 
                   ((void *)0)
...
void
jpeg_mem_src (int *data_flow, j_decompress_ptr cinfo,
       const unsigned char * inbuffer, unsigned long insize)
{
  struct jpeg_source_mgr * src={0};
