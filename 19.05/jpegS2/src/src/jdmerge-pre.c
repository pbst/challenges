...
typedef struct {
  ...
  void (*upmethod) (int *data_flow, j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr, JSAMPARRAY output_buf)
...
static void
build_ycc_rgb_table (int *data_flow, j_decompress_ptr cinfo)

{
  ...
  for (i = 0, x = -128; i <= 255; i++, x++) {
    ...
    upsample->Cr_g_tab[i] = (- ((INT32) ((0.714136286) * (1L<<16) + 0.5))) * x;


    upsample->Cb_g_tab[i] = (- ((INT32) ((0.344136286) * (1L<<16) + 0.5))) * x + ((INT32) 1 << (16 -1));
  }
}


static void
build_bg_ycc_rgb_table (int *data_flow, j_decompress_ptr cinfo)

{
  ...
  for (i = 0, x = -128; i <= 255; i++, x++) {
    ...
    upsample->Cr_g_tab[i] = (- ((INT32) ((1.428272572) * (1L<<16) + 0.5))) * x;


    upsample->Cb_g_tab[i] = (- ((INT32) ((0.688272572) * (1L<<16) + 0.5))) * x + ((INT32) 1 << (16 -1));
  }
}
...
static void
start_pass_merged_upsample (int *data_flow, j_decompress_ptr cinfo)
{
  my_upsample_ptr upsample = (my_upsample_ptr) cinfo->upsample;
...
static void
merged_2v_upsample (int *data_flow, j_decompress_ptr cinfo,
...
{
  my_upsample_ptr upsample = (my_upsample_ptr) cinfo->upsample;
  JSAMPROW work_ptrs[2]={0};
  JDIMENSION num_rows={0};

  if (upsample->spare_full) {

    jcopy_sample_rows(data_flow, & upsample->spare_row, 0, output_buf + *out_row_ctr, 0,
    ...
    (*upsample->upmethod) (data_flow, cinfo, input_buf, *in_row_group_ctr, work_ptrs);
  }
...
static void
merged_1v_upsample (int *data_flow, j_decompress_ptr cinfo,
      JSAMPIMAGE input_buf, JDIMENSION *in_row_group_ctr,
      JDIMENSION in_row_groups_avail,
      JSAMPARRAY output_buf, JDIMENSION *out_row_ctr,
      JDIMENSION out_rows_avail)

{
  ...
  (*upsample->upmethod) (data_flow, cinfo, input_buf, *in_row_group_ctr,
...
static void
h2v1_merged_upsample (int *data_flow, j_decompress_ptr cinfo,
        JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr,
        JSAMPARRAY output_buf)
{
  my_upsample_ptr upsample = (my_upsample_ptr) cinfo->upsample;
  register int y, cred, cgreen, cblue={0};
  int cb, cr={0};
  register JSAMPROW outptr={0};
  JSAMPROW inptr0, inptr1, inptr2={0};
  JDIMENSION col={0};
...
static void
h2v2_merged_upsample (int *data_flow, j_decompress_ptr cinfo,
        JSAMPIMAGE input_buf, JDIMENSION in_row_group_ctr,
        JSAMPARRAY output_buf)
{
  my_upsample_ptr upsample = (my_upsample_ptr) cinfo->upsample;
  register int y, cred, cgreen, cblue={0};
  int cb, cr={0};
  register JSAMPROW outptr0, outptr1={0};
  JSAMPROW inptr00, inptr01, inptr1, inptr2={0};
  JDIMENSION col={0};
...
void
jinit_merged_upsampler (int *data_flow, j_decompress_ptr cinfo)
{
  my_upsample_ptr upsample={0};
  ...
  if (cinfo->jpeg_color_space == JCS_BG_YCC)
    build_bg_ycc_rgb_table(data_flow, cinfo);
  else
    build_ycc_rgb_table(data_flow, cinfo);
}
