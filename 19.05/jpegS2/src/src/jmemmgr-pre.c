...
static noreturn_t
out_of_memory (int *data_flow, j_common_ptr cinfo, int which)
...

{
  ...
  ((cinfo)->err->msg_code = (JERR_OUT_OF_MEMORY), (cinfo)->err->msg_parm.i[0] = (which), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
}
...
static void *
alloc_small (int *data_flow, j_common_ptr cinfo, int pool_id, size_t sizeofobject)

{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  small_pool_ptr hdr_ptr, prev_hdr_ptr={0};
  char * data_ptr={0};
  size_t odd_bytes, min_request, slop={0};


  if (sizeofobject > (size_t) (1000000000L -((size_t) sizeof(small_pool_hdr))))
    out_of_memory(data_flow, cinfo, 1);
  ...
  if (pool_id < 0 || pool_id >= 2)
    ((cinfo)->err->msg_code = (JERR_BAD_POOL_ID), (cinfo)->err->msg_parm.i[0] = (pool_id), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  prev_hdr_ptr = 
                ((void *)0)
                    ;
  ...
  if (hdr_ptr == 
    ...
    for (;;) {
      hdr_ptr = (small_pool_ptr) jpeg_get_small(data_flow, cinfo, min_request + slop);
      if (hdr_ptr != 
                    ((void *)0)
                        )
 break;
      slop /= 2;
      if (slop < 50)
 out_of_memory(data_flow, cinfo, 2);
    }
...
static void *
alloc_large (int *data_flow, j_common_ptr cinfo, int pool_id, size_t sizeofobject)

{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  large_pool_ptr hdr_ptr={0};
  size_t odd_bytes={0};


  if (sizeofobject > (size_t) (1000000000L -((size_t) sizeof(large_pool_hdr))))
    out_of_memory(data_flow, cinfo, 3);
  ...
  hdr_ptr = (large_pool_ptr) jpeg_get_large(data_flow, cinfo, sizeofobject +
         ((size_t) sizeof(large_pool_hdr)));
  if (hdr_ptr == 
                ((void *)0)
                    )
    out_of_memory(data_flow, cinfo, 4);
...
static JSAMPARRAY
alloc_sarray (int *data_flow, j_common_ptr cinfo, int pool_id,
       JDIMENSION samplesperrow, JDIMENSION numrows)

{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  JSAMPARRAY result={0};
  JSAMPROW workspace={0};
  JDIMENSION rowsperchunk, currow, i={0};
  long ltemp={0};
  ...
  if (ltemp <= 0)
    ((cinfo)->err->msg_code = (JERR_WIDTH_OVERFLOW), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  ...
  result = (JSAMPARRAY) alloc_small(data_flow, cinfo, pool_id,
  ...
  while (currow < numrows) {
    ...
    workspace = (JSAMPROW) alloc_large(data_flow, cinfo, pool_id,
...
static JBLOCKARRAY
alloc_barray (int *data_flow, j_common_ptr cinfo, int pool_id,
       JDIMENSION blocksperrow, JDIMENSION numrows)

{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  JBLOCKARRAY result={0};
  JBLOCKROW workspace={0};
  JDIMENSION rowsperchunk, currow, i={0};
  long ltemp={0};
  ...
  result = (JBLOCKARRAY) alloc_small(data_flow, cinfo, pool_id,
  ...
  while (currow < numrows) {
    rowsperchunk = ((rowsperchunk) < (numrows - currow) ? (rowsperchunk) : (numrows - currow));
    workspace = (JBLOCKROW) alloc_large(data_flow, cinfo, pool_id,
...
request_virt_sarray (int *data_flow, j_common_ptr cinfo, int pool_id, boolean pre_zero,
       JDIMENSION samplesperrow, JDIMENSION numrows,
       JDIMENSION maxaccess)

{
  ...
  result = (jvirt_sarray_ptr) alloc_small(data_flow, cinfo, pool_id,
...
request_virt_barray (int *data_flow, j_common_ptr cinfo, int pool_id, boolean pre_zero,
       JDIMENSION blocksperrow, JDIMENSION numrows,
       JDIMENSION maxaccess)

{
  ...
  result = (jvirt_barray_ptr) alloc_small(data_flow, cinfo, pool_id,
...
static void
realize_virt_arrays (int *data_flow, j_common_ptr cinfo)

{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  long space_per_minheight, maximum_space, avail_mem={0};
  long minheights, max_minheights={0};
  jvirt_sarray_ptr sptr={0};
  jvirt_barray_ptr bptr={0};
  ...
  maximum_space = 0;
  for (sptr = mem->virt_sarray_list; sptr != 
                                            ((void *)0)
                                                ; sptr = sptr->next) {
    if (sptr->mem_buffer == 
                           ((void *)0)
  ...
  for (bptr = mem->virt_barray_list; bptr != 
                                            ((void *)0)
                                                ; bptr = bptr->next) {
    if (bptr->mem_buffer == 
                           ((void *)0)
  ...
  avail_mem = jpeg_mem_available(data_flow, cinfo, space_per_minheight, maximum_space,
 ...
 jpeg_open_backing_store(data_flow, cinfo, & sptr->b_s_info,
    (long) sptr->rows_in_array *
    (long) sptr->samplesperrow *
    (long) ((size_t) sizeof(JSAMPLE)));
 sptr->b_s_open = TRUE;
      }
      sptr->mem_buffer = alloc_sarray(data_flow, cinfo, 1,
 ...
 jpeg_open_backing_store(data_flow, cinfo, & bptr->b_s_info,
    (long) bptr->rows_in_array *
    (long) bptr->blocksperrow *
    (long) ((size_t) sizeof(JBLOCK)));
 bptr->b_s_open = TRUE;
      }
      bptr->mem_buffer = alloc_barray(data_flow, cinfo, 1,
...
static void
do_sarray_io (int *data_flow, j_common_ptr cinfo, jvirt_sarray_ptr ptr, boolean writing)

{
  long bytesperrow, file_offset, byte_count, rows, thisrow, i={0};
  ...
  for (i = 0; i < (long) ptr->rows_in_mem; i += ptr->rowsperchunk) {
    ...
    if (writing)
      (*ptr->b_s_info.write_backing_store) (data_flow, cinfo, & ptr->b_s_info,
         (void *) ptr->mem_buffer[i],
         file_offset, byte_count);
    else
      (*ptr->b_s_info.read_backing_store) (data_flow, cinfo, & ptr->b_s_info,
        (void *) ptr->mem_buffer[i],
        file_offset, byte_count);
    file_offset += byte_count;
  }
...
static void
do_barray_io (int *data_flow, j_common_ptr cinfo, jvirt_barray_ptr ptr, boolean writing)
...
static JSAMPARRAY
access_virt_sarray (int *data_flow, j_common_ptr cinfo, jvirt_sarray_ptr ptr,
...

{
  ...
  if (end_row > ptr->rows_in_array || num_rows > ptr->maxaccess ||
    ...
    ((cinfo)->err->msg_code = (JERR_BAD_VIRTUAL_ACCESS), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));


  if (start_row < ptr->cur_start_row ||
      end_row > ptr->cur_start_row+ptr->rows_in_mem) {
    if (! ptr->b_s_open)
      ((cinfo)->err->msg_code = (JERR_VIRTUAL_BUG), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));

    if (ptr->dirty) {
      do_sarray_io(data_flow, cinfo, ptr, TRUE);
      ptr->dirty = FALSE;
    }
 ...
 ltemp = 0;
    ...
    do_sarray_io(data_flow, cinfo, ptr, FALSE);
  }
...
static JBLOCKARRAY
access_virt_barray (int *data_flow, j_common_ptr cinfo, jvirt_barray_ptr ptr,
      JDIMENSION start_row, JDIMENSION num_rows,
      boolean writable)
...

{
  ...
  if (start_row < ptr->cur_start_row ||
    ...
    if (ptr->dirty) {
      do_barray_io(data_flow, cinfo, ptr, TRUE);
      ptr->dirty = FALSE;
    }
 ...
 ltemp = 0;
    ...
    do_barray_io(data_flow, cinfo, ptr, FALSE);
  }
...
static void
free_pool (int *data_flow, j_common_ptr cinfo, int pool_id)
{
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;
  small_pool_ptr shdr_ptr={0};
  large_pool_ptr lhdr_ptr={0};
  ...
  if (pool_id == 1) {
    ...
    for (sptr = mem->virt_sarray_list; sptr != 
      ...
      if (sptr->b_s_open) {
 sptr->b_s_open = FALSE;
 (*sptr->b_s_info.close_backing_store) (data_flow, cinfo, & sptr->b_s_info);
      }
    }
    mem->virt_sarray_list = 
                           ((void *)0)
                               ;
    for (bptr = mem->virt_barray_list; bptr != 
                                              ((void *)0)
      ...
      if (bptr->b_s_open) {
 bptr->b_s_open = FALSE;
 (*bptr->b_s_info.close_backing_store) (data_flow, cinfo, & bptr->b_s_info);
      }
    }
  ...
  while (lhdr_ptr != 
    ...
    ((size_t) sizeof(large_pool_hdr));
    jpeg_free_large(data_flow, cinfo, (void *) lhdr_ptr, space_freed);
  ...
  while (shdr_ptr != 
    ...
    jpeg_free_small(data_flow, cinfo, (void *) shdr_ptr, space_freed);
    mem->total_space_allocated -= space_freed;
...
static void
self_destruct (int *data_flow, j_common_ptr cinfo)
{
  int pool={0};
  ...
  for (pool = 2 -1; pool >= 0; pool--) {
    free_pool(data_flow, cinfo, pool);
  }


  jpeg_free_small(data_flow, cinfo, (void *) cinfo->mem, ((size_t) sizeof(my_memory_mgr)));
  cinfo->mem = 
              ((void *)0)
                  ;

  jpeg_mem_term(data_flow, cinfo);
}
...
void
jinit_memory_mgr (int *data_flow, j_common_ptr cinfo)
{
  my_mem_ptr mem={0};
  long max_to_use={0};
  int pool={0};
  ...
  if ((((size_t) sizeof(double)) & (((size_t) sizeof(double))-1)) != 0)
    ((cinfo)->err->msg_code = (JERR_BAD_ALIGN_TYPE), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  ...
  if ((long) test_mac != 1000000000L ||
    ...
    ((cinfo)->err->msg_code = (JERR_BAD_ALLOC_CHUNK), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));

  max_to_use = jpeg_mem_init(data_flow, cinfo);


  mem = (my_mem_ptr) jpeg_get_small(data_flow, cinfo, ((size_t) sizeof(my_memory_mgr)));

  if (mem == 
            ((void *)0)
                ) {
    jpeg_mem_term(data_flow, cinfo);
    ((cinfo)->err->msg_code = (JERR_OUT_OF_MEMORY), (cinfo)->err->msg_parm.i[0] = (0), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  }
