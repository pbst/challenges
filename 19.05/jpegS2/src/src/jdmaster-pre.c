...
static boolean
use_merged_upsample (int *data_flow, j_decompress_ptr cinfo)
{
...
void
jpeg_calc_output_dimensions (int *data_flow, j_decompress_ptr cinfo)
...

{
  ...
  jpeg_core_output_dimensions(data_flow, cinfo);
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
       ci++, compptr++) {
    int ssize = 1;
    while (cinfo->min_DCT_h_scaled_size * ssize <=
    (cinfo->do_fancy_upsampling ? 8 : 8 / 2) &&
 ...
 compptr->DCT_v_scaled_size = compptr->DCT_h_scaled_size * 2;
  ...
  if (use_merged_upsample(data_flow, cinfo))
    cinfo->rec_outbuf_height = cinfo->max_v_samp_factor;
  else
    cinfo->rec_outbuf_height = 1;
}
static void
prepare_range_limit_table (int *data_flow, j_decompress_ptr cinfo)

{
  JSAMPLE * table={0};
  int i={0};

  table = (JSAMPLE *) (*cinfo->mem->alloc_small) (data_flow, (j_common_ptr) cinfo,
    1, ((128 << 2) * 2 + 255 + 1) * ((size_t) sizeof(JSAMPLE)));

  memset((void *)(table), 0, (size_t)((128 << 2) * ((size_t) sizeof(JSAMPLE))));
  table += (128 << 2);
...
master_selection (int *data_flow, j_decompress_ptr cinfo)
{
  ...
  jpeg_calc_output_dimensions(data_flow, cinfo);
  prepare_range_limit_table(data_flow, cinfo);
  ...
  master->using_merged_upsample = use_merged_upsample(data_flow, cinfo);
  ...
  if (cinfo->quantize_colors) {
    ...
    if (cinfo->enable_1pass_quant) {
      ...
      jinit_1pass_quantizer(data_flow, cinfo);
    ...
    if (cinfo->enable_2pass_quant || cinfo->enable_external_quant) {
      ...
      jinit_2pass_quantizer(data_flow, cinfo);
  ...
  if (! cinfo->raw_data_out) {
    if (master->using_merged_upsample) {
      ...
      jinit_merged_upsampler(data_flow, cinfo);



    } else {
      jinit_color_deconverter(data_flow, cinfo);
      jinit_upsampler(data_flow, cinfo);
    }
    jinit_d_post_controller(data_flow, cinfo, cinfo->enable_2pass_quant);
  }

  jinit_inverse_dct(data_flow, cinfo);

  if (cinfo->arith_code)
    jinit_arith_decoder(data_flow, cinfo);
  else {
    jinit_huff_decoder(data_flow, cinfo);
  }


  use_c_buffer = cinfo->inputctl->has_multiple_scans || cinfo->buffered_image;
  jinit_d_coef_controller(data_flow, cinfo, use_c_buffer);
  ...
  (*cinfo->inputctl->start_input_pass) (data_flow, cinfo);
...
static void
prepare_for_output_pass (int *data_flow, j_decompress_ptr cinfo)
{
  my_master_ptr master = (my_master_ptr) cinfo->master;
  ...
  if (master->pub.is_dummy_pass) {
    ...
    (*cinfo->cquantize->start_pass) (data_flow, cinfo, FALSE);
    (*cinfo->post->start_pass) (data_flow, cinfo, JBUF_CRANK_DEST);
    (*cinfo->main->start_pass) (data_flow, cinfo, JBUF_CRANK_DEST);
...
static void
finish_output_pass (int *data_flow, j_decompress_ptr cinfo)
{
  my_master_ptr master = (my_master_ptr) cinfo->master;

  if (cinfo->quantize_colors)
    (*cinfo->cquantize->finish_pass) (data_flow, cinfo);
  master->pass_number++;
}
void
jpeg_new_colormap (int *data_flow, j_decompress_ptr cinfo)
{
  my_master_ptr master = (my_master_ptr) cinfo->master;
  ...
  if (cinfo->quantize_colors && cinfo->enable_external_quant &&
    ...
    (*cinfo->cquantize->new_color_map) (data_flow, cinfo);
    master->pub.is_dummy_pass = FALSE;
  } else
    ((cinfo)->err->msg_code = (JERR_MODE_CHANGE), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
}
void
jinit_master_decompress (int *data_flow, j_decompress_ptr cinfo)
{
  ...
  master_selection(data_flow, cinfo);
}
