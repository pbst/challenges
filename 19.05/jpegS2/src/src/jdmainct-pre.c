...
static void process_data_simple_main
 (int *data_flow, j_decompress_ptr cinfo, JSAMPARRAY output_buf, JDIMENSION *out_row_ctr, JDIMENSION out_rows_avail)
                                                          ;
...
static void
alloc_funny_pointers (int *data_flow, j_decompress_ptr cinfo)
...
static void
make_funny_pointers (int *data_flow, j_decompress_ptr cinfo)
...
static void
set_wraparound_pointers (int *data_flow, j_decompress_ptr cinfo)
...
static void
set_bottom_pointers (int *data_flow, j_decompress_ptr cinfo)
...
{
  my_main_ptr mainp = (my_main_ptr) cinfo->main;
  int ci, i, rgroup, iMCUheight, rows_left={0};
  jpeg_component_info *compptr={0};
  JSAMPARRAY xbuf={0};
...
static void
start_pass_main (int *data_flow, j_decompress_ptr cinfo, J_BUF_MODE pass_mode)
{
  my_main_ptr mainp = (my_main_ptr) cinfo->main;
  ...
  case JBUF_PASS_THRU:
    if (cinfo->upsample->need_context_rows) {
      ...
      make_funny_pointers(data_flow, cinfo);
      mainp->whichptr = 0;
...
static void
process_data_simple_main (int *data_flow, j_decompress_ptr cinfo,
     ...
     JDIMENSION out_rows_avail)
{
  ...
  if (mainp->rowgroup_ctr >= mainp->rowgroups_avail) {
    if (! (*cinfo->coef->decompress_data) (data_flow, cinfo, mainp->buffer))
      return;
...
static void
process_data_context_main (int *data_flow, j_decompress_ptr cinfo,
      JSAMPARRAY output_buf, JDIMENSION *out_row_ctr,
      JDIMENSION out_rows_avail)
{
  my_main_ptr mainp = (my_main_ptr) cinfo->main;
  ...
  if (! mainp->buffer_full) {
    if (! (*cinfo->coef->decompress_data) (data_flow, cinfo,
  ...
  case 2:
    ...
    (*cinfo->post->post_process_data) (data_flow, cinfo, mainp->xbuffer[mainp->whichptr],
   &mainp->rowgroup_ctr, mainp->rowgroups_avail,
  ...
  case 0:
    ...
    if (mainp->iMCU_row_ctr == cinfo->total_iMCU_rows)
      set_bottom_pointers(data_flow, cinfo);
  ...
  case 1:
   ...
   output_buf, out_row_ctr, out_rows_avail);
    ...
    if (mainp->iMCU_row_ctr == 1)
      set_wraparound_pointers(data_flow, cinfo);
...
static void
process_data_crank_post (int *data_flow, j_decompress_ptr cinfo,
    JSAMPARRAY output_buf, JDIMENSION *out_row_ctr,
    JDIMENSION out_rows_avail)
{
  (*cinfo->post->post_process_data) (data_flow, cinfo, (JSAMPIMAGE) 
                                                        ((void *)0)
                                                            ,
         (JDIMENSION *) 
                       ((void *)0)
...
void
jinit_d_main_controller (int *data_flow, j_decompress_ptr cinfo, boolean need_full_buffer)
{
  my_main_ptr mainp={0};
  int ci, rgroup, ngroups={0};
  jpeg_component_info *compptr={0};
  ...
  if (cinfo->upsample->need_context_rows) {
    ...
    alloc_funny_pointers(data_flow, cinfo);
