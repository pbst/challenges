...
"Write to XMS failed" ,
"Copyright (C) 2018, Thomas G. Lane, Guido Vollbeding" ,
"9c  14-Jan-2018" ,
...
static noreturn_t
error_exit (int *data_flow, j_common_ptr cinfo)
{

  (*cinfo->err->output_message) (data_flow, cinfo);


  jpeg_destroy(data_flow, cinfo);
...
static void
output_message (int *data_flow, j_common_ptr cinfo)
{
  char buffer[200]={0};


  (*cinfo->err->format_message) (data_flow, cinfo, buffer);
...
static void
emit_message (int *data_flow, j_common_ptr cinfo, int msg_level)
{
  ...
  if (msg_level < 0) {
    ...
    if (err->num_warnings == 0 || err->trace_level >= 3)
      (*err->output_message) (data_flow, cinfo);
...
static void
format_message (int *data_flow, j_common_ptr cinfo, char * buffer)
{
  struct jpeg_error_mgr * err = cinfo->err;
  ...
  } else if (err->addon_message_table != 
                                        ((void *)0) 
                                             &&
...
static void
reset_error_mgr (int *data_flow, j_common_ptr cinfo)
{
...
struct jpeg_error_mgr *
jpeg_std_error (int *data_flow, struct jpeg_error_mgr * err)
{
