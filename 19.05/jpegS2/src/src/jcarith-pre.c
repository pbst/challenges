...
static void
emit_byte (int *data_flow, int val, j_compress_ptr cinfo)
...
static void
finish_pass (int *data_flow, j_compress_ptr cinfo)
{
  ...
  if (e->c & 0xF8000000L) {
    ...
    if (e->buffer >= 0) {
      if (e->zc)
 do emit_byte(data_flow, 0x00, cinfo);
 while (--e->zc);
      emit_byte(data_flow, e->buffer + 1, cinfo);
      if (e->buffer + 1 == 0xFF)
 emit_byte(data_flow, 0x00, cinfo);
    }
    e->zc += e->sc;
    e->sc = 0;
  } else {
 ...
 do emit_byte(data_flow, 0x00, cinfo);
 while (--e->zc);
      emit_byte(data_flow, e->buffer, cinfo);
    }
    if (e->sc) {
      if (e->zc)
 do emit_byte(data_flow, 0x00, cinfo);
 while (--e->zc);
      do {
 emit_byte(data_flow, 0xFF, cinfo);
 emit_byte(data_flow, 0x00, cinfo);
      } while (--e->sc);
    }
  }
  ...
  if (e->c & 0x7FFF800L) {
    if (e->zc)
      do emit_byte(data_flow, 0x00, cinfo);
      while (--e->zc);
    emit_byte(data_flow, (e->c >> 19) & 0xFF, cinfo);
    if (((e->c >> 19) & 0xFF) == 0xFF)
      emit_byte(data_flow, 0x00, cinfo);
    if (e->c & 0x7F800L) {
      emit_byte(data_flow, (e->c >> 11) & 0xFF, cinfo);
      if (((e->c >> 11) & 0xFF) == 0xFF)
 emit_byte(data_flow, 0x00, cinfo);
    }
  }
...
static void
arith_encode (int *data_flow, j_compress_ptr cinfo, unsigned char *st, int val)
{
...
static void
emit_restart (int *data_flow, j_compress_ptr cinfo, int restart_num)
{
  arith_entropy_ptr entropy = (arith_entropy_ptr) cinfo->entropy;
  int ci={0};
  jpeg_component_info * compptr={0};

  finish_pass(data_flow, cinfo);

  emit_byte(data_flow, 0xFF, cinfo);
  emit_byte(data_flow, 0xD0 + restart_num, cinfo);
...
encode_mcu_DC_first (int *data_flow, j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  if (cinfo->restart_interval) {
    if (entropy->restarts_to_go == 0) {
      emit_restart(data_flow, cinfo, entropy->next_restart_num);
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    if ((v = m - entropy->last_dc_val[ci]) == 0) {
      arith_encode(data_flow, cinfo, st, 0);
      entropy->dc_context[ci] = 0;
    } else {
      entropy->last_dc_val[ci] = m;
      arith_encode(data_flow, cinfo, st, 1);


      if (v > 0) {
 arith_encode(data_flow, cinfo, st + 1, 0);
 st += 2;
 entropy->dc_context[ci] = 4;
      } else {
 v = -v;
 arith_encode(data_flow, cinfo, st + 1, 1);
 st += 3;
 ...
 entropy->dc_context[ci] += 8;
      ...
      st += 14;
      while (m >>= 1)
 arith_encode(data_flow, cinfo, st, (m & v) ? 1 : 0);
    }
  }
...
encode_mcu_AC_first (int *data_flow, j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
 ...
 if (v >>= cinfo->Al) {
   arith_encode(data_flow, cinfo, st + 1, 1);
   arith_encode(data_flow, cinfo, entropy->fixed_bin, 0);
   break;
 }
      } else {
 v = -v;
 if (v >>= cinfo->Al) {
   arith_encode(data_flow, cinfo, st + 1, 1);
   arith_encode(data_flow, cinfo, entropy->fixed_bin, 1);
   break;
 }
      }
      arith_encode(data_flow, cinfo, st + 1, 0);
...
encode_mcu_DC_refine (int *data_flow, j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    arith_encode(data_flow, cinfo, st, (MCU_data[blkn][0][0] >> Al) & 1);
  }
...
encode_mcu_AC_refine (int *data_flow, j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
 ...
 if (v >>= cinfo->Al) {
   if (v >> 1)
     arith_encode(data_flow, cinfo, st + 2, (v & 1));
   else {
     arith_encode(data_flow, cinfo, st + 1, 1);
     arith_encode(data_flow, cinfo, entropy->fixed_bin, 0);
   }
...
static boolean
encode_mcu (int *data_flow, j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  arith_entropy_ptr entropy = (arith_entropy_ptr) cinfo->entropy;
  const int * natural_order={0};
  JBLOCKROW block={0};
...
static void
start_pass (int *data_flow, j_compress_ptr cinfo, boolean gather_statistics)
{
  arith_entropy_ptr entropy = (arith_entropy_ptr) cinfo->entropy;
  int ci, tbl={0};
  jpeg_component_info * compptr={0};
...
void
jinit_arith_encoder (int *data_flow, j_compress_ptr cinfo)
{
