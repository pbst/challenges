...
static int decompress_onepass
 (int *data_flow, j_decompress_ptr cinfo, JSAMPIMAGE output_buf);

static int decompress_data
 (int *data_flow, j_decompress_ptr cinfo, JSAMPIMAGE output_buf);


static boolean smoothing_ok (int *data_flow, j_decompress_ptr cinfo);
...
static void
start_iMCU_row (int *data_flow, j_decompress_ptr cinfo)
...
static void
start_output_pass (int *data_flow, j_decompress_ptr cinfo)
{
  ...
  if (coef->pub.coef_arrays != 
    ...
    if (cinfo->do_block_smoothing && smoothing_ok(data_flow, cinfo))
...
static int
decompress_onepass (int *data_flow, j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  JDIMENSION MCU_col_num={0};
  JDIMENSION last_MCU_col = cinfo->MCUs_per_row - 1;
  JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
  int blkn, ci, xindex, yindex, yoffset, useful_width={0};
  JSAMPARRAY output_ptr={0};
  JDIMENSION start_col, output_col={0};
  jpeg_component_info *compptr={0};
  inverse_DCT_method_ptr inverse_DCT={0};
 ...
 memset((void *)((void *) coef->MCU_buffer[0]), 0, (size_t)((size_t) (cinfo->blocks_in_MCU * ((size_t) sizeof(JBLOCK)))))
                                                    ;
      if (! (*cinfo->entropy->decode_mcu) (data_flow, cinfo, coef->MCU_buffer)) {
 ...
 for (yindex = 0; yindex < compptr->MCU_height; yindex++) {
  ...
  (*cinfo->inputctl->finish_input_pass) (data_flow, cinfo);
  return 4;
}
...
static int
dummy_consume_data (int *data_flow, j_decompress_ptr cinfo)
{
  return 0;
}
static int
consume_data (int *data_flow, j_decompress_ptr cinfo)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  JDIMENSION MCU_col_num={0};
  int blkn, ci, xindex, yindex, yoffset={0};
...
static int
decompress_data (int *data_flow, j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
  JDIMENSION block_num={0};
  int ci, block_row, block_rows={0};
  JBLOCKARRAY buffer={0};
  JBLOCKROW buffer_ptr={0};
  JSAMPARRAY output_ptr={0};
  ...
  (cinfo->input_scan_number == cinfo->output_scan_number &&
   cinfo->input_iMCU_row <= cinfo->output_iMCU_row)) {
    if ((*cinfo->inputctl->consume_input)(data_flow, cinfo) == 0)
      return 0;
  }
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    buffer = (*cinfo->mem->access_virt_barray)
      (data_flow, (j_common_ptr) cinfo, coef->whole_image[ci],
 ...
 (*inverse_DCT) (data_flow, cinfo, compptr, (JCOEFPTR) buffer_ptr,
   output_ptr, output_col);
 buffer_ptr++;
 output_col += compptr->DCT_h_scaled_size;
      }
...
static boolean
smoothing_ok (int *data_flow, j_decompress_ptr cinfo)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  ...
  JQUANT_TBL * qtable={0};
  int * coef_bits={0};
  int * coef_bits_latch={0};

  if (! cinfo->progressive_mode || cinfo->coef_bits == 
                                                      ((void *)0)
                                                          )
    return FALSE;


  if (coef->coef_bits_latch == 
                              ((void *)0)
                                  )
...
static int
decompress_smooth_data (int *data_flow, j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
  JDIMENSION block_num, last_block_column={0};
  int ci, block_row, block_rows, access_rows={0};
  JBLOCKARRAY buffer={0};
  JBLOCKROW buffer_ptr, prev_block_row, next_block_row={0};
  JSAMPARRAY output_ptr={0};
  JDIMENSION output_col={0};
  jpeg_component_info *compptr={0};
  inverse_DCT_method_ptr inverse_DCT={0};
  boolean first_row, last_row={0};
  JBLOCK workspace={0};
  int *coef_bits={0};
  JQUANT_TBL *quanttbl={0};
  INT32 Q00,Q01,Q02,Q10,Q11,Q20, num={0};
  int DC1,DC2,DC3,DC4,DC5,DC6,DC7,DC8,DC9={0};
  int Al, pred={0};
 ...
 jcopy_block_row(data_flow, buffer_ptr, (JBLOCKROW) workspace, (JDIMENSION) 1);
 ...
 (*inverse_DCT) (data_flow, cinfo, compptr, (JCOEFPTR) workspace,
   output_ptr, output_col);
...
void
jinit_d_coef_controller (int *data_flow, j_decompress_ptr cinfo, boolean need_full_buffer)
{
  my_coef_ptr coef={0};
 ...
 (data_flow, (j_common_ptr) cinfo, 1, TRUE,
  (JDIMENSION) jround_up(data_flow, (long) compptr->width_in_blocks,
    (long) compptr->h_samp_factor),
  (JDIMENSION) jround_up(data_flow, (long) compptr->height_in_blocks,
