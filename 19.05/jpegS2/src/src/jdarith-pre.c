...
static int
get_byte (int *data_flow, j_decompress_ptr cinfo)

{
  struct jpeg_source_mgr * src = cinfo->src;

  if (src->bytes_in_buffer == 0)
    if (! (*src->fill_input_buffer) (data_flow, cinfo))
      ((cinfo)->err->msg_code = (JERR_CANT_SUSPEND), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  src->bytes_in_buffer--;
  return (*src->next_input_byte++);
}
static int
arith_decode (int *data_flow, j_decompress_ptr cinfo, unsigned char *st)
{
  register arith_entropy_ptr e = (arith_entropy_ptr) cinfo->entropy;
  register unsigned char nl, nm={0};
  register INT32 qe, temp={0};
  register int sv, data={0};
 ...
 data = get_byte(data_flow, cinfo);
 if (data == 0xFF) {
   do data = get_byte(data_flow, cinfo);
   while (data == 0xFF);
   if (data == 0)
     data = 0xFF;
   else {
...
decode_mcu_DC_first (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  JBLOCKROW block={0};
  unsigned char *st={0};
  int blkn, ci, tbl, sign={0};
  int v, m={0};


  if (cinfo->restart_interval) {
    if (entropy->restarts_to_go == 0)
      process_restart(data_flow, cinfo);
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    if (arith_decode(data_flow, cinfo, st) == 0)
      entropy->dc_context[ci] = 0;
    else {


      sign = arith_decode(data_flow, cinfo, st + 1);
      st += 2; st += sign;

      if ((m = arith_decode(data_flow, cinfo, st)) != 0) {
 st = entropy->dc_stats[tbl] + 20;
 while (arith_decode(data_flow, cinfo, st)) {
   if ((m <<= 1) == 0x8000) {
     ((cinfo)->err->msg_code = (JWRN_ARITH_BAD_CODE), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
     entropy->ct = -1;
     return TRUE;
   }
   st += 1;
 ...
 entropy->dc_context[ci] = 4 + (sign * 4);
      ...
      st += 14;
      while (m >>= 1)
 if (arith_decode(data_flow, cinfo, st)) v |= m;
      v += 1; if (sign) v = -v;
...
decode_mcu_AC_first (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  do {
    ...
    if (arith_decode(data_flow, cinfo, st)) break;
    for (;;) {
      k++;
      if (arith_decode(data_flow, cinfo, st + 1)) break;
      st += 3;
      if (k >= cinfo->Se) {
 ((cinfo)->err->msg_code = (JWRN_ARITH_BAD_CODE), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
 entropy->ct = -1;
 return TRUE;
    ...
    sign = arith_decode(data_flow, cinfo, entropy->fixed_bin);
    st += 2;

    if ((m = arith_decode(data_flow, cinfo, st)) != 0) {
      if (arith_decode(data_flow, cinfo, st)) {
 m <<= 1;
...
decode_mcu_AC_refine (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  do {
    ...
    for (;;) {
      ...
      if (*thiscoef) {
 if (arith_decode(data_flow, cinfo, st + 2)) {
   if (*thiscoef < 0)
     *thiscoef += m1;
   else
     *thiscoef += p1;
 ...
 break;
      ...
      if (arith_decode(data_flow, cinfo, st + 1)) {
 if (arith_decode(data_flow, cinfo, entropy->fixed_bin))
   *thiscoef = m1;
 else
   *thiscoef = p1;
 break;
...
static void
start_pass (int *data_flow, j_decompress_ptr cinfo)
{
  arith_entropy_ptr entropy = (arith_entropy_ptr) cinfo->entropy;
  int ci, tbl={0};
  jpeg_component_info * compptr={0};
 ...
 (cinfo->Se < 64 && cinfo->Se != cinfo->lim_Se))
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (! cinfo->progressive_mode || (cinfo->Ss == 0 && cinfo->Ah == 0)) {
      tbl = compptr->dc_tbl_no;
      if (tbl < 0 || tbl >= 16)
 ((cinfo)->err->msg_code = (JERR_NO_ARITH_TABLE), (cinfo)->err->msg_parm.i[0] = (tbl), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
      if (entropy->dc_stats[tbl] == 
                                   ((void *)0)
                                       )
 entropy->dc_stats[tbl] = (unsigned char *) (*cinfo->mem->alloc_small)
   (data_flow, (j_common_ptr) cinfo, 1, 64);
 ...
 entropy->ac_stats[tbl] = (unsigned char *) (*cinfo->mem->alloc_small)
   (data_flow, (j_common_ptr) cinfo, 1, 256);
...
static void
finish_pass (int *data_flow, j_decompress_ptr cinfo)
{
...
void
jinit_arith_decoder (int *data_flow, j_decompress_ptr cinfo)
{
  arith_entropy_ptr entropy={0};
  int i={0};
