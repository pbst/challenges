...
static void
init_destination (int *data_flow, j_compress_ptr cinfo)
{
...
static void
init_mem_destination (int *data_flow, j_compress_ptr cinfo)
{

}
static boolean
empty_output_buffer (int *data_flow, j_compress_ptr cinfo)
{
  my_dest_ptr dest = (my_dest_ptr) cinfo->dest;

  if (((size_t) fwrite((const void *) (dest->buffer), (size_t) 1, (size_t) (4096), (dest->outfile))) !=
      (size_t) 4096)
    ((cinfo)->err->msg_code = (JERR_FILE_WRITE), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
...
static boolean
empty_mem_output_buffer (int *data_flow, j_compress_ptr cinfo)
{
  size_t nextsize={0};
  JOCTET * nextbuffer={0};
  ...
  if (nextbuffer == 
    ...
    ((cinfo)->err->msg_code = (JERR_OUT_OF_MEMORY), (cinfo)->err->msg_parm.i[0] = (10), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));

  memcpy((void *)(nextbuffer), (const void *)(dest->buffer), (size_t)(dest->bufsize));
...
static void
term_destination (int *data_flow, j_compress_ptr cinfo)
{
  my_dest_ptr dest = (my_dest_ptr) cinfo->dest;
...
static void
term_mem_destination (int *data_flow, j_compress_ptr cinfo)
{
  my_mem_dest_ptr dest = (my_mem_dest_ptr) cinfo->dest;
...
void
jpeg_stdio_dest (int *data_flow, j_compress_ptr cinfo, FILE * outfile)
{
  my_dest_ptr dest={0};
...
void
jpeg_mem_dest (int *data_flow, j_compress_ptr cinfo,
        unsigned char ** outbuffer, unsigned long * outsize)
{
  my_mem_dest_ptr dest={0};
  ...
  if (cinfo->dest == 
    ...
    cinfo->dest = (struct jpeg_destination_mgr *)
      (*cinfo->mem->alloc_small) (data_flow, (j_common_ptr) cinfo, 0,
      ((size_t) sizeof(my_mem_destination_mgr)));
  }
