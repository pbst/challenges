...
static void transencode_master_selection
 (int *data_flow, j_compress_ptr cinfo, jvirt_barray_ptr * coef_arrays);
static void transencode_coef_controller
 (int *data_flow, j_compress_ptr cinfo, jvirt_barray_ptr * coef_arrays);
void
jpeg_write_coefficients (int *data_flow, j_compress_ptr cinfo, jvirt_barray_ptr * coef_arrays)
{
  ...
  jpeg_suppress_tables(data_flow, cinfo, FALSE);

  (*cinfo->err->reset_error_mgr) (data_flow, (j_common_ptr) cinfo);
  (*cinfo->dest->init_destination) (data_flow, cinfo);

  transencode_master_selection(data_flow, cinfo, coef_arrays);

  cinfo->next_scanline = 0;
  cinfo->global_state = 103;
}
void
jpeg_copy_critical_parameters (int *data_flow, j_decompress_ptr srcinfo,
          j_compress_ptr dstinfo)
{
  JQUANT_TBL ** qtblptr={0};
  jpeg_component_info *incomp, *outcomp={0};
  JQUANT_TBL *c_quant, *slot_quant={0};
  int tblno, ci, coefi={0};


  if (dstinfo->global_state != 100)
    ((dstinfo)->err->msg_code = (JERR_BAD_STATE), (dstinfo)->err->msg_parm.i[0] = (dstinfo->global_state), (*(dstinfo)->err->error_exit) (data_flow, (j_common_ptr) (dstinfo)));
  ...
  jpeg_set_defaults(data_flow, dstinfo);
  ...
  jpeg_set_colorspace(data_flow, dstinfo, srcinfo->jpeg_color_space);
 ...
 *qtblptr = jpeg_alloc_quant_table(data_flow, (j_common_ptr) dstinfo);
  ...
  if (dstinfo->num_components < 1 || dstinfo->num_components > 10)
    ((dstinfo)->err->msg_code = (JERR_COMPONENT_COUNT), (dstinfo)->err->msg_parm.i[0] = (dstinfo->num_components), (dstinfo)->err->msg_parm.i[1] = (10), (*(dstinfo)->err->error_exit) (data_flow, (j_common_ptr) (dstinfo)))
                     ;
 ...
 srcinfo->quant_tbl_ptrs[tblno] == 
      ...
      ((dstinfo)->err->msg_code = (JERR_NO_QUANT_TABLE), (dstinfo)->err->msg_parm.i[0] = (tblno), (*(dstinfo)->err->error_exit) (data_flow, (j_common_ptr) (dstinfo)));
 ...
 if (c_quant->quantval[coefi] != slot_quant->quantval[coefi])
   ((dstinfo)->err->msg_code = (JERR_MISMATCHED_QUANT_TABLE), (dstinfo)->err->msg_parm.i[0] = (tblno), (*(dstinfo)->err->error_exit) (data_flow, (j_common_ptr) (dstinfo)));
      }
    }
...
static void
jpeg_calc_trans_dimensions (int *data_flow, j_compress_ptr cinfo)

{
  if (cinfo->min_DCT_h_scaled_size != cinfo->min_DCT_v_scaled_size)
    ((cinfo)->err->msg_code = (JERR_BAD_DCTSIZE), (cinfo)->err->msg_parm.i[0] = (cinfo->min_DCT_h_scaled_size), (cinfo)->err->msg_parm.i[1] = (cinfo->min_DCT_v_scaled_size), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)))
                                                                 ;
...
static void
transencode_master_selection (int *data_flow, j_compress_ptr cinfo,
         jvirt_barray_ptr * coef_arrays)
{

  jpeg_calc_trans_dimensions(data_flow, cinfo);


  jinit_c_master_control(data_flow, cinfo, TRUE );
  ...
  transencode_coef_controller(data_flow, cinfo, coef_arrays);
...
static void
start_iMCU_row (int *data_flow, j_compress_ptr cinfo)
...
static void
start_pass_coef (int *data_flow, j_compress_ptr cinfo, J_BUF_MODE pass_mode)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;

  if (pass_mode != JBUF_CRANK_DEST)
    ((cinfo)->err->msg_code = (JERR_BAD_BUFFER_MODE), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));

  coef->iMCU_row_num = 0;
  start_iMCU_row(data_flow, cinfo);
}
static boolean
compress_output (int *data_flow, j_compress_ptr cinfo, JSAMPIMAGE input_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  JDIMENSION MCU_col_num={0};
  JDIMENSION last_MCU_col = cinfo->MCUs_per_row - 1;
  JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
  int blkn, ci, xindex, yindex, yoffset, blockcnt={0};
  JDIMENSION start_col={0};
  JBLOCKARRAY buffer[4]={0};
  JBLOCKROW MCU_buffer[10]={0};
  JBLOCKROW buffer_ptr={0};
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    buffer[ci] = (*cinfo->mem->access_virt_barray)
      (data_flow, (j_common_ptr) cinfo, coef->whole_image[compptr->component_index],
 ...
 for (yindex = 0; yindex < compptr->MCU_height; yindex++) {
   ...
   for (; xindex < compptr->MCU_width; xindex++) {
     ...
     blkn++;
      ...
      if (! (*cinfo->entropy->encode_mcu) (data_flow, cinfo, MCU_buffer)) {
...
static void
transencode_coef_controller (int *data_flow, j_compress_ptr cinfo,
        jvirt_barray_ptr * coef_arrays)
{
  my_coef_ptr coef={0};
  JBLOCKROW buffer={0};
  ...
  memset((void *)((void *) buffer), 0, (size_t)(10 * ((size_t) sizeof(JBLOCK))));
  for (i = 0; i < 10; i++) {
