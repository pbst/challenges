...
static void transdecode_master_selection (int *data_flow, j_decompress_ptr cinfo);
jvirt_barray_ptr *
jpeg_read_coefficients (int *data_flow, j_decompress_ptr cinfo)
{
  if (cinfo->global_state == 202) {

    transdecode_master_selection(data_flow, cinfo);
...
static void
transdecode_master_selection (int *data_flow, j_decompress_ptr cinfo)
{
  ...
  jinit_d_coef_controller(data_flow, cinfo, TRUE);
