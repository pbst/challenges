...
static void
emit_byte (int *data_flow, j_compress_ptr cinfo, int val)

{
  ...
  *(dest->next_output_byte)++ = (JOCTET) val;
  if (--dest->free_in_buffer == 0) {
    if (! (*dest->empty_output_buffer) (data_flow, cinfo))
...
static void
emit_marker (int *data_flow, j_compress_ptr cinfo, JPEG_MARKER mark)

{
  emit_byte(data_flow, cinfo, 0xFF);
  emit_byte(data_flow, cinfo, (int) mark);
}


static void
emit_2bytes (int *data_flow, j_compress_ptr cinfo, int value)

{
  emit_byte(data_flow, cinfo, (value >> 8) & 0xFF);
  emit_byte(data_flow, cinfo, value & 0xFF);
}
...
static int
emit_dqt (int *data_flow, j_compress_ptr cinfo, int index)
...

{
  ...
  if (qtbl == 
    ...
    ((cinfo)->err->msg_code = (JERR_NO_QUANT_TABLE), (cinfo)->err->msg_parm.i[0] = (index), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
  ...
  if (! qtbl->sent_table) {
    emit_marker(data_flow, cinfo, M_DQT);

    emit_2bytes(data_flow, cinfo,
      prec ? cinfo->lim_Se * 2 + 2 + 1 + 2 : cinfo->lim_Se + 1 + 1 + 2);

    emit_byte(data_flow, cinfo, index + (prec<<4));

    for (i = 0; i <= cinfo->lim_Se; i++) {

      unsigned int qval = qtbl->quantval[cinfo->natural_order[i]];
      if (prec)
 emit_byte(data_flow, cinfo, (int) (qval >> 8));
      emit_byte(data_flow, cinfo, (int) (qval & 0xFF));
    }
...
static void
emit_dht (int *data_flow, j_compress_ptr cinfo, int index, boolean is_ac)

{
  ...
  if (htbl == 
    ...
    ((cinfo)->err->msg_code = (JERR_NO_HUFF_TABLE), (cinfo)->err->msg_parm.i[0] = (index), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));

  if (! htbl->sent_table) {
    emit_marker(data_flow, cinfo, M_DHT);

    length = 0;
    for (i = 1; i <= 16; i++)
      length += htbl->bits[i];

    emit_2bytes(data_flow, cinfo, length + 2 + 1 + 16);
    emit_byte(data_flow, cinfo, index);

    for (i = 1; i <= 16; i++)
      emit_byte(data_flow, cinfo, htbl->bits[i]);

    for (i = 0; i < length; i++)
      emit_byte(data_flow, cinfo, htbl->huffval[i]);
...
static void
emit_dac (int *data_flow, j_compress_ptr cinfo)
...

{
  ...
  if (length) {
    emit_marker(data_flow, cinfo, M_DAC);

    emit_2bytes(data_flow, cinfo, length*2 + 2);

    for (i = 0; i < 16; i++) {
      if (dc_in_use[i]) {
 emit_byte(data_flow, cinfo, i);
 emit_byte(data_flow, cinfo, cinfo->arith_dc_L[i] + (cinfo->arith_dc_U[i]<<4));
      }
      if (ac_in_use[i]) {
 emit_byte(data_flow, cinfo, i + 0x10);
 emit_byte(data_flow, cinfo, cinfo->arith_ac_K[i]);
      }
    }
  }
...
static void
emit_dri (int *data_flow, j_compress_ptr cinfo)

{
  emit_marker(data_flow, cinfo, M_DRI);

  emit_2bytes(data_flow, cinfo, 4);

  emit_2bytes(data_flow, cinfo, (int) cinfo->restart_interval);
}


static void
emit_lse_ict (int *data_flow, j_compress_ptr cinfo)

{
  ...
  emit_marker(data_flow, cinfo, M_JPG8);

  emit_2bytes(data_flow, cinfo, 24);

  emit_byte(data_flow, cinfo, 0x0D);
  emit_2bytes(data_flow, cinfo, 255);
  emit_byte(data_flow, cinfo, 3);
  emit_byte(data_flow, cinfo, cinfo->comp_info[1].component_id);
  emit_byte(data_flow, cinfo, cinfo->comp_info[0].component_id);
  emit_byte(data_flow, cinfo, cinfo->comp_info[2].component_id);
  emit_byte(data_flow, cinfo, 0x80);
  emit_2bytes(data_flow, cinfo, 0);
  emit_2bytes(data_flow, cinfo, 0);
  emit_byte(data_flow, cinfo, 0);
  emit_2bytes(data_flow, cinfo, 1);
  emit_2bytes(data_flow, cinfo, 0);
  emit_byte(data_flow, cinfo, 0);
  emit_2bytes(data_flow, cinfo, 1);
  emit_2bytes(data_flow, cinfo, 0);
...
static void
emit_sof (int *data_flow, j_compress_ptr cinfo, JPEG_MARKER code)

{
  int ci={0};
  jpeg_component_info *compptr={0};

  emit_marker(data_flow, cinfo, code);

  emit_2bytes(data_flow, cinfo, 3 * cinfo->num_components + 2 + 5 + 1);


  if ((long) cinfo->jpeg_height > 65535L ||
      (long) cinfo->jpeg_width > 65535L)
    ((cinfo)->err->msg_code = (JERR_IMAGE_TOO_BIG), (cinfo)->err->msg_parm.i[0] = ((unsigned int) 65535), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));

  emit_byte(data_flow, cinfo, cinfo->data_precision);
  emit_2bytes(data_flow, cinfo, (int) cinfo->jpeg_height);
  emit_2bytes(data_flow, cinfo, (int) cinfo->jpeg_width);

  emit_byte(data_flow, cinfo, cinfo->num_components);

  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
       ci++, compptr++) {
    emit_byte(data_flow, cinfo, compptr->component_id);
    emit_byte(data_flow, cinfo, (compptr->h_samp_factor << 4) + compptr->v_samp_factor);
    emit_byte(data_flow, cinfo, compptr->quant_tbl_no);
  }
}


static void
emit_sos (int *data_flow, j_compress_ptr cinfo)

{
  int i, td, ta={0};
  jpeg_component_info *compptr={0};

  emit_marker(data_flow, cinfo, M_SOS);

  emit_2bytes(data_flow, cinfo, 2 * cinfo->comps_in_scan + 2 + 1 + 3);

  emit_byte(data_flow, cinfo, cinfo->comps_in_scan);
  ...
  for (i = 0; i < cinfo->comps_in_scan; i++) {
    ...
    emit_byte(data_flow, cinfo, (td << 4) + ta);
  }

  emit_byte(data_flow, cinfo, cinfo->Ss);
  emit_byte(data_flow, cinfo, cinfo->Se);
  emit_byte(data_flow, cinfo, (cinfo->Ah << 4) + cinfo->Al);
}


static void
emit_pseudo_sos (int *data_flow, j_compress_ptr cinfo)

{
  emit_marker(data_flow, cinfo, M_SOS);

  emit_2bytes(data_flow, cinfo, 2 + 1 + 3);

  emit_byte(data_flow, cinfo, 0);

  emit_byte(data_flow, cinfo, 0);
  emit_byte(data_flow, cinfo, cinfo->block_size * cinfo->block_size - 1);
  emit_byte(data_flow, cinfo, 0);
}


static void
emit_jfif_app0 (int *data_flow, j_compress_ptr cinfo)

{
  emit_marker(data_flow, cinfo, M_APP0);

  emit_2bytes(data_flow, cinfo, 2 + 4 + 1 + 2 + 1 + 2 + 2 + 1 + 1);

  emit_byte(data_flow, cinfo, 0x4A);
  emit_byte(data_flow, cinfo, 0x46);
  emit_byte(data_flow, cinfo, 0x49);
  emit_byte(data_flow, cinfo, 0x46);
  emit_byte(data_flow, cinfo, 0);
  emit_byte(data_flow, cinfo, cinfo->JFIF_major_version);
  emit_byte(data_flow, cinfo, cinfo->JFIF_minor_version);
  emit_byte(data_flow, cinfo, cinfo->density_unit);
  emit_2bytes(data_flow, cinfo, (int) cinfo->X_density);
  emit_2bytes(data_flow, cinfo, (int) cinfo->Y_density);
  emit_byte(data_flow, cinfo, 0);
  emit_byte(data_flow, cinfo, 0);
}
...
static void
emit_adobe_app14 (int *data_flow, j_compress_ptr cinfo)

{
  emit_marker(data_flow, cinfo, M_APP14);

  emit_2bytes(data_flow, cinfo, 2 + 5 + 2 + 2 + 2 + 1);

  emit_byte(data_flow, cinfo, 0x41);
  emit_byte(data_flow, cinfo, 0x64);
  emit_byte(data_flow, cinfo, 0x6F);
  emit_byte(data_flow, cinfo, 0x62);
  emit_byte(data_flow, cinfo, 0x65);
  emit_2bytes(data_flow, cinfo, 100);
  emit_2bytes(data_flow, cinfo, 0);
  emit_2bytes(data_flow, cinfo, 0);
  switch (cinfo->jpeg_color_space) {
  case JCS_YCbCr:
    emit_byte(data_flow, cinfo, 1);
    break;
  case JCS_YCCK:
    emit_byte(data_flow, cinfo, 2);
    break;
  default:
    emit_byte(data_flow, cinfo, 0);
    break;
...
static void
write_marker_header (int *data_flow, j_compress_ptr cinfo, int marker, unsigned int datalen)

{
  if (datalen > (unsigned int) 65533)
    ((cinfo)->err->msg_code = (JERR_BAD_LENGTH), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));

  emit_marker(data_flow, cinfo, (JPEG_MARKER) marker);

  emit_2bytes(data_flow, cinfo, (int) (datalen + 2));
}

static void
write_marker_byte (int *data_flow, j_compress_ptr cinfo, int val)

{
  emit_byte(data_flow, cinfo, val);
}
static void
write_file_header (int *data_flow, j_compress_ptr cinfo)
{
  my_marker_ptr marker = (my_marker_ptr) cinfo->marker;

  emit_marker(data_flow, cinfo, M_SOI);


  marker->last_restart_interval = 0;

  if (cinfo->write_JFIF_header)
    emit_jfif_app0(data_flow, cinfo);
  if (cinfo->write_Adobe_marker)
    emit_adobe_app14(data_flow, cinfo);
}
static void
write_frame_header (int *data_flow, j_compress_ptr cinfo)
{
  int ci, prec={0};
  boolean is_baseline={0};
  jpeg_component_info *compptr={0};
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
       ci++, compptr++) {
    prec += emit_dqt(data_flow, cinfo, compptr->quant_tbl_no);
  }
 ...
 is_baseline = FALSE;
    ...
    if (prec && is_baseline) {
      ...
      ((cinfo)->err->msg_code = (JTRC_16BIT_TABLES), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), (0)));
    }
  }
  ...
  if (cinfo->arith_code) {
    if (cinfo->progressive_mode)
      emit_sof(data_flow, cinfo, M_SOF10);
    else
      emit_sof(data_flow, cinfo, M_SOF9);
  } else {
    if (cinfo->progressive_mode)
      emit_sof(data_flow, cinfo, M_SOF2);
    else if (is_baseline)
      emit_sof(data_flow, cinfo, M_SOF0);
    else
      emit_sof(data_flow, cinfo, M_SOF1);
  }


  if (cinfo->color_transform)
    emit_lse_ict(data_flow, cinfo);


  if (cinfo->progressive_mode && cinfo->block_size != 8)
    emit_pseudo_sos(data_flow, cinfo);
}
static void
write_scan_header (int *data_flow, j_compress_ptr cinfo)
{
  my_marker_ptr marker = (my_marker_ptr) cinfo->marker;
  int i={0};
  jpeg_component_info *compptr={0};
  ...
  if (cinfo->arith_code) {
    ...
    for (i = 0; i < cinfo->comps_in_scan; i++) {
      ...
      if (cinfo->Ss == 0 && cinfo->Ah == 0)
 emit_dht(data_flow, cinfo, compptr->dc_tbl_no, FALSE);

      if (cinfo->Se)
 emit_dht(data_flow, cinfo, compptr->ac_tbl_no, TRUE);
    }
  }
  ...
  if (cinfo->restart_interval != marker->last_restart_interval) {
    emit_dri(data_flow, cinfo);
    marker->last_restart_interval = cinfo->restart_interval;
  }

  emit_sos(data_flow, cinfo);
}
...
static void
write_file_trailer (int *data_flow, j_compress_ptr cinfo)
{
  emit_marker(data_flow, cinfo, M_EOI);
}
static void
write_tables_only (int *data_flow, j_compress_ptr cinfo)
{
  int i={0};
  ...
  for (i = 0; i < 4; i++) {
    if (cinfo->quant_tbl_ptrs[i] != 
      ...
      (void) emit_dqt(data_flow, cinfo, i);
  }
 ...
 emit_dht(data_flow, cinfo, i, FALSE);
      if (cinfo->ac_huff_tbl_ptrs[i] != 
                                       ((void *)0)
                                           )
 emit_dht(data_flow, cinfo, i, TRUE);
    }
  }
...
void
jinit_marker_writer (int *data_flow, j_compress_ptr cinfo)
{
