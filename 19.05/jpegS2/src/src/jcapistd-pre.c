...
void
jpeg_start_compress (int *data_flow, j_compress_ptr cinfo, boolean write_all_tables)
{
  ...
  jinit_compress_master(data_flow, cinfo);
...
JDIMENSION
jpeg_write_scanlines (int *data_flow, j_compress_ptr cinfo, JSAMPARRAY scanlines,
        JDIMENSION num_lines)
{
  JDIMENSION row_ctr, rows_left={0};
  ...
  if (cinfo->master->call_pass_startup)
    (*cinfo->master->pass_startup) (data_flow, cinfo);
  ...
  row_ctr = 0;
  (*cinfo->main->process_data) (data_flow, cinfo, scanlines, &row_ctr, num_lines);
...
JDIMENSION
jpeg_write_raw_data (int *data_flow, j_compress_ptr cinfo, JSAMPIMAGE data,
       JDIMENSION num_lines)
{
  ...
  if (! (*cinfo->coef->compress_data) (data_flow, cinfo, data)) {
