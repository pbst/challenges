...
void
jpeg_calc_jpeg_dimensions (int *data_flow, j_compress_ptr cinfo)

{
  ...
  if (cinfo->scale_num >= cinfo->scale_denom * cinfo->block_size) {
    ...
    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 2L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 2L);
    cinfo->min_DCT_h_scaled_size = 2;
    cinfo->min_DCT_v_scaled_size = 2;
  } else if (cinfo->scale_num * 3 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 3L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 3L);
    cinfo->min_DCT_h_scaled_size = 3;
    cinfo->min_DCT_v_scaled_size = 3;
  } else if (cinfo->scale_num * 4 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 4L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 4L);
    cinfo->min_DCT_h_scaled_size = 4;
    cinfo->min_DCT_v_scaled_size = 4;
  } else if (cinfo->scale_num * 5 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 5L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 5L);
    cinfo->min_DCT_h_scaled_size = 5;
    cinfo->min_DCT_v_scaled_size = 5;
  } else if (cinfo->scale_num * 6 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 6L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 6L);
    cinfo->min_DCT_h_scaled_size = 6;
    cinfo->min_DCT_v_scaled_size = 6;
  } else if (cinfo->scale_num * 7 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 7L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 7L);
    cinfo->min_DCT_h_scaled_size = 7;
    cinfo->min_DCT_v_scaled_size = 7;
  } else if (cinfo->scale_num * 8 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 8L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 8L);
    cinfo->min_DCT_h_scaled_size = 8;
    cinfo->min_DCT_v_scaled_size = 8;
  } else if (cinfo->scale_num * 9 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 9L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 9L);
    cinfo->min_DCT_h_scaled_size = 9;
    cinfo->min_DCT_v_scaled_size = 9;
  } else if (cinfo->scale_num * 10 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 10L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 10L);
    cinfo->min_DCT_h_scaled_size = 10;
    cinfo->min_DCT_v_scaled_size = 10;
  } else if (cinfo->scale_num * 11 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 11L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 11L);
    cinfo->min_DCT_h_scaled_size = 11;
    cinfo->min_DCT_v_scaled_size = 11;
  } else if (cinfo->scale_num * 12 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 12L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 12L);
    cinfo->min_DCT_h_scaled_size = 12;
    cinfo->min_DCT_v_scaled_size = 12;
  } else if (cinfo->scale_num * 13 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 13L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 13L);
    cinfo->min_DCT_h_scaled_size = 13;
    cinfo->min_DCT_v_scaled_size = 13;
  } else if (cinfo->scale_num * 14 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 14L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 14L);
    cinfo->min_DCT_h_scaled_size = 14;
    cinfo->min_DCT_v_scaled_size = 14;
  } else if (cinfo->scale_num * 15 >= cinfo->scale_denom * cinfo->block_size) {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 15L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 15L);
    cinfo->min_DCT_h_scaled_size = 15;
    cinfo->min_DCT_v_scaled_size = 15;
  } else {

    cinfo->jpeg_width = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_width * cinfo->block_size, 16L);
    cinfo->jpeg_height = (JDIMENSION)
      jdiv_round_up(data_flow, (long) cinfo->image_height * cinfo->block_size, 16L);
    cinfo->min_DCT_h_scaled_size = 16;
    cinfo->min_DCT_v_scaled_size = 16;
  }
}
void
jinit_compress_master (int *data_flow, j_compress_ptr cinfo)
{
  long samplesperrow={0};
  JDIMENSION jd_samplesperrow={0};
  ...
  jpeg_calc_jpeg_dimensions(data_flow, cinfo);


  jinit_c_master_control(data_flow, cinfo, FALSE );


  if (! cinfo->raw_data_in) {
    jinit_color_converter(data_flow, cinfo);
    jinit_downsampler(data_flow, cinfo);
    jinit_c_prep_controller(data_flow, cinfo, FALSE );
  }

  jinit_forward_dct(data_flow, cinfo);

  if (cinfo->arith_code)
    jinit_arith_encoder(data_flow, cinfo);
  else {
    jinit_huff_encoder(data_flow, cinfo);
  }


  jinit_c_coef_controller(data_flow, cinfo,
  (boolean) (cinfo->num_scans > 1 || cinfo->optimize_coding));
  jinit_c_main_controller(data_flow, cinfo, FALSE );

  jinit_marker_writer(data_flow, cinfo);
  ...
  (*cinfo->marker->write_file_header) (data_flow, cinfo);
}
