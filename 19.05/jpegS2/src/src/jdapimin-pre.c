...
void
jpeg_CreateDecompress (int *data_flow, j_decompress_ptr cinfo, int version, size_t structsize)
{
  int i={0};
  ...
  if (structsize != ((size_t) sizeof(struct jpeg_decompress_struct)))
    ((cinfo)->err->msg_code = (JERR_BAD_STRUCT_SIZE), (cinfo)->err->msg_parm.i[0] = ((int) ((size_t) sizeof(struct jpeg_decompress_struct))), (cinfo)->err->msg_parm.i[1] = ((int) structsize), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)))
                                                                    ;
  ...
  cinfo->marker_list = 
                      ((void *)0)
                          ;
  jinit_marker_reader(data_flow, cinfo);


  jinit_input_controller(data_flow, cinfo);
...
void
jpeg_abort_decompress (int *data_flow, j_decompress_ptr cinfo)
{
...
static void
default_decompress_parms (int *data_flow, j_decompress_ptr cinfo)
{
  int cid0, cid1, cid2={0};
 ...
 break;
      default:
 ((cinfo)->err->msg_code = (JWRN_ADOBE_XFORM), (cinfo)->err->msg_parm.i[0] = (cinfo->Adobe_transform), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
 cinfo->jpeg_color_space = JCS_YCbCr;
 break;
      }
    } else {
      do { int * _mp = (cinfo)->err->msg_parm.i; _mp[0] = (cid0); _mp[1] = (cid1); _mp[2] = (cid2); (cinfo)->err->msg_code = (JTRC_UNKNOWN_IDS); (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), (1)); } while (0);
      cinfo->jpeg_color_space = JCS_YCbCr;
    }
...
int
jpeg_read_header (int *data_flow, j_decompress_ptr cinfo, boolean require_image)
{
  int retcode={0};
  ...
  retcode = jpeg_consume_input(data_flow, cinfo);
  ...
  case 2:
    if (require_image)
      ((cinfo)->err->msg_code = (JERR_NO_IMAGE), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
...
int
jpeg_consume_input (int *data_flow, j_decompress_ptr cinfo)
{
  int retcode = 0;
  ...
  case 200:
    ...
    (*cinfo->inputctl->reset_input_controller) (data_flow, cinfo);

    (*cinfo->src->init_source) (data_flow, cinfo);
    cinfo->global_state = 201;

  case 201:
    retcode = (*cinfo->inputctl->consume_input) (data_flow, cinfo);
    if (retcode == 1) {
...
boolean
jpeg_input_complete (int *data_flow, j_decompress_ptr cinfo)
{
...
boolean
jpeg_has_multiple_scans (int *data_flow, j_decompress_ptr cinfo)
{
...
boolean
jpeg_finish_decompress (int *data_flow, j_decompress_ptr cinfo)
{
  if ((cinfo->global_state == 205 ||
    ...
    (*cinfo->master->finish_output_pass) (data_flow, cinfo);
    cinfo->global_state = 210;
  ...
  (*cinfo->src->term_source) (data_flow, cinfo);
