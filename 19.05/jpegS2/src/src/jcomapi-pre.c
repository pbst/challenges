...
void
jpeg_abort (int *data_flow, j_common_ptr cinfo)
{
  int pool={0};
  ...
  for (pool = 2 -1; pool > 0; pool--) {
    (*cinfo->mem->free_pool) (data_flow, cinfo, pool);
  }
  ...
  if (cinfo->is_decompressor) {
    ...
    ((j_decompress_ptr) cinfo)->marker_list = 
                                             ((void *)0)
                                                 ;
  } else {
...
void
jpeg_destroy (int *data_flow, j_common_ptr cinfo)
{
  ...
  if (cinfo->mem != 
    ...
    (*cinfo->mem->self_destruct) (data_flow, cinfo);
  cinfo->mem = 
              ((void *)0)
                  ;
...
JQUANT_TBL *
jpeg_alloc_quant_table (int *data_flow, j_common_ptr cinfo)
{
  JQUANT_TBL *tbl={0};

  tbl = (JQUANT_TBL *)
    (*cinfo->mem->alloc_small) (data_flow, cinfo, 0, ((size_t) sizeof(JQUANT_TBL)));
...
JHUFF_TBL *
jpeg_alloc_huff_table (int *data_flow, j_common_ptr cinfo)
{
  JHUFF_TBL *tbl={0};

  tbl = (JHUFF_TBL *)
    (*cinfo->mem->alloc_small) (data_flow, cinfo, 0, ((size_t) sizeof(JHUFF_TBL)));
  tbl->sent_table = FALSE;
  return tbl;
}
