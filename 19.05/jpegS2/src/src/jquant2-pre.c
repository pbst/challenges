...
static void
prescan_quantize (int *data_flow, j_decompress_ptr cinfo, JSAMPARRAY input_buf,
    JSAMPARRAY output_buf, int num_rows)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  register JSAMPROW ptr={0};
  register histptr histp={0};
...
static boxptr
find_biggest_color_pop (int *data_flow, boxptr boxlist, int numboxes)


{
  register boxptr boxp={0};
  register int i={0};
...
static boxptr
find_biggest_volume (int *data_flow, boxptr boxlist, int numboxes)
...
static void
update_box (int *data_flow, j_decompress_ptr cinfo, boxptr boxp)
...

{
  ...
  histptr histp={0};
  int c0,c1,c2={0};
  int c0min,c0max,c1min,c1max,c2min,c2max={0};
  INT32 dist0,dist1,dist2={0};
  long ccount={0};
...
static int
median_cut (int *data_flow, j_decompress_ptr cinfo, boxptr boxlist, int numboxes,
     int desired_colors)

{
  int n,lb={0};
  int c0,c1,c2,cmax={0};
  register boxptr b1,b2={0};
  ...
  while (numboxes < desired_colors) {
    ...
    if (numboxes*2 <= desired_colors) {
      b1 = find_biggest_color_pop(data_flow, boxlist, numboxes);
    } else {
      b1 = find_biggest_volume(data_flow, boxlist, numboxes);
    }
    if (b1 == 
             ((void *)0)
                 )
    ...
    update_box(data_flow, cinfo, b1);
    update_box(data_flow, cinfo, b2);
    numboxes++;
  }
  return numboxes;
}
...
static void
compute_color (int *data_flow, j_decompress_ptr cinfo, boxptr boxp, int icolor)
...
static void
select_colors (int *data_flow, j_decompress_ptr cinfo, int desired_colors)

{
  ...
  boxlist = (boxptr) (*cinfo->mem->alloc_small)
    (data_flow, (j_common_ptr) cinfo, 1, desired_colors * ((size_t) sizeof(box)));
  ...
  update_box(data_flow, cinfo, & boxlist[0]);

  numboxes = median_cut(data_flow, cinfo, boxlist, numboxes, desired_colors);

  for (i = 0; i < numboxes; i++)
    compute_color(data_flow, cinfo, & boxlist[i], i);
  cinfo->actual_number_of_colors = numboxes;
  ((cinfo)->err->msg_code = (JTRC_QUANT_SELECTED), (cinfo)->err->msg_parm.i[0] = (numboxes), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), (1)));
}
static int
find_nearby_colors (int *data_flow, j_decompress_ptr cinfo, int minc0, int minc1, int minc2,
      JSAMPLE colorlist[])
{
  int numcolors = cinfo->actual_number_of_colors;
  int maxc0, maxc1, maxc2={0};
  int centerc0, centerc1, centerc2={0};
  int i, x, ncolors={0};
  INT32 minmaxdist, min_dist, max_dist, tdist={0};
  INT32 mindist[(255 +1)]={0};
...
static void
find_best_colors (int *data_flow, j_decompress_ptr cinfo, int minc0, int minc1, int minc2,
...

{
  int ic0, ic1, ic2={0};
  int i, icolor={0};
  register INT32 * bptr={0};
  JSAMPLE * cptr={0};
  INT32 dist0, dist1={0};
  register INT32 dist2={0};
  INT32 xx0, xx1={0};
  register INT32 xx2={0};
  INT32 inc0, inc1, inc2={0};

  INT32 bestdist[(1<<(5 -3)) * (1<<(6 -3)) * (1<<(5 -3))]={0};
...
static void
fill_inverse_cmap (int *data_flow, j_decompress_ptr cinfo, int c0, int c1, int c2)
...

{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  hist3d histogram = cquantize->histogram;
  int minc0, minc1, minc2={0};
  int ic0, ic1, ic2={0};
  register JSAMPLE * cptr={0};
  register histptr cachep={0};

  JSAMPLE colorlist[(255 +1)]={0};
  int numcolors={0};

  JSAMPLE bestcolor[(1<<(5 -3)) * (1<<(6 -3)) * (1<<(5 -3))]={0};
  ...
  numcolors = find_nearby_colors(data_flow, cinfo, minc0, minc1, minc2, colorlist);


  find_best_colors(data_flow, cinfo, minc0, minc1, minc2, numcolors, colorlist,
     bestcolor);
...
static void
pass2_no_dither (int *data_flow, j_decompress_ptr cinfo,
   JSAMPARRAY input_buf, JSAMPARRAY output_buf, int num_rows)

{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  hist3d histogram = cquantize->histogram;
  register JSAMPROW inptr, outptr={0};
  register histptr cachep={0};
  register int c0, c1, c2={0};
  int row={0};
  JDIMENSION col={0};
  JDIMENSION width = cinfo->output_width;
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    for (col = width; col > 0; col--) {
      ...
      if (*cachep == 0)
 fill_inverse_cmap(data_flow, cinfo, c0,c1,c2);
...
static void
pass2_fs_dither (int *data_flow, j_decompress_ptr cinfo,
   JSAMPARRAY input_buf, JSAMPARRAY output_buf, int num_rows)

{
  ...
  for (row = 0; row < num_rows; row++) {
    ...
    for (col = width; col > 0; col--) {
      ...
      if (*cachep == 0)
 fill_inverse_cmap(data_flow, cinfo, cur0>>(8 -5),cur1>>(8 -6),cur2>>(8 -5));
 ...
 *outptr = (JSAMPLE) pixcode;
 ...
 cur2 -= ((int) (colormap2[pixcode]));
      ...
      { register LOCFSERROR bnexterr, delta={0};
...
static void
init_error_limit (int *data_flow, j_decompress_ptr cinfo)

{
  ...
  table = (int *) (*cinfo->mem->alloc_small)
    (data_flow, (j_common_ptr) cinfo, 1, (255*2+1) * ((size_t) sizeof(int)));
  table += 255;
...
static void
finish_pass1 (int *data_flow, j_decompress_ptr cinfo)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;


  cinfo->colormap = cquantize->sv_colormap;
  select_colors(data_flow, cinfo, cquantize->desired);
...
static void
finish_pass2 (int *data_flow, j_decompress_ptr cinfo)
{
...
static void
start_pass_2_quant (int *data_flow, j_decompress_ptr cinfo, boolean is_pre_scan)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  hist3d histogram = cquantize->histogram;
  int i={0};
  ...
  if (is_pre_scan) {
    ...
    if (i < 1)
      ((cinfo)->err->msg_code = (JERR_QUANT_FEW_COLORS), (cinfo)->err->msg_parm.i[0] = (1), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
    if (i > (255 +1))
      ((cinfo)->err->msg_code = (JERR_QUANT_MANY_COLORS), (cinfo)->err->msg_parm.i[0] = ((255 +1)), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
 ...
 cquantize->fserrors = (FSERRPTR) (*cinfo->mem->alloc_large)
   (data_flow, (j_common_ptr) cinfo, 1, arraysize);

      memset((void *)((void *) cquantize->fserrors), 0, (size_t)(arraysize));

      if (cquantize->error_limiter == 
                                     ((void *)0)
                                         )
 init_error_limit(data_flow, cinfo);
      cquantize->on_odd_row = FALSE;
    }
  ...
  if (cquantize->needs_zeroed) {
    for (i = 0; i < (1<<5); i++) {
      memset((void *)((void *) histogram[i]), 0, (size_t)((1<<6)*(1<<5) * ((size_t) sizeof(histcell))))
                                                       ;
    }
...
static void
new_color_map_2_quant (int *data_flow, j_decompress_ptr cinfo)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
...
void
jinit_2pass_quantizer (int *data_flow, j_decompress_ptr cinfo)
{
  ...
  if (cinfo->out_color_components != 3)
    ((cinfo)->err->msg_code = (JERR_NOTIMPL), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));


  cquantize->histogram = (hist3d) (*cinfo->mem->alloc_small)
    (data_flow, (j_common_ptr) cinfo, 1, (1<<5) * ((size_t) sizeof(hist2d)));
  for (i = 0; i < (1<<5); i++) {
  ...
  if (cinfo->enable_2pass_quant) {
    ...
    if (desired < 8)
      ((cinfo)->err->msg_code = (JERR_QUANT_FEW_COLORS), (cinfo)->err->msg_parm.i[0] = (8), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));

    if (desired > (255 +1))
      ((cinfo)->err->msg_code = (JERR_QUANT_MANY_COLORS), (cinfo)->err->msg_parm.i[0] = ((255 +1)), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));
    cquantize->sv_colormap = (*cinfo->mem->alloc_sarray)
      (data_flow, (j_common_ptr) cinfo,1, (JDIMENSION) desired, (JDIMENSION) 3);
    cquantize->desired = desired;
  } else
    cquantize->sv_colormap = 
                            ((void *)0)
                                ;
