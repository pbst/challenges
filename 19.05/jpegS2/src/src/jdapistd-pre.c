...
static boolean output_pass_setup (int *data_flow, j_decompress_ptr cinfo);
boolean
jpeg_start_decompress (int *data_flow, j_decompress_ptr cinfo)
{
  if (cinfo->global_state == 202) {

    jinit_master_decompress(data_flow, cinfo);
 ...
 if (cinfo->progress != 
  ...
  return output_pass_setup(data_flow, cinfo);
}
static boolean
output_pass_setup (int *data_flow, j_decompress_ptr cinfo)
{
  if (cinfo->global_state != 204) {

    (*cinfo->master->prepare_for_output_pass) (data_flow, cinfo);
 ...
 (*cinfo->progress->progress_monitor) (data_flow, (j_common_ptr) cinfo);
      ...
      (*cinfo->main->process_data) (data_flow, cinfo, (JSAMPARRAY) 
                                                       ((void *)0)
                                                           ,
...
JDIMENSION
jpeg_read_scanlines (int *data_flow, j_decompress_ptr cinfo, JSAMPARRAY scanlines,
       JDIMENSION max_lines)
{
  ...
  if (cinfo->output_scanline >= cinfo->output_height) {
    ((cinfo)->err->msg_code = (JWRN_TOO_MUCH_DATA), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
    return 0;
  }
  ...
  row_ctr = 0;
  (*cinfo->main->process_data) (data_flow, cinfo, scanlines, &row_ctr, max_lines);
...
JDIMENSION
jpeg_read_raw_data (int *data_flow, j_decompress_ptr cinfo, JSAMPIMAGE data,
      JDIMENSION max_lines)
{
  JDIMENSION lines_per_iMCU_row={0};
  ...
  if (max_lines < lines_per_iMCU_row)
    ((cinfo)->err->msg_code = (JERR_BUFFER_SIZE), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)));


  if (! (*cinfo->coef->decompress_data) (data_flow, cinfo, data))
    return 0;
...
boolean
jpeg_start_output (int *data_flow, j_decompress_ptr cinfo, int scan_number)
{
...
boolean
jpeg_finish_output (int *data_flow, j_decompress_ptr cinfo)
{
  if ((cinfo->global_state == 205 ||
