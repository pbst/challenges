...
static void
start_pass_prep (int *data_flow, j_compress_ptr cinfo, J_BUF_MODE pass_mode)
{
...
static void
expand_bottom_edge (int *data_flow, JSAMPARRAY image_data, JDIMENSION num_cols,
      int input_rows, int output_rows)
{
  register int row={0};

  for (row = input_rows; row < output_rows; row++) {
    jcopy_sample_rows(data_flow, image_data, input_rows-1, image_data, row,
        1, num_cols);
  }
}
static void
pre_process_data (int *data_flow, j_compress_ptr cinfo,
    ...
    JDIMENSION out_row_groups_avail)
{
  ...
  *out_row_group_ctr < out_row_groups_avail) {
    ...
    (*cinfo->cconvert->color_convert) (data_flow, cinfo, input_buf + *in_row_ctr,
           prep->color_buf,
 ...
 expand_bottom_edge(data_flow, prep->color_buf[ci], cinfo->image_width,
      prep->next_buf_row, cinfo->max_v_samp_factor);
      }
      prep->next_buf_row = cinfo->max_v_samp_factor;
    }
    ...
    if (prep->next_buf_row == cinfo->max_v_samp_factor) {
      (*cinfo->downsample->downsample) (data_flow, cinfo,
 ...
 *out_row_group_ctr < out_row_groups_avail) {
 ...
 expand_bottom_edge(data_flow, output_buf[ci],
...
static void
pre_process_context (int *data_flow, j_compress_ptr cinfo,
       JSAMPARRAY input_buf, JDIMENSION *in_row_ctr,
       JDIMENSION in_rows_avail,
       JSAMPIMAGE output_buf, JDIMENSION *out_row_group_ctr,
       JDIMENSION out_row_groups_avail)
{
 ...
 for (ci = 0; ci < cinfo->num_components; ci++) {
   ...
   for (row = 1; row <= cinfo->max_v_samp_factor; row++) {
     jcopy_sample_rows(data_flow, prep->color_buf[ci], 0,
...
static void
create_context_buffer (int *data_flow, j_compress_ptr cinfo)
{
  my_prep_ptr prep = (my_prep_ptr) cinfo->prep;
  int rgroup_height = cinfo->max_v_samp_factor;
  int ci, i={0};
  jpeg_component_info * compptr={0};
  JSAMPARRAY true_buffer, fake_buffer={0};
...
void
jinit_c_prep_controller (int *data_flow, j_compress_ptr cinfo, boolean need_full_buffer)
{
  my_prep_ptr prep={0};
  int ci={0};
  jpeg_component_info * compptr={0};
  ...
  if (cinfo->downsample->need_context_rows) {
    ...
    create_context_buffer(data_flow, cinfo);
