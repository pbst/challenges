...
static void
jpeg_make_d_derived_tbl (int *data_flow, j_decompress_ptr cinfo, boolean isDC, int tblno,
    d_derived_tbl ** pdtbl)
{
  JHUFF_TBL *htbl={0};
  d_derived_tbl *dtbl={0};
  int p, i, l, si, numsymbols={0};
  int lookbits, ctr={0};
  char huffsize[257]={0};
...
static boolean
jpeg_fill_bit_buffer (int *data_flow, bitread_working_state * state,
...

{
 ...
 if (! (*cinfo->src->fill_input_buffer) (data_flow, cinfo))
   return FALSE;
 ...
 ((cinfo)->err->msg_code = (JWRN_HIT_MARKER), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
...
static int
jpeg_huff_decode (int *data_flow, bitread_working_state * state,
    register bit_buf_type get_buffer, register int bits_left,
    d_derived_tbl * htbl, int min_bits)
{
  register int l = min_bits;
  register INT32 code={0};




  { if (bits_left < (l)) { if (! jpeg_fill_bit_buffer(data_flow, &(*state),get_buffer,bits_left,l)) { return -1; } get_buffer = (*state).get_buffer; bits_left = (*state).bits_left; } };
  ...
  while (code > htbl->maxcode[l]) {
    code <<= 1;
    { if (bits_left < (1)) { if (! jpeg_fill_bit_buffer(data_flow, &(*state),get_buffer,bits_left,1)) { return -1; } get_buffer = (*state).get_buffer; bits_left = (*state).bits_left; } };
  ...
  if (l > 16) {
    ((state->cinfo)->err->msg_code = (JWRN_HUFF_BAD_CODE), (*(state->cinfo)->err->emit_message) (data_flow, (j_common_ptr) (state->cinfo), -1));
    return 0;
  }
...
static void
finish_pass_huff (int *data_flow, j_decompress_ptr cinfo)
{
...
static boolean
process_restart (int *data_flow, j_decompress_ptr cinfo)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  int ci={0};

  finish_pass_huff(data_flow, cinfo);


  if (! (*cinfo->marker->read_restart_marker) (data_flow, cinfo))
    return FALSE;
...
static boolean
decode_mcu_DC_first (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  int Al = cinfo->Al;
  register int s, r={0};
  int blkn, ci={0};
  JBLOCKROW block={0};
  register bit_buf_type get_buffer={0}; register int bits_left={0}; bitread_working_state br_state={0};
  savable_state state={0};
  d_derived_tbl * tbl={0};
  ...
  if (cinfo->restart_interval) {
    if (entropy->restarts_to_go == 0)
      if (! process_restart(data_flow, cinfo))
 return FALSE;
  }
  ...
  if (! entropy->insufficient_data) {
    ...
    for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
      ...
      { register int nb, look={0}; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(data_flow, &br_state,get_buffer,bits_left, 0)) {return FALSE;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label1; } } look = (((int) (get_buffer >> (bits_left - (8)))) & bmask[8]); if ((nb = tbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = tbl->look_sym[look]; } else { nb = 8 +1; label1: if ((s=jpeg_huff_decode(data_flow, &br_state,get_buffer,bits_left,tbl,nb)) < 0) { return FALSE; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
      if (s) {
...
static boolean
decode_mcu_AC_first (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  register int s, k, r={0};
 ...
 { register int nb, look={0}; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(data_flow, &br_state,get_buffer,bits_left, 0)) {return FALSE;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label2; } } look = (((int) (get_buffer >> (bits_left - (8)))) & bmask[8]); if ((nb = tbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = tbl->look_sym[look]; } else { nb = 8 +1; label2: if ((s=jpeg_huff_decode(data_flow, &br_state,get_buffer,bits_left,tbl,nb)) < 0) { return FALSE; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
 r = s >> 4;
 s &= 15;
 if (s) {
   k += r;
   ...
   if (r != 15) {
     if (r) {
       EOBRUN = 1 << r;
       { if (bits_left < (r)) { if (! jpeg_fill_bit_buffer(data_flow, &(br_state),get_buffer,bits_left,r)) { return FALSE; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
...
static boolean
decode_mcu_DC_refine (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  int p1, blkn={0};
  register bit_buf_type get_buffer={0}; register int bits_left={0}; bitread_working_state br_state={0};
 ...
 return FALSE;
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    { if (bits_left < (1)) { if (! jpeg_fill_bit_buffer(data_flow, &(br_state),get_buffer,bits_left,1)) { return FALSE; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
...
static boolean
decode_mcu_AC_refine (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  register int s, k, r={0};
  unsigned int EOBRUN={0};
 ...
 return FALSE;
  ...
  if (! entropy->insufficient_data) {
    ...
    if (EOBRUN == 0) {
      do {
 { register int nb, look={0}; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(data_flow, &br_state,get_buffer,bits_left, 0)) {goto undoit;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label3; } } look = (((int) (get_buffer >> (bits_left - (8)))) & bmask[8]); if ((nb = tbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = tbl->look_sym[look]; } else { nb = 8 +1; label3: if ((s=jpeg_huff_decode(data_flow, &br_state,get_buffer,bits_left,tbl,nb)) < 0) { goto undoit; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
 r = s >> 4;
 s &= 15;
 if (s) {
   if (s != 1)
     ((cinfo)->err->msg_code = (JWRN_HUFF_BAD_CODE), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
   { if (bits_left < (1)) { if (! jpeg_fill_bit_buffer(data_flow, &(br_state),get_buffer,bits_left,1)) { goto undoit; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
   if ((((int) (get_buffer >> (bits_left -= (1)))) & bmask[1]))
     s = p1;
   else
     s = m1;
   ...
   if (r != 15) {
     EOBRUN = 1 << r;
     if (r) {
       { if (bits_left < (r)) { if (! jpeg_fill_bit_buffer(data_flow, &(br_state),get_buffer,bits_left,r)) { goto undoit; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
       r = (((int) (get_buffer >> (bits_left -= (r)))) & bmask[r]);
       EOBRUN += r;
     }
     break;
...
static boolean
decode_mcu_sub (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  const int * natural_order={0};
  int Se, blkn={0};
 ...
 return FALSE;
  ...
  if (! entropy->insufficient_data) {
    ...
    for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
      ...
      { register int nb, look={0}; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(data_flow, &br_state,get_buffer,bits_left, 0)) {return FALSE;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label1; } } look = (((int) (get_buffer >> (bits_left - (8)))) & bmask[8]); if ((nb = htbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = htbl->look_sym[look]; } else { nb = 8 +1; label1: if ((s=jpeg_huff_decode(data_flow, &br_state,get_buffer,bits_left,htbl,nb)) < 0) { return FALSE; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
 ...
 for (; k < coef_limit; k++) {
   { register int nb, look={0}; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(data_flow, &br_state,get_buffer,bits_left, 0)) {return FALSE;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label2; } } look = (((int) (get_buffer >> (bits_left - (8)))) & bmask[8]); if ((nb = htbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = htbl->look_sym[look]; } else { nb = 8 +1; label2: if ((s=jpeg_huff_decode(data_flow, &br_state,get_buffer,bits_left,htbl,nb)) < 0) { return FALSE; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
 ...
 if (s) {
   ...
   (bits_left -= (s));
      ...
      for (; k <= Se; k++) {
 { register int nb, look={0}; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(data_flow, &br_state,get_buffer,bits_left, 0)) {return FALSE;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label3; } } look = (((int) (get_buffer >> (bits_left - (8)))) & bmask[8]); if ((nb = htbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = htbl->look_sym[look]; } else { nb = 8 +1; label3: if ((s=jpeg_huff_decode(data_flow, &br_state,get_buffer,bits_left,htbl,nb)) < 0) { return FALSE; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
...
static boolean
decode_mcu (int *data_flow, j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  int blkn={0};
  register bit_buf_type get_buffer={0}; register int bits_left={0}; bitread_working_state br_state={0};
...
static void
start_pass_huff_decoder (int *data_flow, j_decompress_ptr cinfo)
{
  huff_entropy_ptr entropy = (huff_entropy_ptr) cinfo->entropy;
  int ci, blkn, tbl, i={0};
  jpeg_component_info * compptr={0};
 ...
 goto bad;
    ...
    if (cinfo->Al > 13) {
      ...
      bad:
      ((cinfo)->err->msg_code = (JERR_BAD_PROGRESSION), (cinfo)->err->msg_parm.i[0] = (cinfo->Ss), (cinfo)->err->msg_parm.i[1] = (cinfo->Se), (cinfo)->err->msg_parm.i[2] = (cinfo->Ah), (cinfo)->err->msg_parm.i[3] = (cinfo->Al), (*(cinfo)->err->error_exit) (data_flow, (j_common_ptr) (cinfo)))
                                                   ;
    }
 ...
 ((cinfo)->err->msg_code = (JWRN_BOGUS_PROGRESSION), (cinfo)->err->msg_parm.i[0] = (cindex), (cinfo)->err->msg_parm.i[1] = (0), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
      for (coefi = cinfo->Ss; coefi <= cinfo->Se; coefi++) {
 int expected = (coef_bit_ptr[coefi] < 0) ? 0 : coef_bit_ptr[coefi];
 if (cinfo->Ah != expected)
   ((cinfo)->err->msg_code = (JWRN_BOGUS_PROGRESSION), (cinfo)->err->msg_parm.i[0] = (cindex), (cinfo)->err->msg_parm.i[1] = (coefi), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
 ...
 if (cinfo->Ah == 0) {
   ...
   jpeg_make_d_derived_tbl(data_flow, cinfo, TRUE, tbl,
 ...
 cinfo->Se != cinfo->lim_Se))
      ((cinfo)->err->msg_code = (JWRN_NOT_SEQUENTIAL), (*(cinfo)->err->emit_message) (data_flow, (j_common_ptr) (cinfo), -1));
...
void
jinit_huff_decoder (int *data_flow, j_decompress_ptr cinfo)
{
