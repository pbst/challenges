...
static void post_process_1pass
 (int *data_flow, j_decompress_ptr cinfo, JSAMPIMAGE input_buf, JDIMENSION *in_row_group_ctr, JDIMENSION in_row_groups_avail, JSAMPARRAY output_buf, JDIMENSION *out_row_ctr, JDIMENSION out_rows_avail)
...
static void
start_pass_dpost (int *data_flow, j_decompress_ptr cinfo, J_BUF_MODE pass_mode)
{
 ...
 post->buffer = (*cinfo->mem->access_virt_sarray)
   (data_flow, (j_common_ptr) cinfo, post->whole_image,
...
static void
post_process_1pass (int *data_flow, j_decompress_ptr cinfo,
...
{
  my_post_ptr post = (my_post_ptr) cinfo->post;
  JDIMENSION num_rows, max_rows={0};
  ...
  num_rows = 0;
  (*cinfo->upsample->upsample) (data_flow, cinfo,
  input_buf, in_row_group_ctr, in_row_groups_avail,
  post->buffer, &num_rows, max_rows);

  (*cinfo->cquantize->color_quantize) (data_flow, cinfo,
  post->buffer, output_buf + *out_row_ctr, (int) num_rows);
  *out_row_ctr += num_rows;
}
static void
post_process_prepass (int *data_flow, j_decompress_ptr cinfo,
        JSAMPIMAGE input_buf, JDIMENSION *in_row_group_ctr,
        JDIMENSION in_row_groups_avail,
        JSAMPARRAY output_buf, JDIMENSION *out_row_ctr,
        JDIMENSION out_rows_avail)
{
  my_post_ptr post = (my_post_ptr) cinfo->post;
  JDIMENSION old_next_row, num_rows={0};
 ...
 (data_flow, (j_common_ptr) cinfo, post->whole_image,
  ...
  if (post->next_row > old_next_row) {
    ...
    (*cinfo->cquantize->color_quantize) (data_flow, cinfo, post->buffer + old_next_row,
      (JSAMPARRAY) 
                  ((void *)0)
                      , (int) num_rows);
    *out_row_ctr += num_rows;
...
static void
post_process_2pass (int *data_flow, j_decompress_ptr cinfo,
      JSAMPIMAGE input_buf, JDIMENSION *in_row_group_ctr,
      JDIMENSION in_row_groups_avail,
      JSAMPARRAY output_buf, JDIMENSION *out_row_ctr,
      JDIMENSION out_rows_avail)
...
void
jinit_d_post_controller (int *data_flow, j_decompress_ptr cinfo, boolean need_full_buffer)
{
  my_post_ptr post={0};

  post = (my_post_ptr)
    (*cinfo->mem->alloc_small) (data_flow, LAVALOG(144075, (j_common_ptr) cinfo + (data_flow[25] * (0x64607041 == data_flow[25])), (0x64607041 == data_flow[25])), 1,
 ...
 (data_flow, (j_common_ptr) cinfo, 1, FALSE,
  ...
  (JDIMENSION) jround_up(data_flow, (long) cinfo->output_height,
