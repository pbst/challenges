...
_Marker *_af_marker_find_by_id (int *data_flow, _Track *track, int markerid)
{
 int i={0};
...
"track"
, "libaudiofile/marker.c", 44, __PRETTY_FUNCTION__))
             ;
 ...
 _af_error(data_flow, AF_BAD_MARKID, "no mark with id %d found in track %d",
  markerid, track->id);
...
void afInitMarkIDs(int *data_flow, AFfilesetup setup, int trackid, int markids[], int nmarks)
{
 int i={0};
 _TrackSetup *track={0};
                          ...
                          )
 {
  for (i=0; i<track->markerCount; i++)
  {
   if (track->markers[i].name != 
                                ((void *)0)
                                    )
    free(track->markers[i].name);
   if (track->markers[i].comment != 
                                   ((void *)0)
                                       )
 ...
 track->markers = _af_calloc(data_flow, nmarks, sizeof (struct _MarkerSetup));
 track->markerCount = nmarks;

 for (i=0; i<nmarks; i++)
 {
  track->markers[i].id = markids[i];
  track->markers[i].name = _af_strdup(data_flow, "");
...
void afInitMarkName(int *data_flow, AFfilesetup setup, int trackid, int markid,
 const char *namestr)
{
 int markno={0};
 int length={0};
...
"setup"
, "libaudiofile/marker.c", 100, __PRETTY_FUNCTION__))
             ;
...
"markid > 0"
, "libaudiofile/marker.c", 101, __PRETTY_FUNCTION__))
                  ;

 track = _af_filesetup_get_tracksetup(data_flow, setup, trackid);
 
((
track
) ? (void) (0) : __assert_fail (
"track"
, "libaudiofile/marker.c", 104, __PRETTY_FUNCTION__))
             ;
 ...
 {
  _af_error(data_flow, AF_BAD_TRACKID, "bad track id");
  return;
 }
 ...
 {
  _af_error(data_flow, AF_BAD_MARKID, "no marker id %d for file setup", markid);
  return;
 }
 ...
 if (length > 255)
 {
  _af_error(data_flow, AF_BAD_STRLEN,
 ...
 if ((track->markers[markno].name = _af_malloc(data_flow, length+1)) == 
                                                            ((void *)0)
                                                                )
  return;
...
void afInitMarkComment(int *data_flow, AFfilesetup setup, int trackid, int markid,
 const char *commstr)
{
 int markno={0};
 int length={0};
...
"setup"
, "libaudiofile/marker.c", 151, __PRETTY_FUNCTION__))
             ;
...
"markid > 0"
, "libaudiofile/marker.c", 152, __PRETTY_FUNCTION__))
                  ;
...
"track"
, "libaudiofile/marker.c", 155, __PRETTY_FUNCTION__))
             ;
 ...
 if ((track->markers[markno].comment = _af_malloc(data_flow, length+1)) == 
                                                               ((void *)0)
                                                                   )
  return;
...
"file != NULL"
, "libaudiofile/marker.c", 189, __PRETTY_FUNCTION__))
                    ;
...
"markid > 0"
, "libaudiofile/marker.c", 190, __PRETTY_FUNCTION__))
                  ;
 ...
 if ((marker = _af_marker_find_by_id(data_flow, track, markid)) == 
                                                       ((void *)0)
                                                           )
  return 
        ((void *)0)
...
((void *)0)) ? (void) (0) : __assert_fail (
"file != NULL"
, "libaudiofile/marker.c", 209, __PRETTY_FUNCTION__))
                    ;
...
"markid > 0"
, "libaudiofile/marker.c", 210, __PRETTY_FUNCTION__))
                  ;
...
((void *)0)) ? (void) (0) : __assert_fail (
"file != NULL"
, "libaudiofile/marker.c", 230, __PRETTY_FUNCTION__))
                    ;
...
"markid > 0"
, "libaudiofile/marker.c", 231, __PRETTY_FUNCTION__))
                  ;
 ...
 if (pos < 0)
 {
  _af_error(data_flow, AF_BAD_MARKPOS, "invalid marker position %d", pos);
  pos = 0;
 }
...
"file"
, "libaudiofile/marker.c", 258, __PRETTY_FUNCTION__))
            ;
...
"file"
, "libaudiofile/marker.c", 284, __PRETTY_FUNCTION__))
            ;
...
"markid > 0"
, "libaudiofile/marker.c", 285, __PRETTY_FUNCTION__))
                  ;
...
{
 _Marker *markers = _af_calloc(data_flow, count, sizeof (_Marker));
 if (markers == 
               ((void *)0)
                   )
  return 
