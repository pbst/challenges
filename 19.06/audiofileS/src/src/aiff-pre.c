...
void _af_convert_to_ieee_extended (int *data_flow, double num, unsigned char *bytes);
double _af_convert_from_ieee_extended (int *data_flow, const unsigned char *bytes);
enum
{
 AU_PVTYPE_LONG = 1,
 AU_PVTYPE_DOUBLE = 2,
...
static status ParseFVER (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type,
 size_t size);
static status ParseAESD (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type,
 size_t size);
static status ParseMiscellaneous (int *data_flow, AFfilehandle file, AFvirtualfile *fh,
 uint32_t type, size_t size);
static status ParseINST (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type,
 size_t size);
static status ParseMARK (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type,
 size_t size);
static status ParseCOMM (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type,
 size_t size);
static status ParseSSND (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type,
 size_t size);

const _InstParamInfo _af_aiff_inst_params[9] =
{
...
const int _af_aiffc_compression_types[2] =
{
...
static status ParseFVER (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type, size_t size)
{
...
"!memcmp(&type, \"FVER\", 4)"
, "libaudiofile/aiff.c", 105, __PRETTY_FUNCTION__))
                                 ;

 af_read_uint32_be(data_flow, &timestamp, fh);
...
static status ParseAESD (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type, size_t size)
{
 _Track *track={0};
 unsigned char aesChannelStatusData[24]={0};
...
"!memcmp(&type, \"AESD\", 4)"
, "libaudiofile/aiff.c", 121, __PRETTY_FUNCTION__))
                                 ;
...
"size == 24"
, "libaudiofile/aiff.c", 122, __PRETTY_FUNCTION__))
                  ;
 ...
 if (af_fread(data_flow, aesChannelStatusData, 1, 24, fh) != 24)
  return (-1);
...
((
!memcmp(&type, "NAME", 4) || !memcmp(&type, "AUTH", 4) || !memcmp(&type, "(c) ", 4) || !memcmp(&type, "ANNO", 4) || !memcmp(&type, "APPL", 4) || !memcmp(&type, "MIDI", 4)
) ? (void) (0) : __assert_fail (
"!memcmp(&type, \"NAME\", 4) || !memcmp(&type, \"AUTH\", 4) || !memcmp(&type, \"(c) \", 4) || !memcmp(&type, \"ANNO\", 4) || !memcmp(&type, \"APPL\", 4) || !memcmp(&type, \"MIDI\", 4)"
,
 ...
 file->miscellaneous = _af_realloc(data_flow, file->miscellaneous,
 ...
 file->miscellaneous[file->miscellaneousCount - 1].buffer = _af_malloc(data_flow, size);
 af_fread(data_flow, file->miscellaneous[file->miscellaneousCount - 1].buffer,
  size, 1, file->fh);
 ...
 size_t size)
{
 ...
 uint16_t sustainLoopPlayMode, sustainLoopBegin, sustainLoopEnd={0};
 uint16_t releaseLoopPlayMode, releaseLoopBegin, releaseLoopEnd={0};
...
"!memcmp(&type, \"INST\", 4)"
, "libaudiofile/aiff.c", 201, __PRETTY_FUNCTION__))
                                 ;

 instrument = _af_calloc(data_flow, 1, sizeof (_Instrument));
 instrument->id = AF_DEFAULT_INST;
 instrument->values = _af_calloc(data_flow, 9, sizeof (AFPVu));
 instrument->loopCount = 2;
 instrument->loops = _af_calloc(data_flow, 2, sizeof (_Loop));

 file->instrumentCount = 1;
 file->instruments = instrument;

 af_read_uint8(data_flow, &baseNote, fh);
 af_read_uint8(data_flow, &detune, fh);
 af_read_uint8(data_flow, &lowNote, fh);
 af_read_uint8(data_flow, &highNote, fh);
 af_read_uint8(data_flow, &lowVelocity, fh);
 af_read_uint8(data_flow, &highVelocity, fh);
 af_read_uint16_be(data_flow, &gain, fh);
 ...
 instrument->values[7].l = 1;
 instrument->values[8].l = 2;

 af_read_uint16_be(data_flow, &sustainLoopPlayMode, fh);
 af_read_uint16_be(data_flow, &sustainLoopBegin, fh);
 af_read_uint16_be(data_flow, &sustainLoopEnd, fh);

 af_read_uint16_be(data_flow, &releaseLoopPlayMode, fh);
 af_read_uint16_be(data_flow, &releaseLoopBegin, fh);
 af_read_uint16_be(data_flow, &releaseLoopEnd, fh);
...
"!memcmp(&type, \"MARK\", 4)"
, "libaudiofile/aiff.c", 277, __PRETTY_FUNCTION__))
                                 ;

 track = _af_filehandle_get_track(data_flow, file, AF_DEFAULT_TRACK);

 af_read_uint16_be(data_flow, &numMarkers, fh);

 track->markerCount = numMarkers;
 if (numMarkers)
  track->markers = _af_marker_new(data_flow, numMarkers);
 ...
 for (i=0; i<numMarkers; i++)
 {
  ...
  af_read_uint16_be(data_flow, &markerID, fh);
  af_read_uint32_be(data_flow, &markerPosition, fh);
  af_fread(data_flow, &sizeByte, sizeof (unsigned char), 1, fh);
  markerName = _af_malloc(data_flow, sizeByte + 1);
  af_fread(data_flow, markerName, sizeof (unsigned char), sizeByte, fh);

  markerName[sizeByte] = '\0';
  if ((sizeByte % 2) == 0)
   af_fseek(data_flow, fh, 1, 
                  1
                          );
  ...
  track->markers[i].comment = _af_strdup(data_flow, "");
 }
...
"!memcmp(&type, \"COMM\", 4)"
, "libaudiofile/aiff.c", 341, __PRETTY_FUNCTION__))
                                 ;

 track = _af_filehandle_get_track(data_flow, file, AF_DEFAULT_TRACK);

 af_read_uint16_be(data_flow, &numChannels, fh);
 track->f.channelCount = numChannels;

 af_read_uint32_be(data_flow, &numSampleFrames, fh);
 track->totalfframes = numSampleFrames;

 af_read_uint16_be(data_flow, &sampleSize, fh);
 track->f.sampleWidth = sampleSize;

 af_fread(data_flow, sampleRate, 10, 1, fh);
 track->f.sampleRate = _af_convert_from_ieee_extended(data_flow, sampleRate);
 ...
 {
  uint8_t compressionID[4]={0};

  char compressionName[256]={0};

  af_fread(data_flow, compressionID, 4, 1, fh);


  af_read_pstring(data_flow, compressionName, fh);
  ...
  {
   _af_error(data_flow, AF_BAD_NOT_IMPLEMENTED, "AIFF-C format does not support Apple's proprietary %s compression format", compressionName);
   return (-1);
  }
  ...
  {
   _af_error(data_flow, AF_BAD_NOT_IMPLEMENTED, "AIFF-C compression type '%c%c%c%c' not currently supported",
    compressionID[0],
    compressionID[1],
    compressionID[2],
    compressionID[3]);
...
"!memcmp(&type, \"SSND\", 4)"
, "libaudiofile/aiff.c", 450, __PRETTY_FUNCTION__))
                                 ;

 track = _af_filehandle_get_track(data_flow, file, AF_DEFAULT_TRACK);

 af_read_uint32_be(data_flow, &offset, fh);
 af_read_uint32_be(data_flow, &blockSize, fh);
 ...
 track->fpos_first_frame = af_ftell(data_flow, fh) + offset;
...
status _af_aiff_read_init (int *data_flow, AFfilesetup setup, AFfilehandle file)
{
 uint32_t type, size, formtype={0};
 size_t index = 0;
 
_Bool 
      hasCOMM, hasFVER, hasSSND, hasMARK, hasINST={0};
 
_Bool 
      hasAESD, hasNAME, hasAUTH, hasCOPY={0};
 _Track *track={0};
...
"file != NULL"
, "libaudiofile/aiff.c", 497, __PRETTY_FUNCTION__))
                    ;
...
((void *)0)) ? (void) (0) : __assert_fail (
"file->fh != NULL"
, "libaudiofile/aiff.c", 498, __PRETTY_FUNCTION__))
                        ;

 af_fseek(data_flow, file->fh, 0, 
                      0
                              );

 af_fread(data_flow, &type, 4, 1, file->fh);
 af_read_uint32_be(data_flow, &size, file->fh);
 af_fread(data_flow, &formtype, 4, 1, file->fh);
 ...
 while (index < size)
 {
  ...
  af_fread(data_flow, &chunkid, 4, 1, file->fh);
  af_read_uint32_be(data_flow, &chunksize, file->fh);
  ...
  if (!memcmp("COMM", &chunkid, 4))
  {
   hasCOMM = 
            1
                ;
   result = ParseCOMM(data_flow, file, file->fh, chunkid, chunksize);
  }
  else if (!memcmp("FVER", &chunkid, 4))
  {
   hasFVER = 
            1
                ;
   ParseFVER(data_flow, file, file->fh, chunkid, chunksize);
  }
  else if (!memcmp("INST", &chunkid, 4))
  {
   hasINST = 
            1
                ;
   ParseINST(data_flow, file, file->fh, chunkid, chunksize);
  }
  else if (!memcmp("MARK", &chunkid, 4))
  {
   hasMARK = 
            1
                ;
   ParseMARK(data_flow, file, file->fh, chunkid, chunksize);
  }
  else if (!memcmp("AESD", &chunkid, 4))
  {
   hasAESD = 
            1
                ;
   ParseAESD(data_flow, file, file->fh, chunkid, chunksize);
  }
  ...
  {
   ParseMiscellaneous(data_flow, file, file->fh, chunkid, chunksize);
  }
  ...
  else if (!memcmp("SSND", &chunkid, 4))
  {
   if (hasSSND)
   {
    _af_error(data_flow, AF_BAD_AIFF_SSND, "AIFF file has more than one SSND chunk");
    return (-1);
   }
   hasSSND = 
            1
                ;
   result = ParseSSND(data_flow, file, file->fh, chunkid, chunksize);
  }
  ...
  af_fseek(data_flow, file->fh, index + 8, 
                               0
                                       );
 }
 ...
 if (!hasCOMM)
 {
  _af_error(data_flow, AF_BAD_AIFF_COMM, "bad AIFF COMM chunk");
 }
    ...
    _af_aiff_recognize (int *data_flow, AFvirtualfile *fh)
{
 ...
 if (af_fread(data_flow, buffer, 1, 8, fh) != 8 || memcmp(buffer, "FORM", 4) != 0)
  return 
        0
             ;
 if (af_fread(data_flow, buffer, 1, 4, fh) != 4 || memcmp(buffer, "AIFF", 4) != 0)
  return 
        0
             ;
    ...
    _af_aifc_recognize (int *data_flow, AFvirtualfile *fh)
{
 ...
 if (af_fread(data_flow, buffer, 1, 8, fh) != 8 || memcmp(buffer, "FORM", 4) != 0)
  return 
        0
             ;
 if (af_fread(data_flow, buffer, 1, 4, fh) != 4 || memcmp(buffer, "AIFC", 4) != 0)
  return 
        0
             ;
...
AFfilesetup _af_aiff_complete_setup (int *data_flow, AFfilesetup setup)
{
 _TrackSetup *track={0};

 
_Bool 
     isAIFF = setup->fileFormat == AF_FILE_AIFF;

 if (setup->trackSet && setup->trackCount != 1)
 {
  _af_error(data_flow, AF_BAD_NUMTRACKS, "AIFF/AIFF-C file must have 1 track");
 ...
 if (track->sampleFormatSet)
 {
  ...
  {
   _af_error(data_flow, AF_BAD_FILEFMT, "AIFF/AIFF-C format does not support unsigned data");
   return ((struct _AFfilesetup *) 0);
  }
  else if (isAIFF && track->f.sampleFormat != AF_SAMPFMT_TWOSCOMP)
  {
   _af_error(data_flow, AF_BAD_FILEFMT, "AIFF format supports only two's complement integer data");
   return ((struct _AFfilesetup *) 0);
  }
 }
 else
  _af_set_sample_format(data_flow, &track->f, AF_SAMPFMT_TWOSCOMP,
   track->f.sampleWidth);
 ...
 {
  _af_error(data_flow, AF_BAD_BYTEORDER,
 ...
 if (setup->instrumentSet)
 {
  ...
  {
   _af_error(data_flow, AF_BAD_NUMINSTS, "AIFF/AIFF-C file must have 0 or 1 instrument chunk");
   return ((struct _AFfilesetup *) 0);
  }
  if (setup->instruments != 0 &&
   setup->instruments[0].loopCount != 2)
  {
   _af_error(data_flow, AF_BAD_NUMLOOPS, "AIFF/AIFF-C file with instrument must also have 2 loops");
   return ((struct _AFfilesetup *) 0);
  }
 }
 ...
 if (setup->miscellaneousSet)
 {
  ...
  for (i=0; i<setup->miscellaneousCount; i++)
  {
   switch (setup->miscellaneous[i].type)
   {
    ...
    default:
     _af_error(data_flow, AF_BAD_MISCTYPE, "invalid miscellaneous type %d for AIFF/AIFF-C file", setup->miscellaneous[i].type);
     return ((struct _AFfilesetup *) 0);
   }
  }
 }

 return _af_filesetup_copy(data_flow, setup, &_af_aiff_default_filesetup, 
                                                              1
                                                                  );
}
...
_Bool 
    _af_aiff_instparam_valid (int *data_flow, AFfilehandle filehandle, AUpvlist list, int i)
{
...
int _af_aifc_get_version (int *data_flow, AFfilehandle file)
{
 return 0xa2805140;
}
