...
status _af_avr_update (int *data_flow, AFfilehandle file)
{
 _Track *track={0};
 uint32_t size, loopStart, loopEnd={0};

 track = _af_filehandle_get_track(data_flow, file, AF_DEFAULT_TRACK);


 af_fseek(data_flow, file->fh, 26, 
                       0
                               );
 ...
 af_write_uint32_be(data_flow, &size, file->fh);
 af_write_uint32_be(data_flow, &loopStart, file->fh);
 af_write_uint32_be(data_flow, &loopEnd, file->fh);

 return (0);
}

static char *af_basename (int *data_flow, char *filename)
{
 char *base={0};
...
status _af_avr_write_init (int *data_flow, AFfilesetup setup, AFfilehandle filehandle)
{
 _Track *track={0};
 char name[8]={0};
 ...
 if (af_fseek(data_flow, filehandle->fh, 0, 
                                0
                                        ) != 0)
 {
  _af_error(data_flow, AF_BAD_LSEEK, "bad seek");
  return (-1);
 }

 af_fwrite(data_flow, "2BIT", 4, 1, filehandle->fh);
 memset(name, 0, 8);
 if (filehandle->fileName != 
                            ((void *)0)
                                )
  strncpy(name, af_basename(data_flow, filehandle->fileName), 8);
 af_fwrite(data_flow, name, 8, 1, filehandle->fh);

 if (track->f.channelCount == 1)
  mono = 0x0;
 else
  mono = 0xffff;
 af_write_uint16_be(data_flow, &mono, filehandle->fh);

 resolution = track->f.sampleWidth;
 af_write_uint16_be(data_flow, &resolution, filehandle->fh);

 if (track->f.sampleFormat == AF_SAMPFMT_UNSIGNED)
  sign = 0x0;
 else
  sign = 0xffff;
 af_write_uint16_be(data_flow, &sign, filehandle->fh);


 loop = 0;
 af_write_uint16_be(data_flow, &loop, filehandle->fh);
 midi = 0xffff;
 af_write_uint16_be(data_flow, &midi, filehandle->fh);
 ...
 af_write_uint32_be(data_flow, &rate, filehandle->fh);
 af_write_uint32_be(data_flow, &size, filehandle->fh);
 af_write_uint32_be(data_flow, &loopStart, filehandle->fh);
 af_write_uint32_be(data_flow, &loopEnd, filehandle->fh);

 memset(reserved, 0, 26);
 af_fwrite(data_flow, reserved, 26, 1, filehandle->fh);

 memset(user, 0, 64);
 af_fwrite(data_flow, user, 64, 1, filehandle->fh);

 if (track->fpos_first_frame == 0)
  track->fpos_first_frame = af_ftell(data_flow, filehandle->fh);
