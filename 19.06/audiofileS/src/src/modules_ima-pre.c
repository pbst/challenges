...
static int ima_adpcm_decode_block (int *data_flow, ima_adpcm_data *ima, const uint8_t *encoded,
 int16_t *decoded)
{
 ...
 _af_adpcm_decoder(data_flow, encoded, decoded, ima->framesPerBlock - 1, channelCount, state);
...
static void ima_adpcm_decompress_describe (int *data_flow, _AFmoduleinst *i)
{
...
_Bool 
     seekok, 
             _Bool 
                  headerless, AFframecount *chunkframes)
{
 _AFmoduleinst ret = _AFnewmodinst(data_flow, &ima_adpcm_decompress);
 ima_adpcm_data *d;
 AUpvlist pv;
 long l;
...
, "libaudiofile/modules/ima.c", 132, __PRETTY_FUNCTION__))
                                               ;

 d = (ima_adpcm_data *) _af_malloc(data_flow, sizeof (ima_adpcm_data));
...
static void ima_adpcm_run_pull (int *data_flow, _AFmoduleinst *module)
{
...
"module->outc->nframes % framesPerBlock == 0"
, "libaudiofile/modules/ima.c", 167, __PRETTY_FUNCTION__))
                                                   ;
 int blockCount = module->outc->nframes / framesPerBlock;


 ssize_t blocksRead = af_fread(data_flow, module->inc->buf, d->blockAlign, blockCount, d->fh);
 ...
 if (blocksRead < 0)
 {
  ...
  {
   _af_error(data_flow, AF_BAD_READ, "file missing data");
 ...
 {
  ssize_t bytesDecoded = ima_adpcm_decode_block(data_flow, d,
...
, "libaudiofile/modules/ima.c", 201, __PRETTY_FUNCTION__))
                                                    ;
...
static void ima_adpcm_reset1 (int *data_flow, _AFmoduleinst *i)
{
...
static void ima_adpcm_reset2 (int *data_flow, _AFmoduleinst *i)
{
 ima_adpcm_data *d = (ima_adpcm_data *) i->modspec;
...
"d->track->nextfframe % framesPerBlock == 0"
, "libaudiofile/modules/ima.c", 248, __PRETTY_FUNCTION__))
                                                  ;
}
