...
const int _af_next_compression_types[2] =
{
...
status _af_next_read_init (int *data_flow, AFfilesetup setup, AFfilehandle file)
{
 uint32_t id, offset, length, encoding, sampleRate, channelCount={0};
 _Track *track={0};
...
"file != NULL"
, "libaudiofile/next.c", 72, __PRETTY_FUNCTION__))
                    ;
...
((void *)0)) ? (void) (0) : __assert_fail (
"file->fh != NULL"
, "libaudiofile/next.c", 73, __PRETTY_FUNCTION__))
                        ;
 ...
 af_fread(data_flow, &id, 4, 1, file->fh);
 
((
!memcmp(&id, ".snd", 4)
) ? (void) (0) : __assert_fail (
"!memcmp(&id, \".snd\", 4)"
, "libaudiofile/next.c", 89, __PRETTY_FUNCTION__))
                               ;

 af_read_uint32_be(data_flow, &offset, file->fh);
 af_read_uint32_be(data_flow, &length, file->fh);
 af_read_uint32_be(data_flow, &encoding, file->fh);
 af_read_uint32_be(data_flow, &sampleRate, file->fh);
 af_read_uint32_be(data_flow, &channelCount, file->fh);
 ...
 track->data_size = af_flength(data_flow, file->fh) - offset;
 ...
 switch (encoding)
 {
  ...
  default:
   ...
   _af_error(data_flow, AF_BAD_SAMPFMT, "bad sample format");
   return (-1);
 }
 ...
 int frameSize = _af_format_frame_size(data_flow, &track->f, 
                                                 0
                                                      );
    ...
    _af_next_recognize (int *data_flow, AFvirtualfile *fh)
{
 ...
 if (af_fread(data_flow, buffer, 1, 4, fh) != 4 || memcmp(buffer, ".snd", 4) != 0)
  return 
        0
             ;
...
AFfilesetup _af_next_complete_setup (int *data_flow, AFfilesetup setup)
{
 _TrackSetup *track={0};

 if (setup->trackSet && setup->trackCount != 1)
 {
  _af_error(data_flow, AF_BAD_NUMTRACKS, "NeXT files must have exactly 1 track");
 ...
 {
  _af_error(data_flow, AF_BAD_FILEFMT, "NeXT format does not support unsigned data");
 ...
 if (track->f.sampleFormat == AF_SAMPFMT_TWOSCOMP)
 {
  if (track->f.sampleWidth != 8 &&
   ...
   track->f.sampleWidth != 32)
  {
   _af_error(data_flow, AF_BAD_WIDTH, "invalid sample width %d for NeXT file (only 8-, 16-, 24-, and 32-bit data are allowed)");
   return ((struct _AFfilesetup *) 0);
  }
 }
 ...
 if (track->f.compressionType != AF_COMPRESSION_NONE &&
  ...
  track->f.compressionType != AF_COMPRESSION_G711_ALAW)
 {
  _af_error(data_flow, AF_BAD_NOT_IMPLEMENTED, "compression format not implemented for NeXT files");
  return ((struct _AFfilesetup *) 0);
 }

 if (track->f.byteOrder != AF_BYTEORDER_BIGENDIAN && track->byteOrderSet)
 {
  _af_error(data_flow, AF_BAD_BYTEORDER, "NeXT format supports only big-endian data");
  track->f.byteOrder = AF_BYTEORDER_BIGENDIAN;
 }

 if (track->aesDataSet)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "NeXT files cannot have AES data");
  return ((struct _AFfilesetup *) 0);
 }

 if (track->markersSet && track->markerCount != 0)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "NeXT format does not support markers");
  return ((struct _AFfilesetup *) 0);
 }

 if (setup->instrumentSet && setup->instrumentCount != 0)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "NeXT format does not support instruments");
  return ((struct _AFfilesetup *) 0);
 }

 if (setup->miscellaneousSet && setup->miscellaneousCount != 0)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "NeXT format does not support miscellaneous data");
  return ((struct _AFfilesetup *) 0);
 }

 return _af_filesetup_copy(data_flow, setup, &_af_next_default_filesetup, 
                                                              0
                                                                   );
}
