    ...
    _af_ircam_recognize (int *data_flow, AFvirtualfile *fh)
{
 ...
 if (af_fread(data_flow, buffer, 4, 1, fh) != 1)
  return 
        0
             ;
...
AFfilesetup _af_ircam_complete_setup (int *data_flow, AFfilesetup setup)
{
 _TrackSetup *track={0};

 if (setup->trackSet && setup->trackCount != 1)
 {
  _af_error(data_flow, AF_BAD_NUMTRACKS, "BICSF file must have 1 track");
 ...
 if (track->aesDataSet)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "BICSF file cannot have AES data");
  return ((struct _AFfilesetup *) 0);
 }

 if (track->markersSet && track->markerCount != 0)
 {
  _af_error(data_flow, AF_BAD_NUMMARKS, "BICSF format does not support markers");
  return ((struct _AFfilesetup *) 0);
 }

 if (setup->instrumentSet && setup->instrumentCount != 0)
 {
  _af_error(data_flow, AF_BAD_NUMINSTS, "BICSF format does not support instruments");
  return ((struct _AFfilesetup *) 0);
 }
 ...
 {
  _af_error(data_flow, AF_BAD_NOT_IMPLEMENTED, "BICSF format does not currently support miscellaneous chunks");
  return ((struct _AFfilesetup *) 0);
 }

 return _af_filesetup_copy(data_flow, setup, &_af_ircam_default_filesetup, 
                                                               1
                                                                   );
}

status _af_ircam_read_init (int *data_flow, AFfilesetup setup, AFfilehandle handle)
{
 _Track *track={0};
 uint8_t magic[4]={0};
 float rate={0};
...
_Bool 
 ...
 if (af_fread(data_flow, magic, 4, 1, handle->fh) != 1)
 {
  _af_error(data_flow, AF_BAD_READ, "Could not read BICSF file header");
  return (-1);
 }
 ...
 {
  _af_error(data_flow, AF_BAD_FILEFMT,
 ...
 af_fread(data_flow, &rate, 4, 1, handle->fh);
 af_fread(data_flow, &channels, 4, 1, handle->fh);
 af_fread(data_flow, &packMode, 4, 1, handle->fh);

 if (isSwapped)
 {
  rate = _af_byteswap_float32(data_flow, rate);
  channels = _af_byteswap_int32(data_flow, channels);
  packMode = _af_byteswap_int32(data_flow, packMode);
 }
 ...
 {
  _af_error(data_flow, AF_BAD_FILEFMT, "invalid channel count (%d) "
