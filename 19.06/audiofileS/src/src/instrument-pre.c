...
void afInitInstIDs (int *data_flow, AFfilesetup setup, int *instids, int ninsts)
{
 int i={0};
 ...
 if (!_af_unique_ids(data_flow, instids, ninsts, "instrument", AF_BAD_INSTID))
  return;
 ...
 setup->instruments = _af_instsetup_new(data_flow, setup->instrumentCount);
...
void _af_instparam_set (int *data_flow, AFfilehandle file, int instid, AUpvlist pvlist, int npv)
{
 int i, instno, j={0};
 ...
 if (!_af_filehandle_can_write(data_flow, file))
  return;

 if ((instno = _af_handle_instrument_index_from_id(data_flow, file, instid)) == -1)
  return;

 if (AUpvgetmaxitems(data_flow, pvlist) < npv)
 npv = AUpvgetmaxitems(data_flow, pvlist);
 ...
 for (i=0; i < npv; i++)
 {
  ...
  AUpvgetparam(data_flow, pvlist, i, &param);

  if ((j = _af_instparam_index_from_id(data_flow, file->fileFormat, param)) == -1)

   continue;

  if (_af_units[file->fileFormat].write.instparamvalid &&
   !_af_units[file->fileFormat].write.instparamvalid(data_flow, file, pvlist, i))
  ...
  {
   case AU_PVTYPE_LONG:
    AUpvgetval(data_flow, pvlist, i, &file->instruments[instno].values[j].l);
    break;
   case AU_PVTYPE_DOUBLE:
    AUpvgetval(data_flow, pvlist, i, &file->instruments[instno].values[j].d);
    break;
   case AU_PVTYPE_PTR:
    AUpvgetval(data_flow, pvlist, i, &file->instruments[instno].values[j].v);
    break;
   default:
    return;
  }
...
void afSetInstParams (int *data_flow, AFfilehandle file, int instid, AUpvlist pvlist, int npv)
{
 _af_instparam_set(data_flow, file, instid, pvlist, npv);
}

void afSetInstParamLong (int *data_flow, AFfilehandle file, int instid, int param, long value)
{
 AUpvlist pvlist = AUpvnew(data_flow, 1);

 AUpvsetparam(data_flow, pvlist, 0, param);
 AUpvsetvaltype(data_flow, pvlist, 0, AU_PVTYPE_LONG);
 AUpvsetval(data_flow, pvlist, 0, &value);

 _af_instparam_set(data_flow, file, instid, pvlist, 1);

 AUpvfree(data_flow, pvlist);
}
...
void _af_instparam_get (int *data_flow, AFfilehandle file, int instid, AUpvlist pvlist, int npv,
     ...
     forceLong)
{
 ...
 for (i=0; i < npv; i++)
 {
  ...
  {
   _af_error(data_flow, AF_BAD_INSTPTYPE, "type of instrument parameter %d is not AU_PVTYPE_LONG", param);
   continue;
  }

  AUpvsetvaltype(data_flow, pvlist, i, type);

  switch (type)
  {
   case AU_PVTYPE_LONG:
    AUpvsetval(data_flow, pvlist, i, &file->instruments[instno].values[j].l);
    break;
   case AU_PVTYPE_DOUBLE:
    AUpvsetval(data_flow, pvlist, i, &file->instruments[instno].values[j].d);
    break;
   case AU_PVTYPE_PTR:
    AUpvsetval(data_flow, pvlist, i, &file->instruments[instno].values[j].v);
    break;

   default:
    _af_error(data_flow, AF_BAD_INSTPTYPE, "invalid instrument parameter type %d", type);
    return;
  }
 }
}
...
void afGetInstParams (int *data_flow, AFfilehandle file, int inst, AUpvlist pvlist, int npv)
{
 _af_instparam_get(data_flow, file, inst, pvlist, npv, 
                                           0
                                                );
}

long afGetInstParamLong (int *data_flow, AFfilehandle file, int inst, int param)
{
 long val={0};
 AUpvlist pvlist = AUpvnew(data_flow, 1);
 ...
 _af_instparam_get(data_flow, file, inst, pvlist, 1, 
                                         1
                                             );

 AUpvgetval(data_flow, pvlist, 0, &val);
 AUpvfree(data_flow, pvlist);

 return(val);
}
int _af_instparam_index_from_id (int *data_flow, int filefmt, int id)
{
 int i={0};
 ...
 {
  _af_error(data_flow, AF_BAD_INSTPID, "invalid instrument parameter id %d",
   id);
  return -1;
 }
...
int _af_handle_instrument_index_from_id (int *data_flow, AFfilehandle file, int id)
{
 ...
 _af_error(data_flow, AF_BAD_INSTID, "invalid instrument id %d", id);
 return -1;
}
