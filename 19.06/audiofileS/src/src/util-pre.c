    ...
    _af_filesetup_ok (int *data_flow, AFfilesetup setup)
{
 ...
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "null file setup");
  return 
        0
             ;
 }
 ...
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "invalid file setup");
  return 
        0
             ;
 }
    ...
    _af_filehandle_can_read (int *data_flow, AFfilehandle file)
{
 ...
 {
  _af_error(data_flow, AF_BAD_NOREADACC, "file not opened for read access");
  return 
        0
             ;
 }
    ...
    _af_filehandle_can_write (int *data_flow, AFfilehandle file)
{
 ...
 {
  _af_error(data_flow, AF_BAD_NOWRITEACC, "file not opened for write access");
  return 
        0
             ;
 }
...
_Bool 
    _af_filehandle_ok (int *data_flow, AFfilehandle file)
{
 if (file == ((struct _AFfilehandle *) 0))
 {
  _af_error(data_flow, AF_BAD_FILEHANDLE, "null file handle");
  return 
        0
             ;
 }
 ...
 {
  _af_error(data_flow, AF_BAD_FILEHANDLE, "invalid file handle");
  return 
        0
             ;
 }
...
void *_af_malloc (int *data_flow, size_t size)
{
 ...
 if (size <= 0)
 {
  _af_error(data_flow, AF_BAD_MALLOC, "bad memory allocation size request %d", size);
  return 
        ((void *)0)
            ;
 }
 ...
 {
  _af_error(data_flow, AF_BAD_MALLOC, "allocation of %d bytes failed", size);
  return 
        ((void *)0)
            ;
 }
...
void *_af_realloc (int *data_flow, void *p, size_t size)
{
 if (size <= 0)
 {
  _af_error(data_flow, AF_BAD_MALLOC, "bad memory allocation size request %d", size);
...
void *_af_calloc (int *data_flow, size_t nmemb, size_t size)
{
 ...
 if (nmemb <= 0 || size <= 0)
 {
  _af_error(data_flow, AF_BAD_MALLOC, "bad memory allocation size request "
 ...
 {
  _af_error(data_flow, AF_BAD_MALLOC, "allocation of %d bytes failed",
   nmemb*size);
  return 
        ((void *)0)
            ;
...
AUpvlist _af_pv_long (int *data_flow, long val)
{
 ...
 AUpvsetparam(data_flow, ret, 0, 0);
 AUpvsetvaltype(data_flow, ret, 0, AU_PVTYPE_LONG);
 AUpvsetval(data_flow, ret, 0, &val);
 return ret;
}
...
AUpvlist _af_pv_double (int *data_flow, double val)
{
 AUpvlist ret = AUpvnew(data_flow, 1);
 AUpvsetparam(data_flow, ret, 0, 0);
 AUpvsetvaltype(data_flow, ret, 0, AU_PVTYPE_DOUBLE);
 AUpvsetval(data_flow, ret, 0, &val);
 return ret;
}
...
AUpvlist _af_pv_pointer (int *data_flow, void *val)
{
 AUpvlist ret = AUpvnew(data_flow, 1);
 AUpvsetparam(data_flow, ret, 0, 0);
 AUpvsetvaltype(data_flow, ret, 0, AU_PVTYPE_PTR);
 AUpvsetval(data_flow, ret, 0, &val);
 return ret;
}
    ...
    _af_pv_getlong (int *data_flow, AUpvlist pvlist, int param, long *l)
{
 ...
 for (i=0; i<AUpvgetmaxitems(data_flow, pvlist); i++)
 {
  int p, t={0};

  AUpvgetparam(data_flow, pvlist, i, &p);

  if (p != param)
   continue;

  AUpvgetvaltype(data_flow, pvlist, i, &t);
  ...
  AUpvgetval(data_flow, pvlist, i, l);
  return 
        1
            ;
 }
    ...
    _af_pv_getdouble (int *data_flow, AUpvlist pvlist, int param, double *d)
{
 ...
 for (i=0; i<AUpvgetmaxitems(data_flow, pvlist); i++)
 {
  ...
  AUpvgetval(data_flow, pvlist, i, d);
  return 
        1
            ;
 }
    ...
    _af_pv_getptr (int *data_flow, AUpvlist pvlist, int param, void **v)
{
 ...
 for (i=0; i<AUpvgetmaxitems(data_flow, pvlist); i++)
 {
  ...
  AUpvgetval(data_flow, pvlist, i, v);
  return 
        1
            ;
 }
...
_TrackSetup *_af_filesetup_get_tracksetup (int *data_flow, AFfilesetup setup, int trackid)
{
 ...
 _af_error(data_flow, AF_BAD_TRACKID, "bad track id %d", trackid);
...
float _af_format_sample_size (int *data_flow, _AudioFormat *fmt, 
                                                _Bool 
                                                     stretch3to4)
{
 ...
 return _af_format_sample_size_uncompressed(data_flow, fmt, stretch3to4) /
  squishFactor;
}

int _af_format_frame_size_uncompressed (int *data_flow, _AudioFormat *fmt, 
                                                          _Bool 
                                                               stretch3to4)
{
 return _af_format_sample_size_uncompressed(data_flow, fmt, stretch3to4) *
...
float _af_format_frame_size (int *data_flow, _AudioFormat *fmt, 
                                               _Bool 
                                                    stretch3to4)
{
 int compressionIndex={0};
...
status _af_set_sample_format (int *data_flow, _AudioFormat *f, int sampleFormat, int sampleWidth)
{
 switch (sampleFormat)
 {
  ...
  else
  {
   ...
   bytes = _af_format_sample_size_uncompressed(data_flow, f, 
                                                 0
                                                      );
  ...
  case AF_SAMPFMT_DOUBLE:
   ...
   break;
  default:
   _af_error(data_flow, AF_BAD_SAMPFMT, "unknown sample format %d",
    sampleFormat);
   return (-1);
 }
    ...
    _af_unique_ids (int *data_flow, int *ids, int nids, char *idname, int iderr)
{
 ...
 for (i = 0; i < nids; i++)
 {
  ...
  for (j = 0; j < i; j++)
   if (ids[i] == ids[j])
   {
    _af_error(data_flow, iderr, "nonunique %s id %d",
     idname, ids[i]);
    return 
          0
               ;
...
status af_read_uint32_be (int *data_flow, uint32_t *value, AFvirtualfile *vf)
{
 ...
 if (af_fread(data_flow, &v, sizeof (v), 1, vf) != 1)
  return (-1);
 *value = _af_byteswap_int32(data_flow, v);
 return (0);
}
...
status af_read_uint16_be (int *data_flow, uint16_t *value, AFvirtualfile *vf)
{
 ...
 if (af_fread(data_flow, &v, sizeof (v), 1, vf) != 1)
  return (-1);
 *value = _af_byteswap_int16(data_flow, v);
 return (0);
}
...
status af_read_uint8 (int *data_flow, uint8_t *value, AFvirtualfile *vf)
{
 if (af_fread(data_flow, value, 1, 1, vf) != 1)
  return (-1);
 return (0);
}
...
{
 uint32_t v={0};
 v = _af_byteswap_int32(data_flow, *value);
 if (af_fwrite(data_flow, &v, sizeof (v), 1, vf) != 1)
  return (-1);
 return (0);
}
...
{
 uint16_t v={0};
 v = _af_byteswap_int16(data_flow, *value);
 if (af_fwrite(data_flow, &v, sizeof (v), 1, vf) != 1)
  return (-1);
 return (0);
}
...
status af_write_uint8 (int *data_flow, const uint8_t *value, AFvirtualfile *vf)
{
 if (af_fwrite(data_flow, value, 1, 1, vf) != 1)
  return (-1);
 return (0);
}

status af_read_pstring (int *data_flow, char s[256], AFvirtualfile *vf)
{
 uint8_t length={0};

 af_read_uint8(data_flow, &length, vf);
 af_fread(data_flow, s, length, 1, vf);
 s[length] = '\0';
 return (0);
}

status af_write_pstring (int *data_flow, const char *s, AFvirtualfile *vf)
{
 size_t length = strlen(s);
 if (length > 255)
  return (-1);
 uint8_t sizeByte = (uint8_t) length;
 af_write_uint8(data_flow, &sizeByte, vf);
 af_fwrite(data_flow, s, sizeByte, 1, vf);
 ...
 {
  uint8_t zero = 0;
  af_write_uint8(data_flow, &zero, vf);
 }
 return (0);
}
