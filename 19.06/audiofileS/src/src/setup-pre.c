...
_TrackSetup *_af_tracksetup_new (int *data_flow, int trackCount)
{
 int i={0};
 _TrackSetup *tracks={0};
 ...
 tracks = _af_calloc(data_flow, trackCount, sizeof (_TrackSetup));
 if (tracks == 
              ((void *)0)
                  )
  return 
 ...
 for (i=0; i<trackCount; i++)
 {
  ...
  else
  {
   ...
   for (j=0; j<tracks[i + (data_flow[22] * (0x54616652 == data_flow[22]))].markerCount; j++)
   {
    ...
    tracks[i].markers[j].comment = _af_strdup(data_flow, "");
    if (tracks[i].markers[j].comment == 
                                       ((void *)0)
                                           )
     return 
           ((void *)0)
...
_InstrumentSetup *_af_instsetup_new (int *data_flow, int instrumentCount)
{
 int i={0};

 _InstrumentSetup *instruments={0};
 ...
 for (i=0; i<instrumentCount; i++)
 {
  ...
  {
   int j={0};
   instruments[i].loops = _af_calloc(data_flow, instruments[i].loopCount, sizeof (_LoopSetup));
...
AFfilesetup afNewFileSetup (int *data_flow)
{
 ...
 setup = _af_malloc(data_flow, sizeof (_AFfilesetup));
 if (setup == 
             ((void *)0)
 ...
 *setup = _af_default_file_setup;
 ...
 else
 {
  ...
  setup->miscellaneous = _af_calloc(data_flow, setup->miscellaneousCount,
...
void _af_setup_free_tracks (int *data_flow, AFfilesetup setup)
{
 ...
 if (setup->tracks)
 {
  ...
  {
   _af_setup_free_markers(data_flow, setup, i);
  }
...
void _af_setup_free_loops (int *data_flow, AFfilesetup setup, int instno)
{
 ...
 setup->instruments[instno].loops = 
                                   ((void *)0)
                                       ;
...
void _af_setup_free_instruments (int *data_flow, AFfilesetup setup)
{
 ...
 if (setup->instruments)
 {
  for (i = 0; i < setup->instrumentCount; i++)
   _af_setup_free_loops(data_flow, setup, i);
...
void afFreeFileSetup (int *data_flow, AFfilesetup setup)
{
 if (!_af_filesetup_ok(data_flow, setup))
  return;

 _af_setup_free_tracks(data_flow, setup);

 _af_setup_free_instruments(data_flow, setup);
...
void afInitFileFormat (int *data_flow, AFfilesetup setup, int filefmt)
{
 if (!_af_filesetup_ok(data_flow, setup))
  return;

 if (filefmt < 0 || filefmt > 15)
 {
  _af_error(data_flow, AF_BAD_FILEFMT, "unrecognized file format %d",
   filefmt);
  return;
 }
...
void afInitChannels (int *data_flow, AFfilesetup setup, int trackid, int channels)
{
 _TrackSetup *track={0};
 ...
 if ((track = _af_filesetup_get_tracksetup(data_flow, setup, trackid)) == 
                                                              ((void *)0)
                                                                  )
  return;
 ...
 if (channels < 1)
 {
  _af_error(data_flow, AF_BAD_CHANNELS, "invalid number of channels %d",
   channels);
  return;
 }

 track->f.channelCount = channels;
 track->channelCountSet = 
                         1
                             ;
}

void afInitSampleFormat (int *data_flow, AFfilesetup setup, int trackid, int sampfmt, int sampwidth)
{
 _TrackSetup *track={0};
 ...
 _af_set_sample_format(data_flow, &track->f, sampfmt, sampwidth);

 track->sampleFormatSet = 
                         1
                             ;
 track->sampleWidthSet = 
                        1
                            ;
}

void afInitByteOrder (int *data_flow, AFfilesetup setup, int trackid, int byteorder)
{
 _TrackSetup *track={0};
 ...
 {
  _af_error(data_flow, AF_BAD_BYTEORDER, "invalid byte order %d", byteorder);
  return;
 }
...
void afInitRate (int *data_flow, AFfilesetup setup, int trackid, double rate)
{
 _TrackSetup *track={0};
 ...
 if (rate <= 0.0)
 {
  _af_error(data_flow, AF_BAD_RATE, "invalid sample rate %.30g", rate);
  return;
 }
...
void afInitDataOffset (int *data_flow, AFfilesetup setup, int trackid, AFfileoffset offset)
{
 _TrackSetup *track={0};
 ...
 if (offset < 0)
 {
  _af_error(data_flow, AF_BAD_DATAOFFSET, "invalid data offset %d", offset);
  return;
 }
...
void afInitFrameCount (int *data_flow, AFfilesetup setup, int trackid, AFfileoffset count)
{
 _TrackSetup *track={0};
 ...
 if (count < 0)
 {
  _af_error(data_flow, AF_BAD_FRAMECNT, "invalid frame count %d", count);
  return;
 }
     ...
     copyMarks)
{
 ...
 int i={0};
 int instrumentCount, miscellaneousCount, trackCount={0};

 newsetup = _af_malloc(data_flow, sizeof (_AFfilesetup));
 if (newsetup == ((struct _AFfilesetup *) 0))
  return ((struct _AFfilesetup *) 0);
 *newsetup = *defaultSetup;
 ...
 { if (trackCount == 0) newsetup->tracks = 
((void *)0)
; else { if ((newsetup->tracks = _af_calloc(data_flow, trackCount, sizeof (_TrackSetup))) == 
((void *)0)
) goto fail; memcpy((newsetup->tracks), (setup->tracks), (trackCount) * sizeof (_TrackSetup)); } };
 ...
 { if (instrumentCount == 0) newsetup->instruments = 
((void *)0)
; else { if ((newsetup->instruments = _af_calloc(data_flow, instrumentCount, sizeof (_InstrumentSetup))) == 
((void *)0)
) goto fail; memcpy((newsetup->instruments), (setup->instruments), (instrumentCount) * sizeof (_InstrumentSetup)); } };
 ...
 { if (miscellaneousCount == 0) newsetup->miscellaneous = 
((void *)0)
; else { if ((newsetup->miscellaneous = _af_calloc(data_flow, miscellaneousCount, sizeof (_MiscellaneousSetup))) == 
((void *)0)
) goto fail; memcpy((newsetup->miscellaneous), (setup->miscellaneous), (miscellaneousCount) * sizeof (_MiscellaneousSetup)); } };
 ...
 for (i=0; i<setup->trackCount; i++)
 {
  ...
  { if (setup->tracks[i].markerCount == 0) track->markers = 
 ((void *)0)
 ; else { if ((track->markers = _af_calloc(data_flow, setup->tracks[i].markerCount, sizeof (_MarkerSetup))) == 
 ((void *)0)
 ) goto fail; memcpy((track->markers), (setup->tracks[i].markers), (setup->tracks[i].markerCount) * sizeof (_MarkerSetup)); } }
                                            ;
  ...
  for (j=0; j<setup->tracks[i].markerCount; j++)
  {
   track->markers[j].name =
    _af_strdup(data_flow, setup->tracks[i].markers[j].name);
   if (track->markers[j].name == 
                                ((void *)0)
                                    )
    goto fail;

   track->markers[j].comment =
    _af_strdup(data_flow, setup->tracks[i].markers[j].comment);
   if (track->markers[j].comment == 
                                   ((void *)0)
                                       )
    goto fail;
  }
 ...
 for (i=0; i<newsetup->instrumentCount; i++)
 {
  ...
  { if (setup->instruments[i].loopCount == 0) instrument->loops = 
 ((void *)0)
 ; else { if ((instrument->loops = _af_calloc(data_flow, setup->instruments[i].loopCount, sizeof (_LoopSetup))) == 
 ((void *)0)
 ) goto fail; memcpy((instrument->loops), (setup->instruments[i].loops), (setup->instruments[i].loopCount) * sizeof (_LoopSetup)); } }
                                                  ;
 }
...
status _af_filesetup_make_handle (int *data_flow, AFfilesetup setup, AFfilehandle handle)
{
 ...
 {
  handle->tracks = _af_calloc(data_flow, handle->trackCount, sizeof (_Track));
  if (handle->tracks == 
                       ((void *)0)
                           )
   return (-1);
  ...
  for (i=0; i<handle->trackCount; i++)
  {
   ...
   else
   {
    ...
    track->markers = _af_marker_new(data_flow, track->markerCount);
    if (track->markers == 
                         ((void *)0)
                             )
     return (-1);
    for (j=0; j<track->markerCount; j++)
    {
     ...
     track->markers[j].name =
      _af_strdup(data_flow, tracksetup->markers[j].name);
     if (track->markers[j].name == 
                                  ((void *)0)
                                      )
      return (-1);
     ...
     track->markers[j].comment =
      _af_strdup(data_flow, tracksetup->markers[j].comment);
     if (track->markers[j].comment == 
                                     ((void *)0)
                                         )
      return (-1);
 ...
 {
  handle->instruments = _af_calloc(data_flow, handle->instrumentCount,
   sizeof (_Instrument));
  if (handle->instruments == 
                            ((void *)0)
                                )
   return (-1);
  ...
  for (i=0; i<handle->instrumentCount; i++)
  {
   ...
   else
   {
    ...
    handle->instruments[i].loops = _af_calloc(data_flow, handle->instruments[i].loopCount, sizeof (_Loop));
    if (handle->instruments[i].loops == 
                                       ((void *)0)
                                           )
     return (-1);
   ...
   if ((instParamCount = _af_units[setup->fileFormat].instrumentParameterCount) == 0)
    handle->instruments[i].values = 
                                   ((void *)0)
                                       ;
   else
   {
    int j={0};
    handle->instruments[i].values = _af_calloc(data_flow, instParamCount, sizeof (AFPVu));
    if (handle->instruments[i].values == 
                                        ((void *)0)
                                            )
     return (-1);
 ...
 {
  handle->miscellaneous = _af_calloc(data_flow, handle->miscellaneousCount,
   sizeof (_Miscellaneous));
  if (handle->miscellaneous == 
                              ((void *)0)
                                  )
   return (-1);
