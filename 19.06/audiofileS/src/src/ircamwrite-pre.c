...
status _af_ircam_write_init (int *data_flow, AFfilesetup setup, AFfilehandle handle)
{
 _Track *track={0};
...
((
handle->fileFormat == AF_FILE_IRCAM
) ? (void) (0) : __assert_fail (
"handle->fileFormat == AF_FILE_IRCAM"
, "libaudiofile/ircamwrite.c", 69, __PRETTY_FUNCTION__))
                                           ;
...
"track->f.compressionType == AF_COMPRESSION_NONE"
, "libaudiofile/ircamwrite.c", 94, __PRETTY_FUNCTION__))
                                                       ;
 ...
 "track->f.sampleWidth == 16"
 , "libaudiofile/ircamwrite.c", 98, __PRETTY_FUNCTION__))
                                   ;
  packMode = 2;
 }
 ...
 "track->f.sampleWidth == 32"
 , "libaudiofile/ircamwrite.c", 103, __PRETTY_FUNCTION__))
                                   ;
  packMode = 4;
 }
 ...
 af_fwrite(data_flow, magic, 4, 1, handle->fh);
 af_fwrite(data_flow, &rate, 4, 1, handle->fh);
 af_fwrite(data_flow, &channels, 4, 1, handle->fh);
 af_fwrite(data_flow, &packMode, 4, 1, handle->fh);


 memset(zeros, 0, 1024);
 af_fwrite(data_flow, zeros, 1024 - 4*4, 1, handle->fh);

 return (0);
}

status _af_ircam_update (int *data_flow, AFfilehandle file)
{
 return (0);
}
