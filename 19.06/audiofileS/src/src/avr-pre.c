    ...
    _af_avr_recognize (int *data_flow, AFvirtualfile *fh)
{
 ...
 if (af_fread(data_flow, &magic, 4, 1, fh) != 1 || memcmp(&magic, "2BIT", 4) != 0)
  return 
        0
             ;
...
status _af_avr_read_init (int *data_flow, AFfilesetup setup, AFfilehandle file)
{
 uint32_t magic={0};
 char name[8]={0};
 uint16_t mono, resolution, sign, loop, midi={0};
 uint32_t rate, size, loopStart, loopEnd={0};
 char reserved[26]={0};
 char user[64]={0};
...
"file != NULL"
, "libaudiofile/avr.c", 84, __PRETTY_FUNCTION__))
                    ;
...
((void *)0)) ? (void) (0) : __assert_fail (
"file->fh != NULL"
, "libaudiofile/avr.c", 85, __PRETTY_FUNCTION__))
                        ;
 ...
 if (af_fread(data_flow, &magic, 4, 1, file->fh) != 1)
 {
  _af_error(data_flow, AF_BAD_READ, "could not read AVR file header");
  return (-1);
 }

 if (memcmp(&magic, "2BIT", 4) != 0)
 {
  _af_error(data_flow, AF_BAD_FILEFMT, "file is not AVR format");
  return (-1);
 }


 af_fread(data_flow, name, 8, 1, file->fh);

 af_read_uint16_be(data_flow, &mono, file->fh);
 af_read_uint16_be(data_flow, &resolution, file->fh);
 af_read_uint16_be(data_flow, &sign, file->fh);
 af_read_uint16_be(data_flow, &loop, file->fh);
 af_read_uint16_be(data_flow, &midi, file->fh);

 af_read_uint32_be(data_flow, &rate, file->fh);
 af_read_uint32_be(data_flow, &size, file->fh);
 af_read_uint32_be(data_flow, &loopStart, file->fh);
 af_read_uint32_be(data_flow, &loopEnd, file->fh);

 af_fread(data_flow, reserved, 26, 1, file->fh);
 af_fread(data_flow, user, 64, 1, file->fh);

 if ((track = _af_track_new(data_flow)) == 
                                 ((void *)0)
                                     )
  return (-1);
 ...
 {
  _af_error(data_flow, AF_BAD_SAMPFMT, "bad sample format in AVR file");
  return (-1);
 }

 if (resolution != 8 && resolution != 16)
 {
  _af_error(data_flow, AF_BAD_WIDTH, "bad sample width %d in AVR file",
   resolution);
  return (-1);
 }
...
AFfilesetup _af_avr_complete_setup (int *data_flow, AFfilesetup setup)
{
 _TrackSetup *track={0};

 if (setup->trackSet && setup->trackCount != 1)
 {
  _af_error(data_flow, AF_BAD_NUMTRACKS, "AVR files must have exactly 1 track");
 ...
 if (track->f.sampleFormat != AF_SAMPFMT_UNSIGNED &&
  track->f.sampleFormat != AF_SAMPFMT_TWOSCOMP)
 {
  _af_error(data_flow, AF_BAD_FILEFMT, "AVR format does supports only unsigned and two's complement integer data");
  return ((struct _AFfilesetup *) 0);
 }
 ...
 {
  _af_error(data_flow, AF_BAD_WIDTH, "invalid sample width %d for AVR file (only 8- and 16-bit sample widths are allowed)");
  return ((struct _AFfilesetup *) 0);
 }
 ...
 {
  _af_error(data_flow, AF_BAD_NOT_IMPLEMENTED, "compression not supported for AVR files");
  return ((struct _AFfilesetup *) 0);
 }
 ...
 if (track->aesDataSet)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "AVR files do not support AES data");
  return ((struct _AFfilesetup *) 0);
 }

 if (track->markersSet && track->markerCount != 0)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "AVR format does not support markers");
  return ((struct _AFfilesetup *) 0);
 }

 if (setup->instrumentSet && setup->instrumentCount != 0)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "AVR format does not support instruments");
  return ((struct _AFfilesetup *) 0);
 }

 if (setup->miscellaneousSet && setup->miscellaneousCount != 0)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "AVR format does not support miscellaneous data");
  return ((struct _AFfilesetup *) 0);
 }

 return _af_filesetup_copy(data_flow, setup, &_af_avr_default_filesetup, 
                                                             0
                                                                  );
}
