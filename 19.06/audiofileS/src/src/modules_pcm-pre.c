...
"!isnan(f->pcm.slope)"
, "libaudiofile/modules/pcm.c", 49, __PRETTY_FUNCTION__))
                            ;
...
, "libaudiofile/modules/pcm.c", 50, __PRETTY_FUNCTION__))
                                ;
...
"!isnan(f->pcm.minClip)"
, "libaudiofile/modules/pcm.c", 51, __PRETTY_FUNCTION__))
                              ;
...
"!isnan(f->pcm.maxClip)"
, "libaudiofile/modules/pcm.c", 52, __PRETTY_FUNCTION__))
                              ;
...
{
 _AFmoduleinst ret = _AFnewmodinst(data_flow, &pcm);
 pcm_data *d;
...
static void pcmrun_push (int *data_flow, _AFmoduleinst *i)
{
 pcm_data *d = (pcm_data *) i->modspec;
 AFframecount frames2write = i->inc->nframes;
 AFframecount n;
 n = af_fwrite(data_flow, i->inc->buf, d->bytes_per_frame, frames2write, d->fh);
...
, "libaudiofile/modules/pcm.c", 143, __PRETTY_FUNCTION__))
                                                                  ;
}

static void pcmsync1 (int *data_flow, _AFmoduleinst *i)
{
 pcm_data *d = (pcm_data *)i->modspec;
...
static void pcmsync2 (int *data_flow, _AFmoduleinst *i)
{
 pcm_data *d = (pcm_data *)i->modspec;
...
"!d->seekok || (af_ftell(d->fh) == d->trk->fpos_next_frame)"
, "libaudiofile/modules/pcm.c", 159, __PRETTY_FUNCTION__))
                                                                  ;
...
static void pcmrun_pull (int *data_flow, _AFmoduleinst *i)
{
 pcm_data *d = (pcm_data *) i->modspec;
 ...
 n = af_fread(data_flow, i->outc->buf, d->bytes_per_frame, frames2read, d->fh);
...
"!d->seekok || (af_ftell(d->fh) == d->trk->fpos_next_frame)"
, "libaudiofile/modules/pcm.c", 222, __PRETTY_FUNCTION__))
                                                                  ;
...
static void pcmreset1 (int *data_flow, _AFmoduleinst *i)
{
...
static void pcmreset2 (int *data_flow, _AFmoduleinst *i)
{
 pcm_data *d = (pcm_data *) i->modspec;
