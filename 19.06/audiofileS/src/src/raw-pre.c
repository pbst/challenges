...
const int _af_raw_compression_types[2] =
{
...
status _af_raw_read_init (int *data_flow, AFfilesetup filesetup, AFfilehandle filehandle)
{
 ...
 {
  _af_error(data_flow, AF_BAD_FILEHANDLE, "a valid AFfilesetup is required for reading raw data");
  return (-1);
 }

 if (_af_filesetup_make_handle(data_flow, filesetup, filehandle) == (-1))
  return (-1);
 ...
 {
  AFfileoffset filesize={0};
  filesize = af_flength(data_flow, filehandle->fh);
  if (filesize == -1)
  ...
  else
  {
   ...
   {
    _af_error(data_flow, AF_BAD_FILESETUP, "data offset is larger than file size");
    return (-1);
   }

   filesize -= track->fpos_first_frame;
   track->totalfframes = filesize / (int) _af_format_frame_size(data_flow, &track->f, 
                                                                          0
                                                                               );
  }
...
AFfilesetup _af_raw_complete_setup (int *data_flow, AFfilesetup setup)
{
 ...
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "raw file must have exactly one track");
  return ((struct _AFfilesetup *) 0);
 }

 if ((track = _af_filesetup_get_tracksetup(data_flow, setup, AF_DEFAULT_TRACK)) == 
                                                                       ((void *)0)
                                                                           )
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "could not access track in file setup");
  return ((struct _AFfilesetup *) 0);
 }

 if (track->aesDataSet)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "raw file cannot have AES data");
  return ((struct _AFfilesetup *) 0);
 }

 if (track->markersSet && track->markerCount != 0)
 {
  _af_error(data_flow, AF_BAD_NUMMARKS, "raw file cannot have markers");
  return ((struct _AFfilesetup *) 0);
 }

 if (setup->instrumentSet && setup->instrumentCount != 0)
 {
  _af_error(data_flow, AF_BAD_NUMINSTS, "raw file cannot have instruments");
  return ((struct _AFfilesetup *) 0);
 }

 if (setup->miscellaneousSet && setup->miscellaneousCount != 0)
 {
  _af_error(data_flow, AF_BAD_NUMMISC, "raw file cannot have miscellaneous data");
  return ((struct _AFfilesetup *) 0);
 }
 ...
 *newSetup = _af_raw_default_filesetup;
 ...
 newSetup->tracks = _af_malloc(data_flow, sizeof (_TrackSetup));
 newSetup->tracks[0] = setup->tracks[0];
 newSetup->tracks[0].f.compressionParams = 
                                          ((void *)0)
                                              ;

 newSetup->tracks[0].markerCount = 0;
 newSetup->tracks[0].markers = 
                              ((void *)0)
                                  ;
