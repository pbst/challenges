...
static _Miscellaneous *find_misc_by_id (int *data_flow, AFfilehandle file, int id)
{
 int i={0};
 ...
 _af_error(data_flow, AF_BAD_MISCID, "bad miscellaneous id %d", id);
...
static _MiscellaneousSetup *find_miscsetup_by_id (int *data_flow, AFfilesetup setup, int id)
{
 int i={0};
...
void afInitMiscIDs (int *data_flow, AFfilesetup setup, int *ids, int nids)
{
 int i={0};
 ...
 {
  setup->miscellaneous = _af_calloc(data_flow, nids,
 ...
 setup->miscellaneousSet = 
                          1
                              ;
}

int afGetMiscIDs (int *data_flow, AFfilehandle file, int *ids)
{
 int i={0};
...
void afInitMiscType (int *data_flow, AFfilesetup setup, int miscellaneousid, int type)
{
 _MiscellaneousSetup *miscellaneous={0};

 if (!_af_filesetup_ok(data_flow, setup))
  return;

 miscellaneous = find_miscsetup_by_id(data_flow, setup, miscellaneousid);

 if (miscellaneous)
  miscellaneous->type = type;
 else
  _af_error(data_flow, AF_BAD_MISCID, "bad miscellaneous id");
}

int afGetMiscType (int *data_flow, AFfilehandle file, int miscellaneousid)
{
 _Miscellaneous *miscellaneous={0};

 if (!_af_filehandle_ok(data_flow, file))
  return -1;

 miscellaneous = find_misc_by_id(data_flow, file, miscellaneousid);
...
void afInitMiscSize (int *data_flow, AFfilesetup setup, int miscellaneousid, int size)
{
 _MiscellaneousSetup *miscellaneous={0};
...
int afGetMiscSize (int *data_flow, AFfilehandle file, int miscellaneousid)
{
 _Miscellaneous *miscellaneous={0};
...
int afWriteMisc (int *data_flow, AFfilehandle file, int miscellaneousid, void *buf, int bytes)
{
 _Miscellaneous *miscellaneous={0};
 int localsize={0};
 ...
 if ((miscellaneous = find_misc_by_id(data_flow, file, miscellaneousid)) == 
                                                                ((void *)0)
                                                                    )
  return -1;
 ...
 if (bytes <= 0)
 {
  _af_error(data_flow, AF_BAD_MISCSIZE, "invalid size (%d) for miscellaneous chunk", bytes);
 }

 if (miscellaneous->buffer == 
                             ((void *)0) 
                                  && miscellaneous->size != 0)
 {
  miscellaneous->buffer = _af_malloc(data_flow, miscellaneous->size);
...
int afReadMisc (int *data_flow, AFfilehandle file, int miscellaneousid, void *buf, int bytes)
{
 int localsize={0};
 _Miscellaneous *miscellaneous={0};
...
int afSeekMisc (int *data_flow, AFfilehandle file, int miscellaneousid, int offset)
{
 _Miscellaneous *miscellaneous={0};
 ...
 {
  _af_error(data_flow, AF_BAD_MISCSEEK,
