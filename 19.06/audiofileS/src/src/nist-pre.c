    ...
    _af_nist_recognize (int *data_flow, AFvirtualfile *fh)
{
 ...
 if (af_fread(data_flow, buffer, 16, 1, fh) != 1)
  return 
        0
             ;
...
AFfilesetup _af_nist_complete_setup (int *data_flow, AFfilesetup setup)
{
 ...
 {
  _af_error(data_flow, AF_BAD_NUMTRACKS, "NIST SPHERE file must have 1 track");
 ...
 {
  _af_error(data_flow, AF_BAD_RATE,
 ...
 if (track->aesDataSet)
 {
  _af_error(data_flow, AF_BAD_FILESETUP, "NIST SPHERE file cannot have AES data");
  return ((struct _AFfilesetup *) 0);
 }

 if (track->markersSet && track->markerCount != 0)
 {
  _af_error(data_flow, AF_BAD_NUMMARKS, "NIST SPHERE format does not support markers");
  return ((struct _AFfilesetup *) 0);
 }

 if (setup->instrumentSet && setup->instrumentCount != 0)
 {
  _af_error(data_flow, AF_BAD_NUMINSTS, "NIST SPHERE format does not support instruments");
  return ((struct _AFfilesetup *) 0);
 }
 ...
 {
  _af_error(data_flow, AF_BAD_NOT_IMPLEMENTED, "NIST SPHERE format does not currently support miscellaneous chunks");
  return ((struct _AFfilesetup *) 0);
 }

 return _af_filesetup_copy(data_flow, setup, &_af_nist_default_filesetup, 
                                                              1
                                                                  );
}
...
static 
      _Bool 
           nist_header_read_int (int *data_flow, char *header, char *key, int *val)
{
 char *cp={0};
 char keystring[256], scanstring[256]={0};

 snprintf(keystring, 256, "\n%s -i", key);

 if ((cp = strstr(header, keystring)) != 
                                        ((void *)0)
                                            )
 {
...
static 
      _Bool 
           nist_header_read_string (int *data_flow, char *header, char *key, int *length, char *val)
{
 char *cp={0};
 char keystring[256], scanstring[256]={0};
...
{
 _Track *track={0};
 char header[1024 + 1]={0};
 int intval={0};
 char strval[80]={0};
 int sample_n_bytes={0};
 ...
 af_fseek(data_flow, handle->fh, 0, 
                        0
                                );

 if (af_fread(data_flow, header, 1024, 1, handle->fh) != 1)
 {
  _af_error(data_flow, AF_BAD_READ, "Could not read NIST SPHERE file header");
  return (-1);
 }
 ...
 {
  _af_error(data_flow, AF_BAD_FILEFMT, "Bad NIST SPHERE file header");
  return (-1);
 }

 if ((handle->tracks = _af_track_new(data_flow)) == 
                                          ((void *)0)
                                              )
  return (-1);
 ...
 if (!nist_header_read_int(data_flow, header, "sample_n_bytes", &sample_n_bytes))
 {
  _af_error(data_flow, AF_BAD_HEADER, "bytes per sample not specified");
  return (-1);
 }
 ...
 if (nist_header_read_string(data_flow, header, "sample_coding", &intval, strval))
 {
 ...
 if (nist_header_read_int(data_flow, header, "channel_count", &intval))
 {
  if (intval < 1)
  {
 ...
 {
  _af_error(data_flow, AF_BAD_HEADER, "number of channels not specified");
  return (-1);
 }


 if (nist_header_read_string(data_flow, header, "sample_byte_format", &intval, strval))
 {
  if (intval > 1)
  {
 ...
 else
 {
  ...
  {
   _af_error(data_flow, AF_BAD_HEADER, "sample byte order not specified");
   return (-1);
  }
 }


 if (nist_header_read_int(data_flow, header, "sample_sig_bits", &intval))
 {
  if (intval < 1 || intval > 32)
  {
   _af_error(data_flow, AF_BAD_WIDTH, "invalid sample width %d bits\n",
    intval);
   return (-1);
  }
 ...
 if (nist_header_read_int(data_flow, header, "sample_rate", &intval))
 {
  if (intval <= 0)
  {
   _af_error(data_flow, AF_BAD_RATE, "invalid sample rate %d Hz\n", intval);
   return (-1);
  }
 ...
 {
  _af_error(data_flow, AF_BAD_HEADER, "sample rate not specified");
  return (-1);
 }


 if (nist_header_read_int(data_flow, header, "sample_count", &intval))
 {
  track->totalfframes = intval / track->f.channelCount;
 }
 else
 {
  _af_error(data_flow, AF_BAD_HEADER, "number of samples not specified");
  return (-1);
 }

 if (_af_set_sample_format(data_flow, &track->f, track->f.sampleFormat,
  track->f.sampleWidth) == (-1))
 {
  return (-1);
 }

 track->fpos_first_frame = 1024;
 track->data_size = af_flength(data_flow, handle->fh) - 1024;
 track->nextfframe = 0;
 track->fpos_next_frame = track->fpos_first_frame;

 handle->formatSpecific = 
                         ((void *)0)
                             ;
