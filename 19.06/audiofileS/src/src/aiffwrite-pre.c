...
status _af_aiff_update (int *data_flow, AFfilehandle file);

static status WriteCOMM (int *data_flow, AFfilehandle file);
static status WriteSSND (int *data_flow, AFfilehandle file);
static status WriteMARK (int *data_flow, AFfilehandle file);
static status WriteINST (int *data_flow, AFfilehandle file);
static status WriteFVER (int *data_flow, AFfilehandle file);
static status WriteAESD (int *data_flow, AFfilehandle file);
static status WriteMiscellaneous (int *data_flow, AFfilehandle file);

static _AIFFInfo *aiffinfo_new (int *data_flow)
{
 _AIFFInfo *aiff = _af_malloc(data_flow, sizeof (_AIFFInfo));
...
status _af_aiff_write_init (int *data_flow, AFfilesetup setup, AFfilehandle file)
{
 uint32_t fileSize = 0;
...
"file"
, "libaudiofile/aiffwrite.c", 75, __PRETTY_FUNCTION__))
            ;
 
((
file->fileFormat == AF_FILE_AIFF || file->fileFormat == AF_FILE_AIFFC
) ? (void) (0) : __assert_fail (
"file->fileFormat == AF_FILE_AIFF || file->fileFormat == AF_FILE_AIFFC"
,
 ...
 if (_af_filesetup_make_handle(data_flow, setup, file) == (-1))
  return (-1);

 file->formatSpecific = aiffinfo_new(data_flow);

 af_fwrite(data_flow, "FORM", 4, 1, file->fh);
 af_write_uint32_be(data_flow, &fileSize, file->fh);

 if (file->fileFormat == AF_FILE_AIFF)
  af_fwrite(data_flow, "AIFF", 4, 1, file->fh);
 else if (file->fileFormat == AF_FILE_AIFFC)
  af_fwrite(data_flow, "AIFC", 4, 1, file->fh);

 if (file->fileFormat == AF_FILE_AIFFC)
  WriteFVER(data_flow, file);

 WriteCOMM(data_flow, file);
 WriteMARK(data_flow, file);
 WriteINST(data_flow, file);
 WriteAESD(data_flow, file);
 WriteMiscellaneous(data_flow, file);
 WriteSSND(data_flow, file);
...
"file"
, "libaudiofile/aiffwrite.c", 110, __PRETTY_FUNCTION__))
            ;
 ...
 length = af_flength(data_flow, file->fh);
 length -= 8;
 ...
 af_write_uint32_be(data_flow, &length, file->fh);
...
static status WriteCOMM (int *data_flow, const AFfilehandle file)
{
 _Track *track={0};
...
_Bool 
 ...
 uint16_t sb={0};
 uint32_t lb={0};
 unsigned char eb[10]={0};

 uint8_t compressionTag[4]={0};
 ...
 if (aiff->COMM_offset == 0)
  aiff->COMM_offset = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, aiff->COMM_offset, 
                                       0
                                               );
 ...
 if (isAIFFC)
 {
  if (track->f.compressionType == AF_COMPRESSION_NONE)
  {
   ...
   {
    _af_error(data_flow, AF_BAD_SAMPFMT,
   ...
   "0"
   , "libaudiofile/aiffwrite.c", 198, __PRETTY_FUNCTION__))
            ;
    return (-1);
   }
  }
 ...
 af_fwrite(data_flow, "COMM", 4, 1, file->fh);
 if (isAIFFC)
  chunkSize = 22 + strlen(compressionName) + 1;
 else
  chunkSize = 18;
 af_write_uint32_be(data_flow, &chunkSize, file->fh);


 sb = track->f.channelCount;
 af_write_uint16_be(data_flow, &sb, file->fh);


 lb = track->totalfframes;
 af_write_uint32_be(data_flow, &lb, file->fh);
 ...
 _af_convert_to_ieee_extended(data_flow, track->f.sampleRate, eb);
 af_fwrite(data_flow, eb, 10, 1, file->fh);

 if (file->fileFormat == AF_FILE_AIFFC)
 {
  af_fwrite(data_flow, compressionTag, 4, 1, file->fh);

  af_write_pstring(data_flow, compressionName, file->fh);
 }
...
static status WriteAESD (int *data_flow, const AFfilehandle file)
{
 _Track *track={0};
 uint32_t size = 24;
 _AIFFInfo *aiff={0};
...
"file"
, "libaudiofile/aiffwrite.c", 265, __PRETTY_FUNCTION__))
            ;
 ...
 if (aiff->AESD_offset == 0)
  aiff->AESD_offset = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, aiff->AESD_offset, 
                                       0
                                               );

 if (af_fwrite(data_flow, "AESD", 4, 1, file->fh) < 1)
  return (-1);

 if (af_write_uint32_be(data_flow, &size, file->fh) == (-1))
  return (-1);

 if (af_fwrite(data_flow, track->aesData, 24, 1, file->fh) < 1)
  return (-1);
...
"file"
, "libaudiofile/aiffwrite.c", 297, __PRETTY_FUNCTION__))
            ;
...
"file->fh"
, "libaudiofile/aiffwrite.c", 298, __PRETTY_FUNCTION__))
                ;
 ...
 if (aiff->SSND_offset == 0)
  aiff->SSND_offset = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, aiff->SSND_offset, 
                                       0
                                               );
 ...
 af_fwrite(data_flow, "SSND", 4, 1, file->fh);
 af_write_uint32_be(data_flow, &chunkSize, file->fh);


 af_write_uint32_be(data_flow, &zero, file->fh);
...
{
 uint32_t length={0};
 struct _INST instrumentdata={0};

 length = 20;

 instrumentdata.sustainLoopPlayMode =
  afGetLoopMode(data_flow, file, AF_DEFAULT_INST, 1);
 instrumentdata.sustainLoopBegin =
  afGetLoopStart(data_flow, file, AF_DEFAULT_INST, 1);
 instrumentdata.sustainLoopEnd =
  afGetLoopEnd(data_flow, file, AF_DEFAULT_INST, 1);

 instrumentdata.releaseLoopPlayMode =
  afGetLoopMode(data_flow, file, AF_DEFAULT_INST, 2);
 instrumentdata.releaseLoopBegin =
  afGetLoopStart(data_flow, file, AF_DEFAULT_INST, 2);
 instrumentdata.releaseLoopEnd =
  afGetLoopEnd(data_flow, file, AF_DEFAULT_INST, 2);

 af_fwrite(data_flow, "INST", 4, 1, file->fh);
 af_write_uint32_be(data_flow, &length, file->fh);

 instrumentdata.baseNote =
  afGetInstParamLong(data_flow, file, AF_DEFAULT_INST, AF_INST_MIDI_BASENOTE);
 af_write_uint8(data_flow, &instrumentdata.baseNote, file->fh);
 instrumentdata.detune =
  afGetInstParamLong(data_flow, file, AF_DEFAULT_INST, AF_INST_NUMCENTS_DETUNE);
 af_write_uint8(data_flow, &instrumentdata.detune, file->fh);
 instrumentdata.lowNote =
  afGetInstParamLong(data_flow, file, AF_DEFAULT_INST, AF_INST_MIDI_LONOTE);
 af_write_uint8(data_flow, &instrumentdata.lowNote, file->fh);
 instrumentdata.highNote =
  afGetInstParamLong(data_flow, file, AF_DEFAULT_INST, AF_INST_MIDI_HINOTE);
 af_write_uint8(data_flow, &instrumentdata.highNote, file->fh);
 instrumentdata.lowVelocity =
  afGetInstParamLong(data_flow, file, AF_DEFAULT_INST, AF_INST_MIDI_LOVELOCITY);
 af_write_uint8(data_flow, &instrumentdata.lowVelocity, file->fh);
 instrumentdata.highVelocity =
  afGetInstParamLong(data_flow, file, AF_DEFAULT_INST, AF_INST_MIDI_HIVELOCITY);
 af_write_uint8(data_flow, &instrumentdata.highVelocity, file->fh);

 instrumentdata.gain =
  afGetInstParamLong(data_flow, file, AF_DEFAULT_INST, AF_INST_NUMDBS_GAIN);
 af_write_uint16_be(data_flow, &instrumentdata.gain, file->fh);

 af_write_uint16_be(data_flow, &instrumentdata.sustainLoopPlayMode, file->fh);
 af_write_uint16_be(data_flow, &instrumentdata.sustainLoopBegin, file->fh);
 af_write_uint16_be(data_flow, &instrumentdata.sustainLoopEnd, file->fh);

 af_write_uint16_be(data_flow, &instrumentdata.releaseLoopPlayMode, file->fh);
 af_write_uint16_be(data_flow, &instrumentdata.releaseLoopBegin, file->fh);
 af_write_uint16_be(data_flow, &instrumentdata.releaseLoopEnd, file->fh);
...
static status WriteMARK (int *data_flow, AFfilehandle file)
{
 AFfileoffset chunkStartPosition, chunkEndPosition={0};
 uint32_t length = 0;
...
"file"
, "libaudiofile/aiffwrite.c", 392, __PRETTY_FUNCTION__))
            ;

 numMarkers = afGetMarkIDs(data_flow, file, AF_DEFAULT_TRACK, 
                                                  ((void *)0)
                                                      );
 if (numMarkers == 0)
  return (0);
 ...
 if (aiff->MARK_offset == 0)
  aiff->MARK_offset = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, aiff->MARK_offset, 
                                       0
                                               );

 af_fwrite(data_flow, "MARK", 4, 1, file->fh);
 af_write_uint32_be(data_flow, &length, file->fh);

 chunkStartPosition = af_ftell(data_flow, file->fh);

 markids = _af_calloc(data_flow, numMarkers, sizeof (int));
 
((
markids
) ? (void) (0) : __assert_fail (
"markids"
, "libaudiofile/aiffwrite.c", 411, __PRETTY_FUNCTION__))
               ;
 afGetMarkIDs(data_flow, file, AF_DEFAULT_TRACK, markids);

 af_write_uint16_be(data_flow, &numMarkers, file->fh);

 for (i=0; i<numMarkers; i++)
 {
  uint8_t namelength={0}, zero = 0;
  uint16_t id={0};
  uint32_t position={0};
  char *name={0};

  id = markids[i];
  position = afGetMarkPosition(data_flow, file, AF_DEFAULT_TRACK, markids[i]);

  af_write_uint16_be(data_flow, &id, file->fh);
  af_write_uint32_be(data_flow, &position, file->fh);
 ...
 "name"
 , "libaudiofile/aiffwrite.c", 430, __PRETTY_FUNCTION__))
             ;


  af_write_pstring(data_flow, name, file->fh);
 }

 free(markids);

 chunkEndPosition = af_ftell(data_flow, file->fh);
 ...
 af_fseek(data_flow, file->fh, chunkStartPosition - 4, 
                                           0
                                                   );

 af_write_uint32_be(data_flow, &length, file->fh);
 af_fseek(data_flow, file->fh, chunkEndPosition, 
                                     0
                                             );
...
static status WriteFVER (int *data_flow, AFfilehandle file)
{
 uint32_t chunkSize, timeStamp={0};
 _AIFFInfo *aiff={0};
...
"file->fileFormat == AF_FILE_AIFFC"
, "libaudiofile/aiffwrite.c", 462, __PRETTY_FUNCTION__))
                                         ;

 aiff = file->formatSpecific;

 if (aiff->FVER_offset == 0)
  aiff->FVER_offset = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, aiff->FVER_offset, 
                                       0
                                               );

 af_fwrite(data_flow, "FVER", 4, 1, file->fh);

 chunkSize = 4;
 af_write_uint32_be(data_flow, &chunkSize, file->fh);

 timeStamp = 0xa2805140;
 af_write_uint32_be(data_flow, &timeStamp, file->fh);
...
static status WriteMiscellaneous (int *data_flow, AFfilehandle file)
{
 ...
 if (aiff->miscellaneousPosition == 0)
  aiff->miscellaneousPosition = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, aiff->miscellaneousPosition, 
                                                 0
                                                         );
 ...
 for (i=0; i<file->miscellaneousCount; i++)
 {
  ...
  uint32_t chunkType, chunkSize={0};
  uint8_t padByte = 0;
  ...
  af_fwrite(data_flow, &chunkType, 4, 1, file->fh);
  ...
  if (misc->buffer != 
   ...
   af_fwrite(data_flow, misc->buffer, misc->size, 1, file->fh);
  else
   af_fseek(data_flow, file->fh, misc->size, 
                                 1
                                         );

  if (misc->size % 2 != 0)
   af_write_uint8(data_flow, &padByte, file->fh);
 }
