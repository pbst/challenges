...
unsigned char _af_linear2alaw (int *data_flow, int pcm_val);
...
int _af_alaw2linear (int *data_flow, unsigned char a_val);
unsigned char _af_linear2ulaw (int *data_flow, int pcm_val);
int _af_ulaw2linear (int *data_flow, unsigned char u_val);



static void ulaw2linear_buf (int *data_flow, unsigned char *ulaw, signed short int *linear,
 int nsamples)
{
 int i;
 for (i=0; i < nsamples; i++)
 {
  linear[i] = _af_ulaw2linear(data_flow, ulaw[i]);
 }
}

static void linear2ulaw_buf (int *data_flow, signed short int *linear, unsigned char *ulaw,
 int nsamples)
{
 int i;
 for (i=0; i < nsamples; i++)
 {
  ulaw[i] = _af_linear2ulaw(data_flow, linear[i]);
 }
}

static void alaw2linear_buf (int *data_flow, unsigned char *alaw, signed short int *linear,
 int nsamples)
{
 int i;
 for (i=0; i < nsamples; i++)
 {
  linear[i] = _af_alaw2linear(data_flow, alaw[i]);
 }
}

static void linear2alaw_buf (int *data_flow, signed short int *linear, unsigned char *alaw,
 int nsamples)
{
 int i;
 for (i=0; i < nsamples; i++)
 {
  alaw[i] = _af_linear2alaw(data_flow, linear[i]);
 }
}
...
static void g711compressdescribe (int *data_flow, _AFmoduleinst *i)
{
...
_Bool 
     headerless, AFframecount *chunkframes)
{
 _AFmoduleinst ret = _AFnewmodinst(data_flow, &g711compress);
 g711_data *d;

 d = (g711_data *) _af_malloc(data_flow, sizeof (g711_data));
...
static void g711run_push (int *data_flow, _AFmoduleinst *i)
{
 g711_data *d = (g711_data *)i->modspec;
...
((
d->trk->f.compressionType == AF_COMPRESSION_G711_ULAW || d->trk->f.compressionType == AF_COMPRESSION_G711_ALAW
...
,
 ...
 if (d->trk->f.compressionType == AF_COMPRESSION_G711_ULAW)
  linear2ulaw_buf(data_flow, i->inc->buf, i->outc->buf, samps2write);
 else
  linear2alaw_buf(data_flow, i->inc->buf, i->outc->buf, samps2write);



 nfr = af_fwrite(data_flow, i->outc->buf, framesize, frames2write, d->fh);
 ...
 if (nfr != frames2write)
 {
  ...
  if (d->trk->filemodhappy)
  {
   ...
   if (nfr < 0)
    _af_error(data_flow, AF_BAD_WRITE,
...
((
!d->seekok || (af_ftell(data_flow, d->fh) == d->trk->fpos_next_frame)
) ? (void) (0) : __assert_fail (
"!d->seekok || (af_ftell(d->fh) == d->trk->fpos_next_frame)"
, "libaudiofile/modules/g711.c", 202, __PRETTY_FUNCTION__))
                                                                  ;
}

static void g711sync1 (int *data_flow, _AFmoduleinst *i)
{
 g711_data *d = (g711_data *)i->modspec;
...
static void g711sync2 (int *data_flow, _AFmoduleinst *i)
{
 g711_data *d = (g711_data *) i->modspec;
...
"!d->seekok || (af_ftell(d->fh) == d->trk->fpos_next_frame)"
, "libaudiofile/modules/g711.c", 218, __PRETTY_FUNCTION__))
                                                                  ;


 d->trk->fpos_after_data = af_ftell(data_flow, d->fh);

 d->trk->fpos_next_frame = d->saved_fpos_next_frame;
 d->trk->nextfframe = d->saved_nextfframe;
}

static void g711decompressdescribe(int *data_flow, _AFmoduleinst *i)
{
...
_Bool 
     seekok, 
             _Bool 
                  headerless, AFframecount *chunkframes)
{
 _AFmoduleinst ret = _AFnewmodinst(data_flow, &g711decompress);
 g711_data *d;
...
static void g711run_pull (int *data_flow, _AFmoduleinst *i)
{
 g711_data *d = (g711_data *) i->modspec;
 ...
 nfr = af_fread(data_flow, i->inc->buf, framesize, frames2read, d->fh);



 if (d->trk->f.compressionType == AF_COMPRESSION_G711_ULAW)
  ulaw2linear_buf(data_flow, i->inc->buf, i->outc->buf, samps2read);
 else
  alaw2linear_buf(data_flow, i->inc->buf, i->outc->buf, samps2read);
...
"!d->seekok || (af_ftell(d->fh) == d->trk->fpos_next_frame)"
, "libaudiofile/modules/g711.c", 283, __PRETTY_FUNCTION__))
                                                                  ;
...
static void g711reset1 (int *data_flow, _AFmoduleinst *i)
{
...
static void g711reset2 (int *data_flow, _AFmoduleinst *i)
{
 g711_data *d = (g711_data *) i->modspec;
 int framesize = sizeof (g711samp) * (i->inc->f.channelCount);
