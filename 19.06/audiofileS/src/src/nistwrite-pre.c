...
static char *sample_byte_format (int *data_flow, _AudioFormat *fmt)
{
 int nbytes = _af_format_sample_size(data_flow, fmt, 
                                         0
                                              );
...
"nbytes == 1 || nbytes == 2"
, "libaudiofile/nistwrite.c", 50, __PRETTY_FUNCTION__))
                                  ;
...
static char *sample_coding (int *data_flow, _AudioFormat *fmt)
{
...
status WriteNISTHeader (int *data_flow, AFfilehandle file)
{
 ...
 printed = snprintf(header, 1024,
  ...
  (int) _af_format_sample_size(data_flow, &track->f, 
                                         0
                                              ),
  (int) _af_format_sample_size(data_flow, &track->f, 
                                         0
                                              ), sample_byte_format(data_flow , &track->f),
  track->f.sampleWidth,
  (int) strlen(sample_coding(data_flow, &track->f)), sample_coding(data_flow, &track->f));


 if (printed < 1024)
  memset(header + printed, ' ', 1024 - printed);

 return af_fwrite(data_flow, header, 1024, 1, fp);
}
...
((
handle->fileFormat == AF_FILE_NIST_SPHERE
) ? (void) (0) : __assert_fail (
"handle->fileFormat == AF_FILE_NIST_SPHERE"
, "libaudiofile/nistwrite.c", 117, __PRETTY_FUNCTION__))
                                                 ;

 if (_af_filesetup_make_handle(data_flow, setup, handle) == (-1))
  return (-1);
 ...
 WriteNISTHeader(data_flow, handle);
...
status _af_nist_update (int *data_flow, AFfilehandle file)
{
 ...
 WriteNISTHeader(data_flow, file);
