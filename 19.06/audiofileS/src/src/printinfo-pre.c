...
char *copyrightstring (int *data_flow, AFfilehandle file);

void printfileinfo (int *data_flow, char *filename)
{
 int version={0};
 AFfilehandle file={0};
 int sampleFormat, sampleWidth, byteOrder, compressionType={0};
 char *copyright, *formatstring, *labelstring={0};

 file = afOpenFile(data_flow, filename, "r", 
                                 ((void *)0)
                                     );
 ...
 formatstring = afQueryPointer(data_flow, AF_QUERYTYPE_FILEFMT, AF_QUERY_DESC,
  afGetFileFormat(data_flow, file, &version), 0, 0);
 labelstring = afQueryPointer(data_flow, AF_QUERYTYPE_FILEFMT, AF_QUERY_LABEL,
  afGetFileFormat(data_flow, file, &version), 0, 0);
 ...
 afGetSampleFormat(data_flow, file, AF_DEFAULT_TRACK, &sampleFormat, &sampleWidth);
 byteOrder = afGetByteOrder(data_flow, file, AF_DEFAULT_TRACK);

 printf("Data Format    ");

 compressionType = afGetCompression(data_flow, file, AF_DEFAULT_TRACK);
 ...
 {
  char *compressionName={0};
  compressionName = afQueryPointer(data_flow, AF_QUERYTYPE_COMPRESSION,
 ...
 printf("Audio Data     %ld bytes begins at offset %ld (%lx hex)\n",
  ...
  afGetTrackBytes(data_flow, file, AF_DEFAULT_TRACK),
  afGetDataOffset(data_flow, file, AF_DEFAULT_TRACK),
  afGetDataOffset(data_flow, file, AF_DEFAULT_TRACK));
 ...
 printf("               %d channel%s, %ld frames\n",
  ...
  afGetChannels(data_flow, file, AF_DEFAULT_TRACK),
  afGetChannels(data_flow, file, AF_DEFAULT_TRACK) > 1 ? "s" : "",
  afGetFrameCount(data_flow, file, AF_DEFAULT_TRACK));

 printf("Sampling Rate  %.2f Hz\n", afGetRate(data_flow, file, AF_DEFAULT_TRACK));

 printf("Duration       %.3f seconds\n",
  afGetFrameCount(data_flow, file, AF_DEFAULT_TRACK) /
  afGetRate(data_flow, file, AF_DEFAULT_TRACK));

 copyright = copyrightstring(data_flow, file);
 if (copyright)
 {
  printf("Copyright      %s\n", copyright);
  free(copyright);
...
char *copyrightstring (int *data_flow, AFfilehandle file)
{
 ...
 misccount = afGetMiscIDs(data_flow, file, 
                               ((void *)0)
                                   );
 miscids = malloc(sizeof (int) * misccount);
 afGetMiscIDs(data_flow, file, miscids);
 ...
 for (i=0; i<misccount; i++)
 {
  ...
  if (afGetMiscType(data_flow, file, miscids[i]) != AF_MISC_COPY)
   continue;
  ...
  datasize = afGetMiscSize(data_flow, file, miscids[i]);
  data = malloc(datasize);
  afReadMisc(data_flow, file, miscids[i], data, datasize);
  copyright = data;
  break;
 }
