...
int afWriteFrames (int *data_flow, AFfilehandle file, int trackid, const void *samples,
 int nvframes2write)
{
 _AFmoduleinst *firstmod={0};
 _AFchunk *userc={0};
 _Track *track={0};
 int bytes_per_vframe={0};
 AFframecount vframe={0};
 ...
 if (af_fseek(data_flow, file->fh, track->fpos_next_frame, 
                                               0
                                                       ) < 0)
 {
  _af_error(data_flow, AF_BAD_LSEEK, "unable to position write pointer at next frame");
  return -1;
 }



 bytes_per_vframe = _af_format_frame_size(data_flow, &track->v, 
                                                    1
                                                        );
...
"track->ms.mustuseatomicnvframes"
, "libaudiofile/data.c", 100, __PRETTY_FUNCTION__))
                                       ;

 {
  while (vframe < nvframes2write)
  {
   ...
   if (vframe <= nvframes2write - 1024)
    userc->nframes = 1024;
   else
    userc->nframes = nvframes2write - vframe;

   (*firstmod->mod->run_push)(data_flow, firstmod);
...
int afReadFrames (int *data_flow, AFfilehandle file, int trackid, void *samples,
 int nvframeswanted)
{
 _Track *track={0};
 _AFmoduleinst *firstmod={0};
 _AFchunk *userc={0};
 AFframecount nvframesleft, nvframes2read={0};
 int bytes_per_vframe={0};
 AFframecount vframe={0};
 ...
 {
  _af_error(data_flow, AF_BAD_LSEEK, "unable to position read pointer at next frame");
  return -1;
 }
 ...
 "track->frames2ignore == 0"
 , "libaudiofile/data.c", 180, __PRETTY_FUNCTION__))
                                  ;
  userc->buf = samples;
  userc->nframes = nvframes2read;

  (*firstmod->mod->run_pull)(data_flow, firstmod);
  if (track->filemodhappy)
   vframe += userc->nframes;
 }
 else
 ...
 _Bool 
  ...
  if (track->frames2ignore != 0)
  {
   ...
   userc->buf = _af_malloc(data_flow, track->frames2ignore * bytes_per_vframe);
   if (userc->buf == ((void *) 0))
    return 0;
  ...
  {
   AFframecount nvframes2pull={0};
   userc->buf = (char *) samples + bytes_per_vframe * vframe;

   if (vframe <= nvframes2read - 1024)
    nvframes2pull = 1024;
   else
