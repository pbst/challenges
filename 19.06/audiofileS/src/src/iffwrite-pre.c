...
status _af_iff_update (int *data_flow, AFfilehandle file);

static status WriteVHDR (int *data_flow, AFfilehandle file);
static status WriteMiscellaneous (int *data_flow, AFfilehandle file);
static status WriteBODY (int *data_flow, AFfilehandle file);

static _IFFinfo *iff_info_new (int *data_flow)
{
 _IFFinfo *iff = _af_calloc(data_flow, 1, sizeof (_IFFinfo));
...
status _af_iff_write_init (int *data_flow, AFfilesetup setup, AFfilehandle file)
{
 uint32_t fileSize = 0;
 ...
 file->formatSpecific = iff_info_new(data_flow);

 af_fwrite(data_flow, "FORM", 4, 1, file->fh);
 af_write_uint32_be(data_flow, &fileSize, file->fh);

 af_fwrite(data_flow, "8SVX", 4, 1, file->fh);

 WriteVHDR(data_flow, file);
 WriteMiscellaneous(data_flow, file);
 WriteBODY(data_flow, file);
...
static status WriteVHDR (int *data_flow, const AFfilehandle file)
{
 _Track *track={0};
 _IFFinfo *iff={0};
 ...
 if (iff->VHDR_offset == 0)
  iff->VHDR_offset = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, iff->VHDR_offset, 
                                      0
                                              );

 track = _af_filehandle_get_track(data_flow, file, AF_DEFAULT_TRACK);

 af_fwrite(data_flow, "VHDR", 4, 1, file->fh);
 ...
 af_write_uint32_be(data_flow, &oneShotSamples, file->fh);
 repeatSamples = 0;
 af_write_uint32_be(data_flow, &repeatSamples, file->fh);
 samplesPerRepeat = 0;
 af_write_uint32_be(data_flow, &samplesPerRepeat, file->fh);

 sampleRate = track->f.sampleRate;
 af_write_uint16_be(data_flow, &sampleRate, file->fh);

 octaves = 0;
 compression = 0;
 af_write_uint8(data_flow, &octaves, file->fh);
 af_write_uint8(data_flow, &compression, file->fh);


 volume = 65536;
 af_write_uint32_be(data_flow, &volume, file->fh);
...
static status WriteBODY (int *data_flow, AFfilehandle file)
{
 ...
 if (iff->BODY_offset == 0)
  iff->BODY_offset = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, iff->BODY_offset, 
                                      0
                                              );

 af_fwrite(data_flow, "BODY", 4, 1, file->fh);
 ...
 {
  uint8_t zero = 0;
  af_fseek(data_flow, file->fh, iff->BODY_offset + 8 + chunkSize, 
                                                      0
                                                              );
...
static status WriteMiscellaneous (int *data_flow, AFfilehandle file)
{
 ...
 if (iff->miscellaneousPosition == 0)
  iff->miscellaneousPosition = af_ftell(data_flow, file->fh);
 else
  af_fseek(data_flow, file->fh, iff->miscellaneousPosition, 
                                                0
                                                        );
