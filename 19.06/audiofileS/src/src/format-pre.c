...
AFfileoffset afGetDataOffset (int *data_flow, AFfilehandle file, int trackid)
{
 _Track *track={0};
...
AFfileoffset afGetTrackBytes (int *data_flow, AFfilehandle file, int trackid)
{
 _Track *track={0};
 ...
 track = _af_filehandle_get_track(data_flow, file, trackid);

 return track->data_size;
}
float afGetFrameSize (int *data_flow, AFfilehandle file, int trackid, int stretch3to4)
{
 _Track *track={0};
 ...
 return _af_format_frame_size(data_flow, &track->f, stretch3to4);
}

float afGetVirtualFrameSize (int *data_flow, AFfilehandle file, int trackid, int stretch3to4)
{
 _Track *track={0};
 ...
 return _af_format_frame_size(data_flow, &track->v, stretch3to4);
}

AFframecount afSeekFrame (int *data_flow, AFfilehandle file, int trackid, AFframecount frame)
{
 _Track *track={0};
 ...
 if (!_af_filehandle_can_read(data_flow, file))
  return -1;
 ...
 if (track->ms.modulesdirty)
  if (_AFsetupmodules(data_flow, file, track) != (0))
   return -1;
...
AFfileoffset afTellFrame (int *data_flow, AFfilehandle file, int trackid)
{
 return afSeekFrame(data_flow, file, trackid, -1);
}

int afSetVirtualByteOrder (int *data_flow, AFfilehandle handle, int track, int byteorder)
{
 _Track *currentTrack={0};
 ...
 if (
    ((void *)0) 
         == (currentTrack = _af_filehandle_get_track(data_flow, handle, track)))
  return (-1);
...
int afGetByteOrder (int *data_flow, AFfilehandle handle, int track)
{
 _Track *currentTrack={0};
 ...
 if ((currentTrack = _af_filehandle_get_track(data_flow, handle, track)) == 
                                                                ((void *)0)
                                                                    )
  return -1;
...
int afGetVirtualByteOrder (int *data_flow, AFfilehandle handle, int track)
{
 _Track *currentTrack={0};
...
AFframecount afGetFrameCount (int *data_flow, AFfilehandle file, int trackid)
{
 _Track *track={0};
...
double afGetRate (int *data_flow, AFfilehandle file, int trackid)
{
 _Track *track={0};
...
int afGetChannels (int *data_flow, AFfilehandle file, int trackid)
{
 _Track *track={0};
...
void afGetSampleFormat (int *data_flow, AFfilehandle file, int trackid, int *sampleFormat, int *sampleWidth)
{
 _Track *track={0};
...
void afGetVirtualSampleFormat (int *data_flow, AFfilehandle file, int trackid, int *sampleFormat, int *sampleWidth)
{
 _Track *track={0};
...
int afSetVirtualSampleFormat (int *data_flow, AFfilehandle file, int trackid,
 int sampleFormat, int sampleWidth)
{
 _Track *track={0};
 ...
 if (_af_set_sample_format(data_flow, &track->v, sampleFormat, sampleWidth) == (-1))
  return -1;
...
int afGetFileFormat (int *data_flow, AFfilehandle file, int *version)
{
 if (!_af_filehandle_ok(data_flow, file))
  return -1;
                   ...
                   )
 {
   ...
   *version = _af_units[file->fileFormat].getversion(data_flow, file);
  else
   *version = 0;
 }
...
int afSetVirtualChannels (int *data_flow, AFfilehandle file, int trackid, int channelCount)
{
 _Track *track={0};
...
double afGetVirtualRate (int *data_flow, AFfilehandle file, int trackid)
{
 _Track *track={0};
...
int afSetVirtualRate (int *data_flow, AFfilehandle file, int trackid, double rate)
{
 _Track *track={0};
 ...
 if (rate < 0)
 {
  _af_error(data_flow, AF_BAD_RATE, "invalid sampling rate %.30g", rate);
  return -1;
 }
...
void afSetChannelMatrix (int *data_flow, AFfilehandle file, int trackid, double* matrix)
{
 _Track *track={0};
...
int afGetVirtualChannels (int *data_flow, AFfilehandle file, int trackid)
{
 _Track *track={0};
