...
AFvirtualfile *
af_virtual_file_new(int *data_flow)
{
  return (AFvirtualfile *) calloc(sizeof (AFvirtualfile), 1);
}

void
af_virtual_file_destroy(int *data_flow, AFvirtualfile *vfile)
{
  vfile->destroy(data_flow, vfile);
...
size_t af_fwrite (int *data_flow, const void *data, size_t size, size_t nmemb,
 AFvirtualfile *vfile)
{
  if (size == 0 || nmemb == 0)
    return 0;
  ...
  if (vfile->write) {
    ...
    retval = (* vfile->write) (data_flow, vfile, data, size * nmemb);
...
int
af_fclose(int *data_flow, AFvirtualfile *vfile)
{
  af_virtual_file_destroy(data_flow, vfile);

  return 0;
}

long
af_flength(int *data_flow, AFvirtualfile *vfile)
{
  if(vfile->length)
    return (* vfile->length)(data_flow, vfile);
  else
    return 0;
}

int
af_fseek(int *data_flow, AFvirtualfile *vfile, long offset, int whence)
{
  if(whence == 
              1
                      )
    (* vfile->seek) (data_flow, vfile, offset, 1);
  else if(whence == 
                   0
                           )
...
long
af_ftell(int *data_flow, AFvirtualfile *vfile)
{
  if(vfile->tell)
    return (* vfile->tell)(data_flow, vfile);
  else
    return 0;
}

static ssize_t af_file_read (int *data_flow, AFvirtualfile *vfile, void *data, size_t nbytes);
static long af_file_length (int *data_flow, AFvirtualfile *vfile);
static ssize_t af_file_write (int *data_flow, AFvirtualfile *vfile, const void *data,
 size_t nbytes);
static void af_file_destroy(int *data_flow, AFvirtualfile *vfile);
static long af_file_seek(int *data_flow, AFvirtualfile *vfile, long offset, int is_relative);
static long af_file_tell(int *data_flow, AFvirtualfile *vfile);

AFvirtualfile *
af_virtual_file_new_for_file(int *data_flow, FILE *fh)
{
  AFvirtualfile *vf={0};
  ...
  vf = af_virtual_file_new(data_flow);
  vf->closure = fh;
...
static long
af_file_length(int *data_flow, AFvirtualfile *vfile)
{
  long curpos, retval={0};

  curpos = ftell(vfile->closure);
  fseek(vfile->closure, 0, 
                          2
                                  );
  retval = ftell(vfile->closure);
  fseek(vfile->closure, curpos, 
                               0
                                       );
...
static void
af_file_destroy(int *data_flow, AFvirtualfile *vfile)
{
  fclose(vfile->closure); vfile->closure = 
                                          ((void *)0)
                                              ;
}

static long
af_file_seek(int *data_flow, AFvirtualfile *vfile, long offset, int is_relative)
{
  fseek(vfile->closure, offset, is_relative?
                                           1
                                                   :
                                                    0
                                                            );
...
static long
af_file_tell(int *data_flow, AFvirtualfile *vfile)
{
  return ftell(vfile->closure);
}
