...
int _af_compression_index_from_id (int *data_flow, int compressionid)
{
 ...
 _af_error(data_flow, AF_BAD_COMPTYPE, "compression type %d not available", compressionid);

 return -1;
}

static const _CompressionUnit *findCompression (int *data_flow, int compressionid)
{
 int compressionno={0};

 compressionno = _af_compression_index_from_id(data_flow, compressionid);
 if (compressionno != -1)
...
int afGetCompression (int *data_flow, AFfilehandle file, int trackid)
{
 _Track *track={0};
...
void afInitCompression (int *data_flow, AFfilesetup setup, int trackid, int compression)
{
 _TrackSetup *track={0};
 ...
 if (findCompression(data_flow, compression) == 
                                    ((void *)0)
                                        )
  return;
