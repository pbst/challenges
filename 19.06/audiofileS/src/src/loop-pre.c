...
void afInitLoopIDs (int *data_flow, AFfilesetup setup, int instid, int *loopids, int nloops)
{
 int instno={0};
 ...
 if (!_af_unique_ids(data_flow, loopids, nloops, "loop", AF_BAD_LOOPID))
  return;

 if ((instno = _af_setup_instrument_index_from_id(data_flow, setup, instid)) == -1)
  return;

 _af_setup_free_loops(data_flow, setup, instno);

 setup->instruments[instno].loopCount = nloops;
 setup->instruments[instno].loopSet = 
                                     1
                                         ;
 ...
 else
 {
  ...
  if ((setup->instruments[instno].loops = _af_calloc(data_flow, nloops, sizeof (_LoopSetup))) == 
                                                                                     ((void *)0)
                                                                                         )
   return;
...
int afGetLoopIDs (int *data_flow, AFfilehandle file, int instid, int *loopids)
{
 int instno={0};
 int i={0};
 ...
 if ((instno = _af_handle_instrument_index_from_id(data_flow, file, instid)) == -1)
  return (-1);
...
int _af_handle_loop_index_from_id (int *data_flow, AFfilehandle file, int instno, int loopid)
{
 int i={0};
 for (i=0; i<file->instruments[instno].loopCount; i++)
 ...
 _af_error(data_flow, AF_BAD_LOOPID, "no loop with id %d for instrument %d",
...
static _Loop *getLoop (int *data_flow, AFfilehandle handle, int instid, int loopid,
     ...
     mustWrite)
{
 ...
 if (mustWrite && !_af_filehandle_can_write(data_flow, handle))
  return 
        ((void *)0)
            ;

 if ((instno = _af_handle_instrument_index_from_id(data_flow, handle, instid)) == -1)
  return 
        ((void *)0)
            ;

 if ((loopno = _af_handle_loop_index_from_id(data_flow, handle, instno, loopid)) == -1)
  return 
        ((void *)0)
            ;
...
void afSetLoopMode (int *data_flow, AFfilehandle file, int instid, int loopid, int mode)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            1
                                                );
 ...
 {
  _af_error(data_flow, AF_BAD_LOOPMODE, "unrecognized loop mode %d", mode);
  return;
 }
...
int afGetLoopMode (int *data_flow, AFfilehandle file, int instid, int loopid)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            0
                                                 );
...
int afSetLoopCount (int *data_flow, AFfilehandle file, int instid, int loopid, int count)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            1
                                                );
 ...
 if (count < 1)
 {
  _af_error(data_flow, AF_BAD_LOOPCOUNT, "invalid loop count: %d", count);
  return (-1);
 }
...
int afGetLoopCount(int *data_flow, AFfilehandle file, int instid, int loopid)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            0
                                                 );
...
void
afSetLoopStart(int *data_flow, AFfilehandle file, int instid, int loopid, int markid)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            1
                                                );
...
int afGetLoopStart (int *data_flow, AFfilehandle file, int instid, int loopid)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            0
                                                 );
...
int afSetLoopStartFrame (int *data_flow, AFfilehandle file, int instid, int loopid, AFframecount startFrame)
{
 int trackid, beginMarker={0};
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            1
                                                );
 ...
 if (startFrame < 0)
 {
  _af_error(data_flow, AF_BAD_FRAME, "loop start frame must not be negative");
  return (-1);
 }
 ...
 afSetMarkPosition(data_flow, file, trackid, beginMarker, startFrame);
 return (0);
}
...
AFframecount afGetLoopStartFrame (int *data_flow, AFfilehandle file, int instid, int loopid)
{
 int trackid, beginMarker={0};
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            0
 ...
 return afGetMarkPosition(data_flow, file, trackid, beginMarker);
}




void afSetLoopTrack (int *data_flow, AFfilehandle file, int instid, int loopid, int track)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            1
                                                );
...
int afGetLoopTrack (int *data_flow, AFfilehandle file, int instid, int loopid)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            0
                                                 );
...
void afSetLoopEnd (int *data_flow, AFfilehandle file, int instid, int loopid, int markid)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            1
                                                );
...
int afGetLoopEnd (int *data_flow, AFfilehandle file, int instid, int loopid)
{
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            0
                                                 );
...
int afSetLoopEndFrame (int *data_flow, AFfilehandle file, int instid, int loopid, AFframecount endFrame)
{
 int trackid, endMarker={0};
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            1
                                                );
 ...
 if (endFrame < 0)
 {
  _af_error(data_flow, AF_BAD_FRAME, "loop end frame must not be negative");
  return (-1);
 }
 ...
 afSetMarkPosition(data_flow, file, trackid, endMarker, endFrame);
 return (0);
}
...
AFframecount afGetLoopEndFrame (int *data_flow, AFfilehandle file, int instid, int loopid)
{
 int trackid, endMarker={0};
 _Loop *loop = getLoop(data_flow, file, instid, loopid, 
                                            0
 ...
 return afGetMarkPosition(data_flow, file, trackid, endMarker);
}
