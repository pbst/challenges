...
static status ParseVHDR (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type,
 size_t size);
static status ParseBODY (int *data_flow, AFfilehandle file, AFvirtualfile *fh, uint32_t type,
 size_t size);
    ...
    _af_iff_recognize (int *data_flow, AFvirtualfile *fh)
{
 ...
 if (af_fread(data_flow, buffer, 1, 8, fh) != 8 || memcmp(buffer, "FORM", 4) != 0)
  return 
        0
             ;
 if (af_fread(data_flow, buffer, 1, 4, fh) != 4 || memcmp(buffer, "8SVX", 4) != 0)
  return 
        0
             ;
...
((
!memcmp(&type, "NAME", 4) || !memcmp(&type, "AUTH", 4) || !memcmp(&type, "(c) ", 4) || !memcmp(&type, "ANNO", 4)
...
{
 _Track *track={0};
 uint32_t oneShotSamples, repeatSamples, samplesPerRepeat={0};
 uint16_t sampleRate={0};
 uint8_t octaves, compression={0};
 uint32_t volume={0};
...
"!memcmp(&type, \"VHDR\", 4)"
, "libaudiofile/iff.c", 135, __PRETTY_FUNCTION__))
                                 ;

 track = _af_filehandle_get_track(data_flow, file, AF_DEFAULT_TRACK);

 af_read_uint32_be(data_flow, &oneShotSamples, fh);
 af_read_uint32_be(data_flow, &repeatSamples, fh);
 af_read_uint32_be(data_flow, &samplesPerRepeat, fh);
 af_read_uint16_be(data_flow, &sampleRate, fh);
 af_read_uint8(data_flow, &octaves, fh);
 af_read_uint8(data_flow, &compression, fh);
 af_read_uint32_be(data_flow, &volume, fh);
 ...
 size_t size)
{
 ...
 track->fpos_first_frame = af_ftell(data_flow, fh);

 return (0);
}

status _af_iff_read_init (int *data_flow, AFfilesetup setup, AFfilehandle file)
{
...
"file != NULL"
, "libaudiofile/iff.c", 186, __PRETTY_FUNCTION__))
                    ;
...
((void *)0)) ? (void) (0) : __assert_fail (
"file->fh != NULL"
, "libaudiofile/iff.c", 187, __PRETTY_FUNCTION__))
                        ;
 ...
 while (index < size)
 {
  ...
  {
   result = ParseVHDR(data_flow, file, file->fh, chunkid, chunksize);
  }
  else if (!memcmp("BODY", &chunkid, 4))
  {
   result = ParseBODY(data_flow, file, file->fh, chunkid, chunksize);
  }
...
AFfilesetup _af_iff_complete_setup (int *data_flow, AFfilesetup setup)
{
 _TrackSetup *track={0};

 if (setup->trackSet && setup->trackCount != 1)
 {
  _af_error(data_flow, AF_BAD_NUMTRACKS, "IFF/8SVX file must have 1 track");
 ...
 track->f.channelCount = 1;
 _af_set_sample_format(data_flow, &track->f, AF_SAMPFMT_TWOSCOMP, 8);

 if (track->markersSet && track->markerCount != 0)
 {
  _af_error(data_flow, AF_BAD_NUMMARKS,
 ...
 {
  _af_error(data_flow, AF_BAD_NUMINSTS,
 ...
 {
  _af_error(data_flow, AF_BAD_NOT_IMPLEMENTED, "IFF/8SVX format does not "
   "currently support miscellaneous chunks");
  return ((struct _AFfilesetup *) 0);
 }

 return _af_filesetup_copy(data_flow, setup, &_af_iff_default_filesetup, 
                                                             1
                                                                 );
}
