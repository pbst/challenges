...
"list"
, "libaudiofile/debug.c", 67, __PRETTY_FUNCTION__))
            ;
 ...
 for (i=0; i<list->count; i++)
 {
  ...
  switch (list->items[i].type)
  {
   ...
   "0"
   , "libaudiofile/debug.c", 98, __PRETTY_FUNCTION__))
            ;
    break;
  }
 }
...
void _af_print_audioformat (int *data_flow, _AudioFormat *fmt)
{
 ...
 {
  int idx = _af_compression_index_from_id(data_flow, fmt->compressionType);
  if (idx < 0)
  {
...
void _af_print_tracks (int *data_flow, AFfilehandle filehandle)
{
 ...
 for (i=0; i<filehandle->trackCount; i++)
 {
  ...
  _af_print_audioformat(data_flow, &track->f);
  printf(" virtual format\n");
  _af_print_audioformat(data_flow, &track->v);
  ...
  _af_print_channel_matrix(data_flow, track->channelMatrix,
...
void _af_print_filehandle (int *data_flow, AFfilehandle filehandle)
{
 ...
 printf(" tracks: 0x%p\n", filehandle->tracks);
 _af_print_tracks(data_flow, filehandle);
}
...
void _af_print_chunk (int *data_flow, _AFchunk *chnk)
{
 ...
 switch (fmt.compressionType)
 {
  ...
  default:
   printf("LAME-O chunk dumper cannot deal with '%s' compression\n",
    _af_compression[_af_compression_index_from_id(data_flow, fmt.compressionType)].name);
   return;
 }

 if (fmt.sampleWidth > 8 && fmt.byteOrder != (AF_BYTEORDER_LITTLEENDIAN))
 {
 ...
 switch (fmt.sampleFormat)
 {
  ...
  case AF_SAMPFMT_FLOAT:
  {
   ...
   {
    { int s={0}; for(s=0; s < nsamps; s++) outbuf[s] = (double)(((double *)chnk->buf)[s]); };
   }
   else
   {
    { int s={0}; for(s=0; s < nsamps; s++) outbuf[s] = (double)(((float *)chnk->buf)[s]); };
   }
  ...
  _Bool 
       issigned = (fmt.sampleFormat==AF_SAMPFMT_TWOSCOMP);


   int realbytes = _af_format_sample_size_uncompressed(data_flow, &fmt, 
                                                            1
                                                                );
   ...
   {
    case 1:
     if (issigned) { { int s={0}; for(s=0; s < nsamps; s++) outbuf[s] = (double)(((schar1 *)chnk->buf)[s]); }; }
     else { { int s={0}; for(s=0; s < nsamps; s++) outbuf[s] = (double)(((uchar1 *)chnk->buf)[s]); }; }
     break;
    case 2:
     if (issigned) { { int s={0}; for(s=0; s < nsamps; s++) outbuf[s] = (double)(((schar2 *)chnk->buf)[s]); }; }
     else { { int s={0}; for(s=0; s < nsamps; s++) outbuf[s] = (double)(((uchar2 *)chnk->buf)[s]); }; }
     break;
    case 4:
     if (issigned) { { int s={0}; for(s=0; s < nsamps; s++) outbuf[s] = (double)(((schar4 *)chnk->buf)[s]); }; }
     else { { int s={0}; for(s=0; s < nsamps; s++) outbuf[s] = (double)(((uchar4 *)chnk->buf)[s]); }; }
     break;
    default:
  ...
  "0"
  , "libaudiofile/debug.c", 406, __PRETTY_FUNCTION__))
           ;
   return;
 }

 for (fr=0; fr < nframes; fr++)
  _af_print_frame(data_flow, fr, &outbuf[fr*fmt.channelCount],
