...
typedef struct floatrebuffer_data
{
...
static void floatrebufferv2fmax_push(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
...
static void floatrebufferv2frun_push(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
  long samples2push = i->inc->nframes * i->inc->f.channelCount;
  float *inbufp = i->inc->buf;
 ...
 ((
 d->offset >= 0 && d->offset < d->nsamps
 ) ? (void) (0) : __assert_fail (
 "d->offset >= 0 && d->offset < d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 80, __PRETTY_FUNCTION__))
                                                ;
  ...
  if (d->offset + samples2push >= d->nsamps)
    {
      ...
      if (d->offset != 0)
        memcpy(i->outc->buf, d->buf, sizeof(float)*d->offset);
      ...
      if (d->multiple_of)
        {
         ...
         "n > d->offset"
         , "libaudiofile/modules/rebuffer.template", 114, __PRETTY_FUNCTION__))
                              ;
          memcpy((float *)i->outc->buf + d->offset, inbufp,
                 sizeof(float)*(n - d->offset));

          _AFpush(data_flow, i, n / i->outc->f.channelCount);
         ...
         "samples2push >= 0"
         , "libaudiofile/modules/rebuffer.template", 122, __PRETTY_FUNCTION__))
                                  ;
          d->offset = 0;
        }
      else
          ...
          while (d->offset + samples2push >= d->nsamps)
            {
              ...
              _AFpush(data_flow, i, d->nsamps / i->outc->f.channelCount);
             ...
             "samples2push >= 0"
             , "libaudiofile/modules/rebuffer.template", 153, __PRETTY_FUNCTION__))
                                      ;
              d->offset = 0;
            }
        }
     ...
     "d->offset == 0"
     , "libaudiofile/modules/rebuffer.template", 159, __PRETTY_FUNCTION__))
                           ;
    }
 ...
 ((
 d->offset + samples2push < d->nsamps
 ) ? (void) (0) : __assert_fail (
 "d->offset + samples2push < d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 163, __PRETTY_FUNCTION__))
                                             ;


  if ( samples2push != 0 )
    {
      memcpy(d->buf+d->offset, inbufp, sizeof(float)*samples2push);
 ...
 "d->offset >= 0 && d->offset < d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 172, __PRETTY_FUNCTION__))
                                                ;
}

static void floatrebufferv2fsync1(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
 ...
 "d->offset >= 0 && d->offset < d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 179, __PRETTY_FUNCTION__))
                                                ;
  ...
  memcpy(d->saved_buf, d->buf, sizeof(float)*d->nsamps);
  d->saved_offset = d->offset;
}

static void floatrebufferv2fsync2(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
 ...
 "d->offset >= 0 && d->offset < d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 197, __PRETTY_FUNCTION__))
                                                ;
  ...
  _AFpush(data_flow, i, d->offset / i->outc->f.channelCount);



  memcpy(d->buf, d->saved_buf, sizeof(float)*d->nsamps);
 ...
 "d->offset >= 0 && d->offset < d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 217, __PRETTY_FUNCTION__))
                                                ;
}

static void floatrebufferv2ffree(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
  if (d->buf)
    free(d->buf);
...
static const _AFmodule floatrebufferv2f =
{
  "float" "rebufferv2f",
  ((void *) 0),
  ((void *) 0), floatrebufferv2fmax_push,
  ((void *) 0), ((void *) 0), ((void *) 0),
  floatrebufferv2frun_push, floatrebufferv2fsync1, floatrebufferv2fsync2,
  ((void *) 0),
  floatrebufferv2ffree
};
...
_AFmoduleinst _af_initfloatrebufferv2f(int *data_flow, AFframecount nsamps, 
                                                        _Bool 
                                                             multiple_of)
{
  _AFmoduleinst ret = _AFnewmodinst(data_flow, &floatrebufferv2f);
  floatrebuffer_data *d = _af_malloc(data_flow, sizeof (floatrebuffer_data));
  d->nsamps = nsamps;
  d->offset = 0;
  d->buf = _af_malloc(data_flow, sizeof (float) * nsamps);
  d->multiple_of = multiple_of;
  d->saved_buf = _af_malloc(data_flow, sizeof (float) * nsamps);
  ret.modspec = d;
...
static void floatrebufferf2vmax_pull(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
...
static void floatrebufferf2vrun_pull(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
  long samples2pull = i->outc->nframes * i->outc->f.channelCount;
  float *outbufp = i->outc->buf;
 ...
 "d->offset > 0 && d->offset <= d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 281, __PRETTY_FUNCTION__))
                                                ;
 ...
 "!d->sent_short_chunk"
 , "libaudiofile/modules/rebuffer.template", 289, __PRETTY_FUNCTION__))
                             ;
  ...
  if (d->offset != d->nsamps)
    {
      ...
      int n = ((samples2pull)<(buffered) ? (samples2pull) : (buffered));
      memcpy(outbufp, d->buf+d->offset, sizeof(float)*n);
  ...
  while (!d->eof && samples2pull > 0)
    {
     ...
     "req > 0"
     , "libaudiofile/modules/rebuffer.template", 405, __PRETTY_FUNCTION__))
                    ;



      _AFpull(data_flow, i, req / i->inc->f.channelCount);

      got = i->inc->nframes * i->inc->f.channelCount;
      ...
      memcpy(outbufp, i->inc->buf, sizeof(float)*((samples2pull)<(got) ? (samples2pull) : (got)));
      ...
      if (d->multiple_of)
       ...
       ((
       d->eof || samples2pull <= 0
       ) ? (void) (0) : __assert_fail (
       "d->eof || samples2pull <= 0"
       , "libaudiofile/modules/rebuffer.template", 429, __PRETTY_FUNCTION__))
                                          ;
      ...
      if (samples2pull < 0)
        {
         ...
         "d->offset==d->nsamps"
         , "libaudiofile/modules/rebuffer.template", 438, __PRETTY_FUNCTION__))
                                     ;
         ...
         "d->offset > 0 && d->offset <= d->nsamps"
         , "libaudiofile/modules/rebuffer.template", 443, __PRETTY_FUNCTION__))
                                                        ;

          memcpy(d->buf+d->offset, (float *)i->inc->buf+d->offset,
                 sizeof(float)*(d->nsamps-d->offset));
        }
      else
        {
         ...
         "d->offset==d->nsamps"
         , "libaudiofile/modules/rebuffer.template", 450, __PRETTY_FUNCTION__))
                                     ;
        }
    }
                            ...
                            ;
  if (d->eof && samples2pull > 0)
    {
      i->outc->nframes -= samples2pull / i->inc->f.channelCount;
      d->sent_short_chunk = 
                           1
                               ;
     ...
     "d->offset == d->nsamps"
     , "libaudiofile/modules/rebuffer.template", 487, __PRETTY_FUNCTION__))
                                   ;
    }
  else
    {
     ...
     "samples2pull <= 0"
     , "libaudiofile/modules/rebuffer.template", 492, __PRETTY_FUNCTION__))
                              ;
     ...
     "d->offset == d->nsamps + samples2pull"
     , "libaudiofile/modules/rebuffer.template", 495, __PRETTY_FUNCTION__))
                                                  ;
    }
 ...
 "d->offset > 0 && d->offset <= d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 497, __PRETTY_FUNCTION__))
                                                ;
...
static void floatrebufferf2vreset1(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
  d->offset = d->nsamps;
  d->eof = 
 ...
 "d->offset > 0 && d->offset <= d->nsamps"
 , "libaudiofile/modules/rebuffer.template", 510, __PRETTY_FUNCTION__))
                                                ;
}

static void floatrebufferf2vreset2(int *data_flow, struct _AFmoduleinst *i)
{
...
static void floatrebufferf2vfree(int *data_flow, struct _AFmoduleinst *i)
{
  floatrebuffer_data *d = i->modspec;
  if (d->buf)
    free(d->buf);
...
static const _AFmodule floatrebufferf2v =
{
  "float" "rebufferf2v",
  ((void *) 0),
  floatrebufferf2vmax_pull, ((void *) 0),
  floatrebufferf2vrun_pull, floatrebufferf2vreset1, floatrebufferf2vreset2,
  ((void *) 0), ((void *) 0), ((void *) 0),
  ((void *) 0),
  floatrebufferf2vfree
};
...
_AFmoduleinst _af_initfloatrebufferf2v(int *data_flow, AFframecount nsamps, 
                                                        _Bool 
                                                             multiple_of)
{
  _AFmoduleinst ret = _AFnewmodinst(data_flow, &floatrebufferf2v);
  floatrebuffer_data *d = _af_malloc(data_flow, sizeof (floatrebuffer_data));
  d->nsamps = nsamps;
  d->offset = d->nsamps;
  d->buf = _af_malloc(data_flow, sizeof (float) * nsamps);
...
typedef struct int2rebuffer_data
{
...
static void int2rebufferv2fmax_push(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
  int nframes = d->nsamps/i->inc->f.channelCount;
...
static void int2rebufferv2frun_push(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
  long samples2push = i->inc->nframes * i->inc->f.channelCount;
  schar2 *inbufp = i->inc->buf;
  ...
  if (d->offset + samples2push >= d->nsamps)
    {
      ...
      if (d->offset != 0)
        memcpy(i->outc->buf, d->buf, sizeof(schar2)*d->offset);
      ...
      if (d->multiple_of)
        {
         ...
         , "libaudiofile/modules/rebuffer.template", 114, __PRETTY_FUNCTION__))
                              ;
          memcpy((schar2 *)i->outc->buf + d->offset, inbufp,
                 sizeof(schar2)*(n - d->offset));
 ...
 , "libaudiofile/modules/rebuffer.template", 163, __PRETTY_FUNCTION__))
  ...
  if ( samples2push != 0 )
    {
      memcpy(d->buf+d->offset, inbufp, sizeof(schar2)*samples2push);
      d->offset += samples2push;
    }
...
static void int2rebufferv2fsync1(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
 ...
 , "libaudiofile/modules/rebuffer.template", 179, __PRETTY_FUNCTION__))
  ...
  memcpy(d->saved_buf, d->buf, sizeof(schar2)*d->nsamps);
  d->saved_offset = d->offset;
}

static void int2rebufferv2fsync2(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
 ...
 , "libaudiofile/modules/rebuffer.template", 197, __PRETTY_FUNCTION__))
  ...
  memcpy(d->buf, d->saved_buf, sizeof(schar2)*d->nsamps);
  d->offset = d->saved_offset;
...
static void int2rebufferv2ffree(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
  if (d->buf)
    free(d->buf);
...
static const _AFmodule int2rebufferv2f =
{
  "int2" "rebufferv2f",
  ((void *) 0),
  ((void *) 0), int2rebufferv2fmax_push,
  ((void *) 0), ((void *) 0), ((void *) 0),
  int2rebufferv2frun_push, int2rebufferv2fsync1, int2rebufferv2fsync2,
  ((void *) 0),
  int2rebufferv2ffree
};
...
_AFmoduleinst _af_initint2rebufferv2f(int *data_flow, AFframecount nsamps, 
                                                        _Bool 
                                                             multiple_of)
{
  _AFmoduleinst ret = _AFnewmodinst(data_flow, &int2rebufferv2f);
  int2rebuffer_data *d = _af_malloc(data_flow, sizeof (int2rebuffer_data));
  d->nsamps = nsamps;
  d->offset = 0;
  d->buf = _af_malloc(data_flow, sizeof (schar2) * nsamps);
  d->multiple_of = multiple_of;
  d->saved_buf = _af_malloc(data_flow, sizeof (schar2) * nsamps);
  ret.modspec = d;
...
static void int2rebufferf2vmax_pull(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
  long nframes = d->nsamps / i->inc->f.channelCount;
...
static void int2rebufferf2vrun_pull(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
  long samples2pull = i->outc->nframes * i->outc->f.channelCount;
  schar2 *outbufp = i->outc->buf;
  ...
  if (d->offset != d->nsamps)
    {
      int buffered = d->nsamps - d->offset;
      int n = ((samples2pull)<(buffered) ? (samples2pull) : (buffered));
      memcpy(outbufp, d->buf+d->offset, sizeof(schar2)*n);
  ...
  while (!d->eof && samples2pull > 0)
    {
     ...
     , "libaudiofile/modules/rebuffer.template", 405, __PRETTY_FUNCTION__))
      ...
      memcpy(outbufp, i->inc->buf, sizeof(schar2)*((samples2pull)<(got) ? (samples2pull) : (got)));
      ...
      if (samples2pull < 0)
        {
         ...
         , "libaudiofile/modules/rebuffer.template", 443, __PRETTY_FUNCTION__))
          ...
          memcpy(d->buf+d->offset, (schar2 *)i->inc->buf+d->offset,
                 sizeof(schar2)*(d->nsamps-d->offset));
        }
      else
        {
...
static void int2rebufferf2vreset1(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
  d->offset = d->nsamps;
  d->eof = 
...
static void int2rebufferf2vreset2(int *data_flow, struct _AFmoduleinst *i)
{
...
static void int2rebufferf2vfree(int *data_flow, struct _AFmoduleinst *i)
{
  int2rebuffer_data *d = i->modspec;
  if (d->buf)
    free(d->buf);
...
static const _AFmodule int2rebufferf2v =
{
  "int2" "rebufferf2v",
  ((void *) 0),
  int2rebufferf2vmax_pull, ((void *) 0),
  int2rebufferf2vrun_pull, int2rebufferf2vreset1, int2rebufferf2vreset2,
  ((void *) 0), ((void *) 0), ((void *) 0),
  ((void *) 0),
  int2rebufferf2vfree
};
...
_AFmoduleinst _af_initint2rebufferf2v(int *data_flow, AFframecount nsamps, 
                                                        _Bool 
                                                             multiple_of)
{
  _AFmoduleinst ret = _AFnewmodinst(data_flow, &int2rebufferf2v);
  int2rebuffer_data *d = _af_malloc(data_flow, sizeof (int2rebuffer_data));
  d->nsamps = nsamps;
  d->offset = d->nsamps;
  d->buf = _af_malloc(data_flow, sizeof (schar2) * nsamps);
