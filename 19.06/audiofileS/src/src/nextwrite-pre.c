...
status _af_next_update (int *data_flow, AFfilehandle file);

static uint32_t nextencodingtype (int *data_flow, _AudioFormat *format);
static status next_write_header (int *data_flow, AFfilehandle file);


status _af_next_update (int *data_flow, AFfilehandle file)
{
 next_write_header(data_flow, file);
 return (0);
}
...
static status next_write_header (int *data_flow, AFfilehandle file)
{
 _Track *track={0};
 int frameSize={0};
 uint32_t offset, length, encoding, sampleRate, channelCount={0};

 track = _af_filehandle_get_track(data_flow, file, AF_DEFAULT_TRACK);

 frameSize = _af_format_frame_size(data_flow, &track->f, 
                                             0
                                                  );

 offset = track->fpos_first_frame;
 length = track->totalfframes * frameSize;
 encoding = nextencodingtype(data_flow, &track->f);
 sampleRate = track->f.sampleRate;
 channelCount = track->f.channelCount;

 if (af_fseek(data_flow, file->fh, 0, 
                          0
                                  ) != 0)
  _af_error(data_flow, AF_BAD_LSEEK, "bad seek");

 af_fwrite(data_flow, ".snd", 4, 1, file->fh);
 af_write_uint32_be(data_flow, &offset, file->fh);
 af_write_uint32_be(data_flow, &length, file->fh);
 af_write_uint32_be(data_flow, &encoding, file->fh);
 af_write_uint32_be(data_flow, &sampleRate, file->fh);
 af_write_uint32_be(data_flow, &channelCount, file->fh);
...
status _af_next_write_init (int *data_flow, AFfilesetup setup, AFfilehandle filehandle)
{
 _Track *track={0};
 ...
 {
  _af_error(data_flow, AF_BAD_NUMMISC, "NeXT format supports no miscellaneous chunks");
  return (-1);
 }

 next_write_header(data_flow, filehandle);
